...
int TagColumn::open(const std::string& col_file_name, const std::string &db_path,
  ...
  int error_code;
  if ((error_code = MMapFile::open(col_file_name, db_path, dbname, flags)) < 0) {
  ...
  if (m_attr_.m_data_type == VARSTRING || m_attr_.m_data_type == VARBINARY) {
    ...
    if ((error_code = m_str_file_->open(col_str_file_name, db_path_, dbname, flags)) < 0) {
...
int TagColumn::rename(std::string& new_col_file_name) {
  ...
  if (m_str_file_) {
    ...
    if ((m_str_file_->open(new_str_file_name, m_db_path_, m_db_name_, MMAP_OPEN)) < 0) {
...
int MMapTagColumnTable::initMetaData(ErrorInfo &err_info) {
  ...
  TagInfo ainfo = {0x00};
  ainfo.m_offset = 0;
  ainfo.m_size = BigObjectConfig::pageSize();
  m_meta_file_ = new TagColumn(-1, ainfo);
  err_info.errcode = m_meta_file_->open(file_name, db_path_, tbl_sub_path_, flags_);
...
int MMapTagColumnTable::open_(const string &url, const std::string &db_path,
  ...
  setObjectReady();
  LOG_DEBUG("open the tag table %s%s succssfully",
           tbl_sub_path.c_str(), m_name_.c_str());
  return 0;
}
...
int MMapTagColumnTable::create_mmap_file(const string& url, const std::string& db_path,
  ...
  m_name_ = getURLObjectName(url);
  int error_code = MMapFile::open(file_name, db_path, tbl_sub_path, flags, metaDataSize(), err_info);
  if (error_code < 0) {
    err_info.setError(error_code);
    LOG_ERROR("failed to open the tag file %s%s, error: %s",
  ...
  // m_meta_data_->m_mem_length = file_length_;
  return err_info.errcode;
}
...
int MMapTagColumnTable::initBitMapColumn(ErrorInfo& err_info) {
  ...
  err_info.errcode = m_bitmap_file_->open(bitmap_file_name, db_path_, tbl_sub_path_, flags_);
  if (err_info.errcode < 0) {
    err_info.errmsg = "initBitMapColumn failed.";
    LOG_ERROR("failed to open the bitmap file %s%s, error: %s",
              tbl_sub_path_.c_str(), bitmap_file_name.c_str(), err_info.errmsg.c_str());
  }
  return err_info.errcode;
}
...
int MMapTagColumnTable::initIndex(ErrorInfo& err_info) {
  ...
  err_info.errcode = m_index_->open(index_name, db_path_, tbl_sub_path_, flags_, err_info);
  if (err_info.errcode < 0) {
    err_info.errmsg = "initIndex failed.";
    LOG_ERROR("failed to open the tag hash index file %s%s, error: %s",
              tbl_sub_path_.c_str(), index_name.c_str(), err_info.errmsg.c_str())
    return err_info.errcode;
  }
...
int MMapTagColumnTable::readColumnInfo(ErrorInfo& err_info) {
  ...
  for (int idx = 0; idx < m_meta_data_->m_column_count; ++idx) {
    ...
    if (tag_col->attributeInfo().m_tag_type == GENERAL_TAG) {
      if ((error_code = tag_col->open(m_name_ + "." + std::to_string(ainfo.m_id),
                                      db_path_, m_db_name_, flags_)) < 0) {
	    LOG_WARN("faild to open the tag %s%s%s(%u), retry open it after recovery",
          db_path_.c_str(), m_db_name_.c_str(), m_name_.c_str(), ainfo.m_id);
	  delete tag_col;
	  continue;
      }
...
int MMapTagColumnTable::writeColumnInfo(uint64_t start_offset, const std::vector<TagColumn*>& tag_schemas) {
  ...
  if (new_mem_len > BigObjectConfig::pageSize()) {
    //TODO(zhuderun): error
    LOG_ERROR("failed to write tag info for the tag table %s%s, new memory "
      ...
      m_db_name_.c_str(), m_name_.c_str(),
      new_mem_len, BigObjectConfig::pageSize(),
      start_offset, tag_schemas.size(), sizeof(TagInfo));
    return -1;
  }
  if (m_meta_file_->fileLen() < new_mem_len) {
    size_t old_row_count = m_meta_file_->fileLen() / BigObjectConfig::pageSize();
    size_t new_row_count = (getPageOffset(new_mem_len)) / BigObjectConfig::pageSize();
...
int MMapTagColumnTable::create(const vector<TagInfo>& schema, int32_t entity_group_id, ErrorInfo& err_info) {
  ...
  m_meta_data_->m_record_start_offset = file_length_;  // one page size
...
int MMapTagColumnTable::remove() {
  ...
  if (mem_) {
    reinterpret_cast<TagColumnMetaData*>(mem_)->m_droped = true;
  }
...
int MMapTagColumnTable::extend(size_t new_record_count, ErrorInfo& err_info) {
  ...
  if (file_length_ < (m_meta_data_->m_record_start_offset + new_mem_len)) {
    err_code = mremap(m_meta_data_->m_record_start_offset + new_mem_len);
    if (err_code < 0) {
      return err_code;
    }
  }
...
TagColumn* MMapTagColumnTable::cloneMetaData(ErrorInfo& err_info) {
  ...
  err_info.errcode = tmp_meta_file->open(meta_file_name, db_path_, tbl_sub_path_, flags_ | MMAP_CREATTRUNC);
  if (err_info.errcode < 0) {
    err_info.errmsg = "initMetaData failed.";
    LOG_ERROR("failed to open the tag meta file %s%s, error: %s",
              tbl_sub_path_.c_str(), meta_file_name.c_str(), err_info.errmsg.c_str());
    return nullptr;
  }

  // extend backup file
  size_t new_row_count = m_meta_file_->fileLen() / BigObjectConfig::pageSize();
...
TagColumn* MMapTagColumnTable::addNewColumn(TagInfo& tag_schema, bool need_ext, ErrorInfo& err_info) {
  ...
  if ((err_code = tag_column->open(col_file_name,
                                   db_path_, m_db_name_, MMAP_CREAT_EXCL)) < 0) {
    LOG_ERROR("failed to open the tag file %s, error_code: %d",
