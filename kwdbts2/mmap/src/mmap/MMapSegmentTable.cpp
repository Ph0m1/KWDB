...
#include "dirent.h"
#include "objcntl.h"
#include "mmap/MMapSegmentTable.h"
...
int MMapSegmentTable::create(EntityMetaManager* entity_meta, const vector<AttributeInfo>& schema,
                           int encoding, ErrorInfo& err_info) {
  if (init(entity_meta, schema, encoding, err_info) < 0)
    return err_info.errcode;

  meta_data_->magic = magic();
  meta_data_->struct_type |= (ST_COLUMN_TABLE);
  meta_data_->has_data = true;
  if (initColumn(flags_, err_info) < 0)
    return err_info.errcode;
...
int MMapSegmentTable::addColumnFile(int col, int flags, ErrorInfo& err_info) {
  ...
  col_file_[col] = col_file;
  err_info.errcode = col_file->open(col_file_name, db_path_, tbl_sub_path_, flags);
...
int MMapSegmentTable::initColumn(int flags, ErrorInfo& err_info) {
  col_file_.resize(hrchy_info_.size());
  col_block_header_size_.resize(hrchy_info_.size());
  col_block_size_.resize(hrchy_info_.size());
  for (size_t i = 0 ; i < hrchy_info_.size() ; ++i) {
    ...
    col_block_header_size_[i] = entity_meta_->getBlockBitmapSize() + BLOCK_AGG_COUNT_SIZE
                                + hrchy_info_[i].size * BLOCK_AGG_NUM;
    col_block_header_size_[i] = (col_block_header_size_[i] + 63) / 64 * 64;

    col_block_size_[i] = col_block_header_size_[i] + hrchy_info_[i].size * entity_meta_->getBlockMaxRows();
    col_block_size_[i] = (col_block_size_[i] + 63) / 64 * 64;

    // vartype column will use string file to store real values.
    if ((hrchy_info_[i].type == VARSTRING || hrchy_info_[i].type == VARBINARY) && !m_str_file_) {
      ...
      int err_code = m_str_file_->open(col_str_file_name, db_path_, tbl_sub_path_, flags);
      if (err_code < 0) {
        return err_code;
      }
...
int MMapSegmentTable::open_(int char_code, const string& file_path, const std::string& db_path,
  ...
  if ((err_info.errcode = TsTableMMapObject::open(file_path, db_path, tbl_sub_path, char_code, flags))
      < 0) {
    return err_info.errcode;
  }
...
int MMapSegmentTable::open(EntityMetaManager* entity_meta, BLOCK_ID segment_id, const string& file_path,
  ...
  entity_meta_ = entity_meta;
  segment_id_ = segment_id;
  db_path_ = db_path;
  tbl_sub_path_ = tbl_sub_path;
  file_path_ = file_path;
  real_file_path_ = db_path_ + tbl_sub_path_ + file_path;
  // check if we should mount sqfs file
  string segment_dir = db_path + tbl_sub_path;
  string sqfs_path = getSqfsFilePath();
  sqfs_is_exists_ = IsExists(sqfs_path);
  bool is_mounted = isMounted(segment_dir);
  if (!(flags & O_CREAT) && lazy_open) {
    if (sqfs_is_exists_ && !is_mounted && IsExists(segment_dir)) {
      Remove(segment_dir);
    }
    return 0;
  }
  if (sqfs_is_exists_ && !mount(sqfs_path, segment_dir, err_info)) {
  ...
  if (!(flags & O_CREAT) && !sqfs_is_exists_ && getSegmentStatus() >= ImmuWithRawSegment) {
    setSegmentStatus(InActiveSegment);
  }
  ...
  if (meta_data_ != nullptr && _reservedSize() == 0) {
    ...
    setObjectReady();
    err_info.errcode = reserve(entity_meta_->getReservedRows());
    if (err_info.errcode < 0) {
      err_info.setError(err_info.errcode, tbl_sub_path + file_path);
    }
...
int MMapSegmentTable::init(EntityMetaManager* entity_meta, const vector<AttributeInfo>& schema, int encoding,
                         ErrorInfo& err_info) {
  entity_meta_ = entity_meta;
...
int MMapSegmentTable::reopen(bool lazy_open, ErrorInfo &err_info) {
  ...
  string db_path = db_path_;
  string tbl_sub_path = tbl_sub_path_;
  ...
  return open(entity_meta_, segment_id_, file_path, db_path, tbl_sub_path, MMAP_OPEN_NORECURSIVE, lazy_open, err_info);
}
...
int MMapSegmentTable::setAttributeInfo(vector<AttributeInfo>& info, int encoding,
                                     ErrorInfo& err_info) {
  int offset = 0;
...
int MMapSegmentTable::reserveBase(size_t n) {
  ...
  for (size_t i = 0 ; i < hrchy_info_.size() ; ++i) {
    ...
    err_code = col_file_[i]->mremap(entity_meta_->getBlockMaxNum() * col_block_size_[i]);
    if (err_code < 0) {
      break;
    }
  }
...
#if defined(DEBUG_LOCK)
  BO_DEBUG(cout, "<Write Unlock>: " << URL());
#endif
...
int MMapSegmentTable::truncate() {
  ...
  for (size_t column = 0 ; column < hrchy_info_.size() ; ++column) {
    if (col_file_[column] == nullptr) {
      continue;
    }
    // Truncate the unused space in each column of data files
    size_t length = block_num * getDataBlockSize(column);
    error_code = col_file_[column]->resize(length);
    if (error_code < 0) {
      TSObject::stopWrite();
      return error_code;
    }
...
int MMapSegmentTable::pushToColumn(MetricRowID start_row, size_t payload_col, kwdbts::Payload* payload,
                                size_t start_in_payload, const BlockSpan& span, kwdbts::DedupInfo& dedup_info) {


  int error_code = 0;
  size_t segment_col = actual_cols_[payload_col];
  MetricRowID block_first_row = span.block_item->getRowID(1);
  if (actual_hrchy_info_[payload_col].type != DATATYPE::VARSTRING &&
      actual_hrchy_info_[payload_col].type != DATATYPE::VARBINARY) {
    ...
    error_code = push_var_payload_direct(payload, start_row, segment_col, payload_col, start_in_payload, span.row_num);
  ...
  push_null_bitmap(payload, start_row, segment_col, payload_col, start_in_payload, span.row_num);
  ...
  if (hasValue(start_row, span.row_num, segment_col)) {
    if (actual_hrchy_info_[payload_col].type != DATATYPE::VARSTRING &&
      ...
      AggCalculator aggCal(src, columnNullBitmapAddr(start_row.block_id, segment_col),
                           start_row.offset_row, DATATYPE(actual_hrchy_info_[payload_col].type),
                           actual_hrchy_info_[payload_col].size, span.row_num);
      ...
      VarColAggCalculator aggCal(mem, var_mem, columnNullBitmapAddr(start_row.block_id, segment_col),
                                 begin_r.offset_row,
                                 actual_hrchy_info_[payload_col].size, end_r.offset_row - begin_r.offset_row);
...
int MMapSegmentTable::PushPayload(uint32_t entity_id, MetricRowID start_row, kwdbts::Payload* payload,
                                size_t start_in_payload, const BlockSpan& span, kwdbts::DedupInfo& dedup_info) {
  int error_code = 0;
  if (actual_hrchy_info_.size() != actual_cols_.size()) {
  ...
  int payload_col = 1;
  for (size_t i = 1; i < actual_hrchy_info_.size(); ++i) {
    ...
    error_code = pushToColumn(start_row, i, payload, start_in_payload, span, dedup_info);
    if (error_code < 0) {
      return error_code;
    }
    payload_col++;
  ...
  error_code = pushToColumn(start_row, 0, payload, start_in_payload, span, dedup_info);
...
void MMapSegmentTable::push_back_payload(kwdbts::Payload* payload, MetricRowID row_id, size_t segment_column,
  ...
  memcpy(columnAddr(row_id, segment_column), payload->GetColumnAddr(start_row, payload_column),
         num * hrchy_info_[segment_column].size);
  return;
}

int MMapSegmentTable::push_var_payload_direct(kwdbts::Payload* payload, MetricRowID row_id, size_t segment_column,
  ...
  for (int i = 0 ; i < payload_num ; i++) {
    if (!payload->IsNull(payload_column, payload_start_row)) {
      ...
      if (var_offset >= UINT64_MAX / 2) {  // reuse string in stringfile.
        ...
        if (hrchy_info_[segment_column].type == DATATYPE::VARSTRING) {
          var_c_len -= 1;
        }
      } else {
    ...
    memcpy(columnAddr(row_id, segment_column), &loc, hrchy_info_[segment_column].size);
    row_id.offset_row++;
    payload_start_row++;
  }
  mutexUnlock();
...
void MMapSegmentTable::push_null_bitmap(kwdbts::Payload* payload, MetricRowID row_id, size_t segment_column,
                                      size_t payload_column, size_t payload_start_row, size_t payload_num) {
...
int MMapSegmentTable::swap_nolock(TSObject* other, ErrorInfo& err_info) {
  MMapSegmentTable* rhs = dynamic_cast<MMapSegmentTable*>(other);
  if (!rhs)
    return -1;
  if (TsTableMMapObject::swap(*rhs) >= 0) {
    std::swap(name_, rhs->name_);
    std::swap(col_file_, rhs->col_file_);
    std::swap(reserved_rows_, rhs->reserved_rows_);
    std::swap(m_str_file_, rhs->m_str_file_);
  }
  return 0;
}
...
void MMapSegmentTable::sync(int flags) {
  meta_data_->num_node = actual_writed_count_.load();
  TsTableMMapObject::sync(flags);
  for (size_t i = 0 ; i < col_file_.size() ; ++i) {
    if (hrchy_info_[i].isFlag(AINFO_DROPPED)) {
      continue;
    }
    if (col_file_[i]) {
      col_file_[i]->sync(flags);
    }
  }
...
int MMapSegmentTable::remove() {
  ...
  for (auto it = col_file_.begin() ; it != col_file_.end() ; ++it) {
    auto& file = *it;
  ...
  TsTableMMapObject::close();
  string sqfs_file_path = getSqfsFilePath();
  if (sqfs_is_exists_) {
...
int MMapSegmentTable::addColumn(AttributeInfo& col_info,
                              ErrorInfo& err_info) {
  return err_info.setError(BOEPERM, "cannot support to add column " + quoteString(name()));
}
...
int convertVarToNum(const std::string& str, DATATYPE new_type, char* data, int32_t old_len,
  ...
  try {
    ...
    return err_info.setError(BOEPERM, "Incorrect integer value '" + str + "'");
  }
  catch (std::out_of_range const &ex) {
    return err_info.setError(BOEPERM, "Out of range value '" + str + "'");
  }
  if (pos < str.size()) {
    return err_info.setError(BOEPERM, "Data truncated '" + str + "'");
  }
