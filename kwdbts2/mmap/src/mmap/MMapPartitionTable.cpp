...
impl_latch_virtual_func(MMapPartitionTable, rw_latch_)

MMapPartitionTable::~MMapPartitionTable() {
...
int MMapPartitionTable::open(const string& url, const std::string& db_path, const string& tbl_sub_path,
  ...
  // load EntityMeta
  if (openInitEntityMeta(flags, err_info) < 0) {
...
int MMapPartitionTable::loadSegments(ErrorInfo& err_info) {
  releaseSegments();
  ...
  if (last_sta_tbl != nullptr) {
    ...
    last_sta_tbl->verifySchema(root_table_->getSchemaInfo(), is_consistent);  // handle add/drop column exception
    if (!is_consistent) {
...
int MMapPartitionTable::init(const vector<AttributeInfo>& schema, int encoding, bool init_data, ErrorInfo& err_info) {
  return err_info.errcode;
}

MMapSegmentTable* MMapPartitionTable::createSegmentTable(BLOCK_ID segment_id, ErrorInfo& err_info) {
  // Check if the metadata manager has a valid first meta entry.
  if (meta_manager_.GetFirstMeta() == nullptr) {
    err_info.setError(BOEINVALPATH, db_path_ + tbl_sub_path_ + " is not data path.");
    return nullptr;
  }
  ...
  {
    // Scope for read-locking and unlocking the root table during creation.
    root_table_->rdLock();
    if (segment_tbl->create(&meta_manager_, root_table_->hierarchyInfo(), encoding, err_info) < 0) {
      root_table_->unLock();
      return nullptr;
    }
    root_table_->unLock();
...
int MMapPartitionTable::openInitEntityMeta(const int flags, ErrorInfo& err_info) {
  ...
  if (!(flags & O_CREAT)) {
    ...
    if (stat(meta_path.c_str(), &buff) != 0) {
      err_info.setError(BOENOOBJ, meta_path);
      return err_info.errcode;
    }
  }
...
void MMapPartitionTable::sync(int flags) {
...
size_t MMapPartitionTable::size(uint32_t entity_id) const {
  return meta_manager_.getEntityItem(entity_id)->row_written;
}

ostream& MMapPartitionTable::printRecord(uint32_t entity_id, std::ostream& os, size_t row) {
  os << "[ not support ]\n";
  return os;
}

int MMapPartitionTable::ProcessDuplicateData(kwdbts::Payload* payload, size_t start_in_payload, size_t count,
...
int64_t MMapPartitionTable::push_back_payload(kwdbts::kwdbContext_p ctx, uint32_t entity_id, kwdbts::Payload* payload,
  ...
  int deleted_rows = 0;
  {
    ...
    err_code = AllocateAllDataSpace(entity_id, num, alloc_spans);
    if (err_code < 0) {
      err_info.errcode = err_code;
...
void MMapPartitionTable::Compress(const timestamp64& compress_ts, ErrorInfo& err_info) {
...
int MMapPartitionTable::Flush(kwdbts::TS_LSN check_lsn, ErrorInfo& err_info) {
  sync(MS_SYNC);
  meta_manager_.sync(MS_SYNC);
  return 0;
}

int MMapPartitionTable::Flush(kwdbts::TS_LSN check_lsn, map<uint32_t, uint64_t>& rows, ErrorInfo& err_info) {
  sync(MS_SYNC);
  meta_manager_.sync(MS_SYNC);
...
int MMapPartitionTable::DeleteEntity(uint32_t entity_id, kwdbts::TS_LSN lsn, uint64_t* count, ErrorInfo& err_info) {
  *count = 0;
  EntityItem* entity_item = getEntityItem(entity_id);
...
int MMapPartitionTable::DeleteData(uint32_t entity_id, kwdbts::TS_LSN lsn, const std::vector<KwTsSpan>& ts_spans,
...
int MMapPartitionTable::UndoPut(uint32_t entity_id, kwdbts::TS_LSN lsn, uint64_t start_row, size_t num,
...
int MMapPartitionTable::UndoDelete(uint32_t entity_id, kwdbts::TS_LSN lsn,
...
int MMapPartitionTable::UndoDeleteEntity(uint32_t entity_id, kwdbts::TS_LSN lsn, uint64_t* count, ErrorInfo& err_info) {
  int err_code = 0;
  EntityItem* entity_item = getEntityItem(entity_id);
...
int MMapPartitionTable::RedoPut(kwdbts::kwdbContext_p ctx, uint32_t entity_id, kwdbts::TS_LSN lsn,
...
int MMapPartitionTable::RedoDelete(uint32_t entity_id, kwdbts::TS_LSN lsn,
                                   const vector<DelRowSpan>* rows, ErrorInfo& err_info) {
  for (auto row_span : *rows) {
...
int MMapPartitionTable::GetAllBlockItems(uint32_t entity_id, std::deque<BlockItem*>& block_items, bool reverse) {
  return meta_manager_.GetAllBlockItems(entity_id, block_items, reverse);
}

BlockItem* MMapPartitionTable::GetBlockItem(BLOCK_ID item_id) {
  return meta_manager_.GetBlockItem(item_id);
}

int MMapPartitionTable::AllocateAllDataSpace(uint entity_id, size_t batch_num, std::vector<BlockSpan>* spans) {
  int err_code = 0;
  ...
  do {
    ...
    err_code = allocateBlockItem(entity_id, &block_item);
    if (err_code < 0) {
...
int MMapPartitionTable::GetAndAllocateAllDataSpace(uint entity_id, size_t batch_num, uint64_t start_row,
...
int MMapPartitionTable::AllocateSpace(uint entity_id, size_t batch_num, BLOCK_ID* segment_id, BlockSpan* span) {
  KWDB_DURATION(StStatistics::Get().alloc_space);
  assert(entity_id > 0);
  int err_code = 0;
  BlockItem* block_item = nullptr;
  MUTEX_LOCK(partition_table_latch_);
  err_code = allocateBlockItem(entity_id, &block_item);
  if (err_code != BOENOEGMENT && err_code < 0) {
    MUTEX_UNLOCK(partition_table_latch_);
    return err_code;
  }
  span->start_row = block_item->alloc_row_count;
  span->row_num = std::min(batch_num, size_t(meta_manager_.getBlockMaxRows() - block_item->alloc_row_count));
  span->block_item = block_item;
  block_item->alloc_row_count += span->row_num;
  MUTEX_UNLOCK(partition_table_latch_);
  *segment_id = block_item->block_id;
  return err_code;
}

int MMapPartitionTable::allocateBlockItem(uint entity_id, BlockItem** blk_item) {
  int err_code = 0;
  BlockItem* block_item = nullptr;
  ...
  if (need_add_block_item) {
    ...
    if (segment_block_num >= meta_manager_.getBlockMaxNum()) {
      active_segment_ = nullptr;
    }
    ...
    if (active_segment_ == nullptr || active_segment_->getSegmentStatus() >= InActiveSegment) {
      ...
      MMapSegmentTable* tbl = createSegmentTable(next_segment_id, err_info);
    ...
    meta_manager_.UpdateEnityItem(entity_id, block_item);
  }
...
void MMapPartitionTable::publish_payload_space(const std::vector<BlockSpan>& alloc_spans,
 const std::vector<MetricRowID>& delete_rows, uint32_t entity_id, bool success) {
  // All requested space is marked as deleted space.
  if (!success) {
    for (size_t i = 0 ; i < alloc_spans.size() ; i++) {
      meta_manager_.MarkSpaceDeleted(entity_id, const_cast<BlockSpan*>(&(alloc_spans.at(i))));
    }
  } else {
    for (size_t i = 0; i < delete_rows.size(); i++) {
      if (delete_rows[i].block_id == 0) {
        continue;
      }
      BlockItem* cur_blk_item = meta_manager_.GetBlockItem(delete_rows[i].block_id);
      cur_blk_item->setDeleted(delete_rows[i].offset_row);
    }
  }
}

int MMapPartitionTable::AlterTable(AttributeInfo& attr_info, ErrorInfo& err_info) {
  // stop current segments inserting while add/drop/alter column
  // then will create new segments using new schema when inserting data to this partition
  MUTEX_LOCK(segments_lock_);
...
int MMapPartitionTable::PrepareDup(kwdbts::DedupRule dedup_rule, uint32_t entity_id,
...
int MMapPartitionTable::mergeToPayload(const vector<AttributeInfo>& schema_info, const vector<uint32_t>& actual_cols,
      ...
      for (int i = payload_dedup.size() - 1; i >= 0; i--) {
        ...
        if (schema_info[column].type != DATATYPE::VARSTRING && schema_info[column].type != DATATYPE::VARBINARY) {
          memcpy(payload->GetColumnAddr(merge_row, column),
                 payload->GetColumnAddr(payload_dedup[i], actual_cols[column]),
              schema_info[column].size);
        } else {
          memcpy(payload->GetColumnAddr(merge_row, column),
                 payload->GetColumnAddr(payload_dedup[i], actual_cols[column]), PAYLOAD_VARCOLUMN_TUPLE_LEN);
        }
    ...
    if (dedup_info.table_real_rows.find(cur_ts) != dedup_info.table_real_rows.end()) {
      for (int i = dedup_info.table_real_rows[cur_ts].size() - 1; i >= 0; i--) {
        ...
        if (segment_tbl->getSchemaInfo()[column].type != schema_info[column].type) {
          auto err_code = ConvertDataType(static_cast<DATATYPE>(segment_tbl->getSchemaInfo()[column].type),
                                          static_cast<DATATYPE>(schema_info[column].type), payload, merge_row, column,
                                          segment_tbl, dedup_info.table_real_rows[cur_ts][i], actual_cols[column]);
          if (err_code < 0) {
            return err_code;
          }
        } else {
          if (schema_info[column].type != DATATYPE::VARSTRING && schema_info[column].type != DATATYPE::VARBINARY) {
            memcpy(payload->GetColumnAddr(merge_row, column), segment_tbl->columnAddr(dedup_info.table_real_rows[cur_ts][i], actual_cols[column]),
                   schema_info[column].size);
          } else {
            // copy varbytes to payload.
            std::shared_ptr<void> var_addr = segment_tbl->varColumnAddr(dedup_info.table_real_rows[cur_ts][i], actual_cols[column]);
            // multiplex highest bit for mark this is entity offset.
            payload->tmp_var_col_values_4_dedup_merge_.push_back(var_addr);
            KUint64(payload->GetColumnAddr(merge_row, column)) = payload->tmp_var_col_values_4_dedup_merge_.size() - 1 + UINT64_MAX / 2;
          }
        }
        // The merged column now contains values; the bitmap needs to be updated accordingly
        setRowValid(payload->GetNullBitMapAddr(column), merge_row + 1);
...
int MMapPartitionTable::updatePayloadUsingDedup(uint32_t entity_id, const BlockSpan& first_span,
  ...
  root_table_->rdLock();
  auto schema_info = root_table_->getActualSchemaInfo();
  auto actual_cols =  root_table_->getActualCols();
  root_table_->unLock();
  bool ts_with_lsn = schema_info[0].type == DATATYPE::TIMESTAMP64_LSN;
  ...
  for (int i = 0; i < num; ++i) {
    timestamp64 current_ts = *(reinterpret_cast<timestamp64*>((intptr_t) (ts_begin) + (start_in_payload + i) * schema_info[0].size));
  ...
  if (payload->dedup_rule_ == DedupRule::MERGE) {
    ...
    // update payload.
    for (size_t i = start_in_payload; i < start_in_payload + num; i++) {
      err_code = mergeToPayload(schema_info, actual_cols, payload, i, dedup_info);
      if (err_code < 0) {
        return err_code;
      }
    }
...
int MMapPartitionTable::GetDedupRowsInBlk(uint entity_id, BlockItem* block_item, int read_count, kwdbts::DedupInfo& dedup_info, bool has_lsn) {
...
void MMapPartitionTable::waitBlockItemDataFilled(uint entity_id, BlockItem* block_item, int read_count, bool has_lsn) {
...
int MMapPartitionTable::GetDedupRows(uint entity_id, const BlockSpan& first_span, kwdbts::DedupInfo& dedup_info, bool has_lsn) {
  int err_code = 0;
...
int MMapPartitionTable::ConvertDataType(DATATYPE old_type, DATATYPE new_type, kwdbts::Payload* payload,
                                        size_t payload_start_row, size_t column,
                                        std::shared_ptr<MMapSegmentTable>& segment_table,
                                        MetricRowID dup_real_r, uint32_t actual_c) {

  std::shared_ptr<void> var_mem;
  int ret = ConvertDataTypeToMem(old_type, new_type, payload->GetColumnAddr(payload_start_row, column),
                                 &var_mem, segment_table, dup_real_r, actual_c);
  if (ret < 0) {
    return ret;
  }
  // multiplex highest bit for mark this is entity offset.
  if (new_type == DATATYPE::VARSTRING || new_type == DATATYPE::VARBINARY) {
    payload->tmp_var_col_values_4_dedup_merge_.push_back(var_mem);
    KUint64(payload->GetColumnAddr(payload_start_row, column)) =
        payload->tmp_var_col_values_4_dedup_merge_.size() - 1 + UINT64_MAX / 2;
  }
  return ret;
}

int MMapPartitionTable::ConvertDataTypeToMem(DATATYPE old_type, DATATYPE new_type, char* mem,
                                             std::shared_ptr<void>* var_mem, std::shared_ptr<MMapSegmentTable>& segment_table,
                                             MetricRowID dup_real_r, uint32_t actual_c) {
  ErrorInfo err_info;
  if (new_type != DATATYPE::VARSTRING && new_type != DATATYPE::VARBINARY) {
    if (old_type != DATATYPE::VARSTRING && old_type != DATATYPE::VARBINARY) {
      if (new_type == DATATYPE::CHAR || new_type == DATATYPE::BINARY) {
        err_info.errcode = convertFixedToStr(old_type, (char*)segment_table->columnAddr(dup_real_r, actual_c),
                                             mem, err_info);
        if (err_info.errcode < 0) {
          return err_info.errcode;
        }
      } else {
        err_info.errcode = convertFixedToFixed(old_type, new_type, (char*)segment_table->columnAddr(dup_real_r, actual_c),
                                               mem, err_info);
        if (err_info.errcode < 0) {
          return err_info.errcode;
        }
      ...
      auto var_data = segment_table->varColumnAddr(dup_real_r, actual_c);
      std::string v_value((char*) var_data.get() + MMapStringFile::kStringLenLen);
      convertVarToNum(v_value, new_type,
                      mem, KInt16(var_data.get()), err_info);
    }
  } else {
    if (old_type != DATATYPE::VARSTRING && old_type != DATATYPE::VARBINARY) {
      auto cur_var_data = convertFixedToVar(old_type, new_type,
                                            static_cast<char*>(segment_table->columnAddr(dup_real_r, actual_c)), err_info);
      *var_mem = cur_var_data;
    } else {
      if (old_type == VARSTRING) {
        auto old_data = segment_table->varColumnAddr(dup_real_r, actual_c);
        auto old_len = KInt16(old_data.get()) - 1;
        ...
        *var_mem = ptr;
        ...
        auto old_data = segment_table->varColumnAddr(dup_real_r, actual_c);
        auto old_len = KInt16(old_data.get());
        char* var_data = static_cast<char*>(std::malloc(old_len + 1 + MMapStringFile::kStringLenLen));
        memset(var_data, 0, old_len + 1 + MMapStringFile::kStringLenLen);
        KInt16(var_data) = old_len + 1;
        memcpy(var_data + MMapStringFile::kStringLenLen, (char*) old_data.get() +
                                                         MMapStringFile::kStringLenLen, old_len);
        std::shared_ptr<void> ptr(var_data, free);
        *var_mem = ptr;
      }
    }
...
int MMapPartitionTable::ConvertDataTypeToMem(DATATYPE old_type, DATATYPE new_type, int32_t new_type_size, void* old_mem,
  ...
  if (new_type != DATATYPE::VARSTRING && new_type != DATATYPE::VARBINARY) {
    ...
    if (old_type != DATATYPE::VARSTRING && old_type != DATATYPE::VARBINARY) {
      if (new_type == DATATYPE::CHAR || new_type == DATATYPE::BINARY) {
        ...
        err_info.errcode = convertFixedToFixed(old_type, new_type, (char*)old_mem, (char*)temp_new_mem, err_info);
      }
      if (err_info.errcode < 0) {
        free(temp_new_mem);
        return err_info.errcode;
      ...
      convertVarToNum(var_value, new_type, (char*)temp_new_mem, var_len, err_info);
    }
...
string convertFixedToStr(DATATYPE old_type, char* data, ErrorInfo& err_info) {
  ...
  switch (old_type) {
    ...
    default:
      err_info.setError(BOEPERM, "Incorrect integer value");
      break;
  }
  return res;
}

int MMapPartitionTable::CheckAlterValid(const std::vector<uint32_t>& entities, int col_index, DATATYPE new_type,
                                        bool& is_valid, ErrorInfo& err_info) {
  for (auto& entity_id: entities) {
    EntityItem* entity_item = getEntityItem(entity_id);
    if (entity_item->row_written <= 0 || entity_item->is_deleted)
      continue;
    std::deque<BlockItem*> block_item_queue;
    GetAllBlockItems(entity_id, block_item_queue);

    while (!block_item_queue.empty()) {
      BlockItem* cur_block_item = block_item_queue.front();
      block_item_queue.pop_front();

      std::shared_ptr<MMapSegmentTable> segment_tbl = getSegmentTable(cur_block_item->block_id);
      if (segment_tbl == nullptr) {
        LOG_WARN("Segment [%s] is null", (db_path_ + segment_tbl_sub_path(segment_tbl->segment_id())).c_str());
        continue;
      }
      if (!segment_tbl->isColExist(col_index)) {
        continue;
      }
      auto old_info = segment_tbl->getSchemaInfo();

      k_uint32 cur_row = 1;
      while (cur_row <= cur_block_item->publish_row_count) {
        bool is_deleted;
        if (cur_block_item->isDeleted(cur_row, &is_deleted) < 0) {
          return -1;
        }
        if (is_deleted) {
          ++cur_row;
        ...
        MetricRowID real_row = cur_block_item->getRowID(cur_row);
        if (segment_tbl->isNullValue(real_row, col_index)) {
          ++cur_row;
          continue;
        }
        if (old_info[col_index].type == DATATYPE::VARSTRING || old_info[col_index].type == DATATYPE::VARBINARY) {
          std::shared_ptr<void> data = segment_tbl->varColumnAddr(real_row, col_index);
          uint16_t len = *(static_cast<uint16_t*>(data.get()));
          if (old_info[col_index].type == DATATYPE::VARSTRING) {
            // VARSTRING type need truncate \0 at tail when construct std::string
            len -= 1;
          }
          string value((char*) data.get() + MMapStringFile::kStringLenLen, len);
          if (tryAlterType(value, new_type, err_info) < 0) {
            is_valid = false;
            return 0;
          }
        } else {
          string str_tmp = convertFixedToStr(static_cast<DATATYPE>(old_info[col_index].type),
                                              (char *) segment_tbl->columnAddr(real_row, col_index), err_info);
          if (tryAlterType(str_tmp, new_type, err_info) < 0) {
            is_valid = false;
            return 0;
          }
        }
...
int MMapPartitionTable::tryAlterType(const std::string& str, DATATYPE new_type, ErrorInfo& err_info) {
  std::size_t pos{};
  int res = 0;
  ...
  try {
    ...
    return err_info.setError(BOEPERM, incorrect_str + value_str);
  }
  catch (std::out_of_range const &ex) {
    return err_info.setError(BOEPERM, "Out of range value" + value_str);
  }
  if (pos < str.size()) {
    return err_info.setError(BOEPERM, incorrect_str + value_str);
  }
...
int MMapPartitionTable::FindFirstBlockItem(uint32_t entity_id, kwdbts::TS_LSN lsn, timestamp64 start_payload_ts,
                                           std::deque<BlockItem*>& block_items,
