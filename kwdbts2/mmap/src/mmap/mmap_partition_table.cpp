...
int TsTimePartition::loadSegments(ErrorInfo& err_info) {
    ...
    if (dir_ptr) {
      ...
      while ((entity = readdir(dir_ptr)) != nullptr) {
        ...
        if (segment_id > 0 && segment_ids.count(segment_id) == 0) {
          ...
          if (tbl->open(&entity_idx_manager_, segment_id, file_path, db_path_, segment_tbl_sub_path(segment_id),
  ...
  if (last_sta_tbl != nullptr) {
    ...
    last_sta_tbl->verifySchema(root_table_->getSchemaInfoWithHidden(), is_consistent);  // handle add/drop column exception
    if (!is_consistent) {
...
MMapSegmentTable* TsTimePartition::createSegmentTable(BLOCK_ID segment_id, ErrorInfo& err_info) {
  // Check if the metadata manager has a valid first meta entry.
  if (entity_idx_manager_.GetFirstMeta() == nullptr) {
  ...
  string file_path = name_ + ".bt";
  if (segment_tbl->open(&entity_idx_manager_, segment_id, file_path, db_path_, segment_sand, MMAP_CREAT_EXCL, true, err_info) < 0) {
    return nullptr;
  }
  ...
  int encoding = ENTITY_TABLE | NO_DEFAULT_TABLE;
  {
    ...
    root_table_->rdLock();
    if (segment_tbl->create(&entity_idx_manager_, root_table_->colsInfoWithHidden(), encoding, err_info) < 0) {
      root_table_->unLock();
      return nullptr;
    }
    root_table_->unLock();
  ...
  err_info.errcode = segment_tbl->reserve(entity_idx_manager_.getReservedRows());
  if (err_info.errcode < 0) {
...
int TsTimePartition::openInitEntityMeta(const int flags, ErrorInfo& err_info) {
  ...
  int ret = entity_idx_manager_.Open(meta_url, db_path_, tbl_sub_path_, true);
  if (ret < 0) {
...
int TsTimePartition::remove() {
  ...
  error_code = entity_idx_manager_.remove();

  return error_code;
}

void TsTimePartition::sync(int flags) {
  entity_idx_manager_.sync(flags);
...
size_t TsTimePartition::size(uint32_t entity_id) const {
  return entity_idx_manager_.getEntityItem(entity_id)->row_written;
}
...
int64_t TsTimePartition::push_back_payload(kwdbts::kwdbContext_p ctx, uint32_t entity_id, kwdbts::Payload* payload,
  ...
  for (size_t idx = 0; idx < alloc_spans->size(); idx++) {
    ...
    MetricRowID entity_num_node = entity_idx_manager_.GetFileStartRow(span);
      ...
      case ActiveInWriteSegment:
        if (maxTimestamp() >= compress_ts) {
          ...
          if (segment_tbl->maxTimestamp() / 1000 < compress_ts
              && segment_tbl->size() >= 0.9 * entity_idx_manager_.getReservedRows()) {
...
int TsTimePartition::Sync(kwdbts::TS_LSN check_lsn, ErrorInfo& err_info) {
  sync(MS_SYNC);
  entity_idx_manager_.sync(MS_SYNC);
  return 0;
}
...
int TsTimePartition::Sync(kwdbts::TS_LSN check_lsn, map<uint32_t, uint64_t>& rows, ErrorInfo& err_info) {
  ...
  std::vector<uint32_t> entities = entity_idx_manager_.getEntities();
...
int TsTimePartition::GetAllBlockItems(uint32_t entity_id, std::deque<BlockItem*>& block_items, bool reverse) {
  return entity_idx_manager_.GetAllBlockItems(entity_id, block_items, reverse);
}
...
int TsTimePartition::AllocateAllDataSpace(uint entity_id, size_t batch_num, std::vector<BlockSpan>* spans) {
  ...
  do {
    ...
    span.row_num = std::min(left_count, size_t(entity_idx_manager_.getBlockMaxRows() - block_item->alloc_row_count));
  ...
  if (err_code < 0) {
    for (size_t i = 0 ; i < spans->size() ; i++) {
      entity_idx_manager_.MarkSpaceDeleted(entity_id, &(spans->at(i)));
    }
    spans->clear();
  }
...
int TsTimePartition::GetAndAllocateAllDataSpace(uint entity_id, size_t batch_num, uint64_t start_row,
  ...
  do {
    ...
    if (pre_block_item != nullptr) {
      ...
      auto row_count = std::min(left_count, size_t(entity_idx_manager_.getBlockMaxRows() - block_item->alloc_row_count));
...
int TsTimePartition::AllocateSpace(uint entity_id, size_t batch_num, BLOCK_ID* segment_id, BlockSpan* span) {
  ...
  MUTEX_LOCK(partition_table_latch_);
  err_code = allocateBlockItem(entity_id, &block_item);
  if (err_code != KWENOEGMENT && err_code < 0) {
    MUTEX_UNLOCK(partition_table_latch_);
    return err_code;
  }
  span->start_row = block_item->alloc_row_count;
  span->row_num = std::min(batch_num, size_t(entity_idx_manager_.getBlockMaxRows() - block_item->alloc_row_count));
  span->block_item = block_item;
  block_item->alloc_row_count += span->row_num;
  MUTEX_UNLOCK(partition_table_latch_);
...
int TsTimePartition::allocateBlockItem(uint entity_id, BlockItem** blk_item) {
  int err_code = 0;
  BlockItem* block_item = nullptr;
  ...
  if (need_add_block_item) {
    ...
    err_code = entity_idx_manager_.AddBlockItem(entity_id, &block_item);
    ...
    BLOCK_ID segment_block_num = entity_idx_manager_.getEntityHeader()->cur_block_id
                                 - entity_idx_manager_.getEntityHeader()->cur_datafile_id;
    if (segment_block_num >= entity_idx_manager_.getBlockMaxNum()) {
      active_segment_ = nullptr;
    }
    ...
    if (active_segment_ == nullptr || active_segment_->getSegmentStatus() >= InActiveSegment) {
      ...
      BLOCK_ID next_segment_id = entity_idx_manager_.getEntityHeader()->cur_block_id;
      MMapSegmentTable* tbl = createSegmentTable(next_segment_id, err_info);
      if (tbl == nullptr || err_info.errcode < 0) {
        entity_idx_manager_.getEntityHeader()->cur_block_id--;
      ...
      entity_idx_manager_.getEntityHeader()->cur_datafile_id = next_segment_id;
    }
    ...
    if (ReachMetaMaxBlock(entity_idx_manager_.getEntityHeader()->cur_block_id)) {
      LOG_FATAL("fatal: partition table block num reach max.");
      return KWENOSPC;
    }
    // Updates the corresponding BlockItem information.
    entity_idx_manager_.UpdateEnityItem(entity_id, block_item);
  }
...
void TsTimePartition::publish_payload_space(const std::vector<BlockSpan>& alloc_spans,
  ...
  if (!success) {
    for (size_t i = 0 ; i < alloc_spans.size() ; i++) {
      entity_idx_manager_.MarkSpaceDeleted(entity_id, const_cast<BlockSpan*>(&(alloc_spans.at(i))));
    }
  } else {
    for (size_t i = 0; i < delete_rows.size(); i++) {
      ...
      BlockItem* cur_blk_item = entity_idx_manager_.GetBlockItem(delete_rows[i].block_id);
      cur_blk_item->setDeleted(delete_rows[i].offset_row);
    }
  }
}

int TsTimePartition::AlterTable(AttributeInfo& attr_info, ErrorInfo& err_info) {
...
int TsTimePartition::mergeToPayload(const vector<AttributeInfo>& schema_info, const vector<uint32_t>& actual_cols,
...
int TsTimePartition::updatePayloadUsingDedup(uint32_t entity_id, const BlockSpan& first_span,
  ...
  root_table_->rdLock();
  auto schema_info = root_table_->getSchemaInfoWithoutHidden();
  auto actual_cols = root_table_->getColsIdxForHidden();
  root_table_->unLock();
...
int TsTimePartition::GetDedupRows(uint entity_id, const BlockSpan& first_span, kwdbts::DedupInfo& dedup_info, bool has_lsn) {
  ...
  entity_idx_manager_.GetAllBlockItems(entity_id, block_items);
...
int TsTimePartition::ConvertDataType(DATATYPE old_type, DATATYPE new_type, kwdbts::Payload* payload,
...
int TsTimePartition::ConvertDataTypeToMem(DATATYPE old_type, DATATYPE new_type, char* mem,
...
int TsTimePartition::CheckAlterValid(const std::vector<uint32_t>& entities, int col_index, DATATYPE new_type,
    ...
    while (!block_item_queue.empty()) {
      ...
      while (cur_row <= cur_block_item->publish_row_count) {
        ...
        if (old_info[col_index].type == DATATYPE::VARSTRING || old_info[col_index].type == DATATYPE::VARBINARY) {
          ...
          if (checkTypeAlter(value, new_type, err_info) < 0) {
            is_valid = false;
            return 0;
          }
        } else {
          ...
          if (checkTypeAlter(str_tmp, new_type, err_info) < 0) {
            is_valid = false;
            return 0;
          }
        }
...
int TsTimePartition::checkTypeAlter(const std::string& str, DATATYPE new_type, ErrorInfo& err_info) {
  std::size_t pos{};
  int res = 0;
