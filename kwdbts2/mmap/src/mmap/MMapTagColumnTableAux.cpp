...
void MMapTagColumnTable::setLSN(kwdbts::TS_LSN lsn) {
  if (mem_) {
    reinterpret_cast<TagColumnMetaData*>(mem_)->m_lsn = lsn;
  }
...
kwdbts::TS_LSN MMapTagColumnTable::getLSN() {
  kwdbts::TS_LSN minLSN = reinterpret_cast<TagColumnMetaData*>(mem_)->m_lsn;
  for (size_t i = 0; i < m_cols_.size(); ++i) {
    if (!m_cols_[i]->isPrimaryTag()) {
...
bool MMapTagColumnTable::isDroped() {
  if (mem_) {
    return reinterpret_cast<TagColumnMetaData*>(mem_)->m_droped;
  } else {
    return false;
  }
}
 ...
 * @Return: 0
 */
int MMapTagColumnTable::flush(kwdbts::TS_LSN lsn) {
  setLSN(lsn);

  MMapFile::sync(MS_SYNC);
  for (size_t i = 0; i < m_cols_.size(); ++i) {
    if (!m_cols_[i]->isPrimaryTag()) {
...
string MMapTagColumnTable::getFileDir() {
  size_t lastSep = realFilePath().find_last_of('/');
  return realFilePath().substr(0, lastSep);
}
...
int MMapTagColumnTable::flipOldMetaFile(bool toOld) {
  ...
  int rc = 0;
  if (toOld) {
    rc = copyFile(oldFileName, newFileName, MMapFile::flags());
    if (rc != 0) {
      return rc;
    }
  } else {
    rc = copyFile(newFileName, oldFileName, MMapFile::flags());
    if (rc != 0) {
      return rc;
    }
  }
...
int MMapTagColumnTable::flipOldFile(TagInfo& info, bool toOld) {
  ...
  int rc = 0;
  if (toOld) {
    rc = copyFile(priOldFileName, priFileName, MMapFile::flags());
    if (rc != 0) {
      return rc;
    }

    if (!secFileName.empty()) {
      rc = copyFile(secOldFileName, secFileName, MMapFile::flags());
    }
  } else {
    rc = copyFile(priFileName, priOldFileName, MMapFile::flags());
    if (rc != 0) {
      return rc;
    }

    if (!secOldFileName.empty()) {
      rc = copyFile(secFileName, secOldFileName, MMapFile::flags());
    }
  }
...
TagColumn* MMapTagColumnTable::openTagCol(int32_t idx, const TagInfo& info) {
  ...
  int rc = tagCol->open(m_name_ + "." + std::to_string(info.m_id),
		    db_path_, m_db_name_, (flags_ & ~O_CREAT));
  if (rc < 0) {
...
int MMapTagColumnTable::AlterAlterRU(TagInfo& oldInfo, TagInfo& newInfo,
  ...
  if (isRedo) {
    ...
    metaInfo.m_offset = 0;
    metaInfo.m_size = BigObjectConfig::pageSize();
    ...
    rc = metaFile->open(m_name_ + ".mt",
                        db_path_, m_db_name_, (flags_ & ~O_CREAT));
    if (rc < 0) {
       LOG_ERROR("faild to open meta file of the tag table %s%s%s, undo failed.",
		db_path_.c_str(), m_db_name_.c_str(), m_name_.c_str());
      delete metaFile;
      return -1;
    }
	...
	if (tagCol == nullptr) {
	    LOG_WARN("faild to open the tag file(%u) of the tag table %s%s%s, "
          "undo failed",
          oldInfo.m_id, db_path_.c_str(), m_db_name_.c_str(), m_name_.c_str());
	  return -1;
	}
    ...
    if (rc != 0) {
      LOG_ERROR("failed to rename the tag file(%u) of the tag table %s%s%s, "
        "undo failed",
        oldInfo.m_id, db_path_.c_str(), m_db_name_.c_str(), m_name_.c_str());
      return -1;
    }
    ...
    tagCol = openTagCol(serialNo, oldInfo);
    if (tagCol == nullptr) {
	LOG_WARN("failed to open the tag file(%u) of the tag table %s%s%s, "
      "undo failed.",
      oldInfo.m_id, db_path_.c_str(), m_db_name_.c_str(), m_name_.c_str());
      return -1;
    }
...
int MMapTagColumnTable::AlterAddRU(TagInfo& newInfo, bool isRedo) {
  ...
  if (isRedo) {
    ...
    int rc = flipOldMetaFile(false);
    if (rc != 0) {
      LOG_ERROR("failed to rename meta file of the tag table %s%s%s, "
        "undo failed",
		db_path_.c_str(), m_db_name_.c_str(), m_name_.c_str());
      return -1;
    }
    ...
    if (m_cols_.back() != tagCol) {
       LOG_ERROR("the added tag(%u) in the tag table %s%s%s is not the last, "
         "undo failed.",
         newInfo.m_id, db_path_.c_str(), m_db_name_.c_str(), m_name_.c_str());
      return -1;
    }
    m_cols_.pop_back();
    tagCol->remove();
...
int MMapTagColumnTable::AlterDropRU(TagInfo& oldInfo, bool isRedo) {
  ...
  if (isRedo) {
    ...
    rc = metaFile->open(m_name_ + ".mt",
                        db_path_, m_db_name_, (flags_ & ~O_CREAT));
    if (rc < 0) {
      LOG_ERROR("failed to open the meta file of the tag table %s%s%s, "
        "undo failed.",
		db_path_.c_str(), m_db_name_.c_str(), m_name_.c_str());
      delete metaFile;
      return -1;
