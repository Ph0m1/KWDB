...
class MMapPartitionTable : public TSObject {
 public:
  using MMapPartitionTableCntMutex = KLatch;
  using MMapPartitionTableCondVal = KCond_t;
  MMapPartitionTableCntMutex* m_ref_cnt_mtx_;
  MMapPartitionTableCondVal* m_ref_cnt_cv_;
 private:
  using MMapPartitionTableRWLatch = KRWLatch;
  using MMapPartitionTableLatch = KLatch;

  MMapPartitionTableRWLatch* rw_latch_;
  MMapPartitionTableLatch* segments_lock_;  // data_segments lock
  MMapPartitionTableLatch* compress_mtx_;
  MMapPartitionTableLatch* partition_table_latch_;  // partition table object latch
 ...
 protected:
  ...
  mutable EntityMetaManager meta_manager_;
 ...
 public:
  explicit MMapPartitionTable(MMapMetricsTable*& root_table, uint16_t config_subgroup_entities) :
        TSObject(), root_table_(root_table) {
    meta_manager_.config_subgroup_entities = config_subgroup_entities;
    rw_latch_ = new MMapPartitionTableRWLatch(RWLATCH_ID_MMAP_PARTITION_TABLE_RWLOCK);
    segments_lock_ = new MMapPartitionTableLatch(LATCH_ID_MMAP_PARTITION_TABLE_SEGMENTS_MUTEX);
    compress_mtx_ = new MMapPartitionTableLatch(LATCH_ID_MMAP_PARTITION_TABLE_COMPRESS_MUTEX);
    m_ref_cnt_mtx_ = new MMapPartitionTableCntMutex(LATCH_ID_PARTITION_REF_COUNT_MUTEX);
    m_ref_cnt_cv_ = new MMapPartitionTableCondVal(COND_ID_PARTITION_REF_COUNT_COND);
    partition_table_latch_ = new MMapPartitionTableLatch(LATCH_ID_PARTITION_TABLE_MUTEX);
  }

  virtual ~MMapPartitionTable();
 ...
 */
  ...
  const vector<AttributeInfo>& getSchemaInfo() const {
    return root_table_->getSchemaInfo();
  }
  ...
  virtual int reserve(size_t size) { return BOEPERM; }
  ...
  inline std::shared_ptr<MMapSegmentTable> getSegmentTable(BLOCK_ID segment_id, bool lazy_open = false) const {
    ...
    if (ret) {
      ...
      if (value->getObjectStatus() != OBJ_READY && !lazy_open) {
        ErrorInfo err_info;
        if (value->open(const_cast<EntityMetaManager*>(&meta_manager_), key, name_ + ".bt",
 ...
 */
  ...
  int Flush(kwdbts::TS_LSN check_lsn, ErrorInfo& err_info);

  int Flush(kwdbts::TS_LSN check_lsn, map<uint32_t, uint64_t>& rows, ErrorInfo& err_info);
