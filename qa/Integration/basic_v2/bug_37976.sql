drop database if exists test_select_last cascade;
...
insert into test_select_last.tb values ('2022-05-01 12:10:31.22','2020-05-01 22:30:11',500,5000,50000,-500000.505,-5000000.505055,true,'h', 'ar2  ', 'c', 'r255测试2()*&^%{}','\\', 'v2551cdf~#   ', '  测试%&!','ar-1', 'ar255()*&^%{}  ','6_1测试1&^%{} ','y', 's1023_1','ytes_2', null,b'\xcc\xcc\xdd\xdd',4,300,300,false,60.666,600.678,'','\\test测试！！！@TEST1',' ','test测试！T  EST1xaa','查询查询 ','\\','','    ','','  ');
...
select * from (select time_bucket(k_timestamp, '10s') as tb,last(t15) as lastVal,last(t16) from test_select_last.tb group by tb,k_timestamp) order by tb,lastVal;

select * from (select time_bucket(k_timestamp, '10s') as tb,last(t15) as lastVal,first(t16) from test_select_last.tb group by tb,k_timestamp) order by tb,lastVal;
