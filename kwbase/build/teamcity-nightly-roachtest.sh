...
# Note that when this script is called, the kwbase binary to be tested
# already exists in the current directory.
KWBASE_BINARY="${PWD}/kwbase.linux-2.6.32-gnu-amd64"
chmod +x "${KWBASE_BINARY}"
...
function upload_stats {
 if [[ "${TC_BUILD_BRANCH}" == "master" ]]; then
      bucket="kwbase-nightly-${CLOUD}"
      if [[ "${CLOUD}" == "gce" ]]; then
	  # GCE, having been there first, gets an exemption.
          bucket="kwbase-nightly"
      fi
