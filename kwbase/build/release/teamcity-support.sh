...
configure_git_ssh_key() {
  ...
  touch .kwbase-teamcity-key
  chmod 600 .kwbase-teamcity-key
  echo "${github_ssh_key}" > .kwbase-teamcity-key
...
push_to_git() {
  ...
  GIT_SSH_COMMAND="ssh -i .kwbase-teamcity-key" git push "$@"
}
