...
image=kwbasedb/builder
version=${USE_BUILDER_IMAGE:-20220509-101000}
...
# Absolute path to the toplevel kwbase directory.
kwbase_toplevel=$(dirname "$(cd "$(dirname "${0}")"; pwd)")
...
mkdir -p "${kwbase_toplevel}"/artifacts
export TMPDIR=$kwbase_toplevel/artifacts
...
host_home=${kwbase_toplevel}/build/builder_home
mkdir -p "${host_home}"
...
#
# We always map the kwbase source directory that contains this script into
...
# setting BUILDER_HIDE_GOPATH_SRC to 1, which results in only the kwbase
...
# alternates_file=${kwbase_toplevel}/.git/objects/info/alternates
...
vols="${vols} --volume=${kwbase_toplevel}:/go/src/github.com/kwbasedb/kwbase${cached_volume_mode}"
vols="${vols} --volume=${GOROOT}:/usr/local/go"

# If ${kwbase_toplevel}/bin doesn't exist on the host, Docker creates it as
...
mkdir -p "${kwbase_toplevel}"/bin{.docker_amd64,}
vols="${vols} --volume=${kwbase_toplevel}/bin.docker_amd64:/go/src/gitee.com/kwbasedb/kwbase/bin${delegated_volume_mode}"
mkdir -p "${kwbase_toplevel}"/lib{.docker_amd64,}
vols="${vols} --volume=${kwbase_toplevel}/lib.docker_amd64:/go/src/gitee.com/kwbasedb/kwbase/lib${delegated_volume_mode}"
...
docker run --init --privileged -i ${tty-} --rm \
  -u "$uid:$gid" \
  ${vols} \
  --workdir="/go/src/gitee.com/kwbasedb/kwbase" \
  --env="TMPDIR=/go/src/gitee.com/kwbasedb/kwbase/artifacts" \
  --env="PAGER=cat" \
  --env="GOTRACEBACK=${GOTRACEBACK-all}" \
  --env="TZ=America/New_York" \
  --env=KWBASE_BUILDER_CCACHE \
  --env=KWBASE_BUILDER_CCACHE_MAXSIZE \
  "${image}:${version}" "$@"
