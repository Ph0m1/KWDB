...
curl -f -s -S -o- https://binaries.kwbasedb.com/kwbase-${release_version}.linux-amd64.tgz | tar xfz - --strip-components 1

image=docker.io/kwbasedb/kwbase-unstable
if [[ "$release_version" != *-* && -z "$FORCE_PUSH_TO_KWBASE_UNSTABLE" ]]; then
  image=docker.io/kwbasedb/kwbase
fi
...
if [[ "$release_version" == v$VERSION_TO_TAG_AS_LATEST.* && -z "$FORCE_PUSH_TO_KWBASE_UNSTABLE" ]]; then
