...
serverUrl=https://teamcity.kwbasedb.com
name=
workDir=../work
tempDir=../temp
systemDir=../system
...
wget https://teamcity.kwbasedb.com/update/buildAgent.zip
...
# alternate system.
git clone --bare https://github.com/kwbasedb/kwbase system/git/kwbase.git
cat > system/git/map <<EOS
https://github.com/kwbasedb/kwbase = kwbase.git
EOS

# For master and the last two release, download the builder and acceptance
# containers.
repo="$GOPATH"/src/github.com/kwbasedb/kwbase
git clone --shared system/git/kwbase.git "$repo"
cd "$repo"
...
do
  git checkout "$branch"
  KWBASE_BUILDER_CCACHE=1 build/builder.sh make test testrace TESTS=-
  # TODO(benesch): store the acceptanceversion somewhere more accessible.
  docker pull $(git grep kwbasedb/acceptance -- '*.go' | sed -E 's/.*"([^"]*).*"/\1/') || true
done
cd -
EOF
write_teamcity_config
