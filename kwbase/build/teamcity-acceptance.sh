...
run ln -s kwbase-linux-2.6.32-gnu-amd64 kwbase  # For the tests that run without Docker.
...
run_text_test gitee.com/kwbasedb/kwbase/pkg/acceptance env TZ=America/New_York stdbuf -eL -oL ./acceptance.test -l "$TMPDIR" -test.v -test.timeout 30m
run cd ../..
