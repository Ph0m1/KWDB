...
int main()
{
    ...
    SQLWCHAR dsn[] = L"PostgreSQL35W"; // dsn
    SQLWCHAR uid[] = L"root";          // uid
    SQLWCHAR pwd[] = L"123456";        // pwd
    ...
    SQLSMALLINT bufferLength = sizeof(cursorName) / sizeof(cursorName[0]);
    SQLSMALLINT actualLength = 0;
    retcode = SQLGetCursorNameW(stmtHandle, cursorName, bufferLength, &actualLength);
    if (SQL_SUCCESS != retcode && SQL_SUCCESS_WITH_INFO != retcode)
    {
        printf("SQLGetCursorNameW failed\n");
    }
    else
    {
        wprintf(L"Cursor Name: %s\n", cursorName);
    }
