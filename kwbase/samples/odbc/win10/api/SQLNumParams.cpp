...
int main()
{
    ...
    SQLWCHAR sqlStmt[] = L"SELECT * FROM t2 WHERE 1 = ?";

    retcode = SQLPrepareW(stmtHandle, sqlStmt, SQL_NTS);
    if (SQL_SUCCESS != retcode && SQL_SUCCESS_WITH_INFO != retcode)
    {
        // handle error
        printf("SQLPrepareW failed\n");
    ...
    retcode = SQLNumParams(stmtHandle, &numParams);
    if (SQL_SUCCESS != retcode && SQL_SUCCESS_WITH_INFO != retcode)
    {
        // handle error
        printf("SQLNumParams failed\n");
    }
    else
    {
