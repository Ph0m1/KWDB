...
void show_error(SQLHANDLE handle, SQLSMALLINT handle_type)
{
    ...
    SQLGetDiagField(handle_type, handle, 1, SQL_DIAG_SQLSTATE, sql_state, sizeof(sql_state), &msg_len);
    SQLGetDiagField(handle_type, handle, 1, SQL_DIAG_NATIVE, &native_error, sizeof(native_error), &msg_len);
    SQLGetDiagField(handle_type, handle, 1, SQL_DIAG_MESSAGE_TEXT, error_msg, sizeof(error_msg), &msg_len);

    wprintf(L"SQLGetDiagField SQL Error : %s - %s\n", sql_state, error_msg);
}
...
int main()
{
    ...
    // Retrieve information about the first column in the result set
    SQLCHAR columnName[MAX_BUF_LEN];
    SQLINTEGER columnNameLen;
    retcode = SQLGetDescField(hdesc, 1, SQL_DESC_NAME, columnName, sizeof(columnName), &columnNameLen);
    if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
    {
        printf("Error getting column information\n");
        show_error(hdesc, SQL_HANDLE_STMT);
        SQLFreeHandle(SQL_HANDLE_STMT, hstmt);
        SQLDisconnect(hdbc);
        SQLFreeHandle(SQL_HANDLE_DBC, hdbc);
        SQLFreeHandle(SQL_HANDLE_ENV, henv);
