...
int main()
{
    ...
    // Enable connection pooling in the environment
    retcode = SQLSetEnvAttr(envHandle, SQL_ATTR_CONNECTION_POOLING, (SQLPOINTER)SQL_CP_ONE_PER_HENV, SQL_IS_UINTEGER);
    if (retcode != SQL_SUCCESS)
    {
        printf("Failed to enable connection pooling in the environment\n");
    }
    else
    {
        printf("Successed to enable connection pooling in the environment\n");
    }
    ...
    SQLHDESC app_row_desc = SQL_NULL_HDESC;
    retcode = SQLSetStmtAttr(stmtHandle, SQL_ATTR_APP_ROW_DESC, (SQLPOINTER)app_row_desc, 0);
