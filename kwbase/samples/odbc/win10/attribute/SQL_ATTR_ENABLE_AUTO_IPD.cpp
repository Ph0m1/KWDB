...
int main()
{
    ...
    // set SQL_ATTR_ENABLE_AUTO_IPD
    retcode = SQLSetStmtAttr(stmtHandle, SQL_ATTR_ENABLE_AUTO_IPD, (SQLPOINTER)SQL_FALSE, 0);
    if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
    {
        return 1;
    }
    SQLULEN autoIPD;
    // get SQL_ATTR_ENABLE_AUTO_IPD
    retcode = SQLGetStmtAttr(stmtHandle, SQL_ATTR_ENABLE_AUTO_IPD, &autoIPD, 0, NULL);
    if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
    {
        return 1;
    }

    printf("SQL_ATTR_ENABLE_AUTO_IPD attribute: %lu\n", autoIPD);
