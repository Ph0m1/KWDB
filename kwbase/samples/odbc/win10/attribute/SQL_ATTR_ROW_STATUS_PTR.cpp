...
int main()
{
    ...
    SQLCHAR col1[256];
    SQLLEN indicator1;
    retcode = SQLBindCol(hstmt, 1, SQL_C_WCHAR, col1, sizeof(col1), &indicator1);
    if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
    {
        wprintf(L"Error binding result set column\n");
        show_error(hstmt, SQL_HANDLE_STMT);
        SQLFreeHandle(SQL_HANDLE_STMT, hstmt);
        SQLDisconnect(hdbc);
        SQLFreeHandle(SQL_HANDLE_DBC, hdbc);
    ...
    while (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO)
    {
        ...
        {
            wprintf(L"Column 1: %s\n", col1);
        }
        else if (retcode == SQL_SUCCESS_WITH_INFO)
        {
            wprintf(L"Column 1: %s (truncated)\n", col1);
        }
        retcode = SQLFetch(hstmt);
    }

    // Check for row status pointer
    SQLPOINTER row_status;
    retcode = SQLGetStmtAttr(hstmt, SQL_ATTR_ROW_STATUS_PTR, &row_status, SQL_IS_POINTER, NULL);
    if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
    {
        wprintf(L"Error getting row status pointer\n");
        show_error(hstmt, SQL_HANDLE_STMT);
    }
    else
    {
        wprintf(L"Row status pointer: %p\n", (void *)row_status);
    }
