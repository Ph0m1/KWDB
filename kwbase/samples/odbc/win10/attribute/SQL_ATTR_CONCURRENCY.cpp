...
int main()
{
    ...
    retcode = SQLSetStmtAttr(stmtHandle, SQL_ATTR_CONCURRENCY, (SQLPOINTER)SQL_CONCUR_READ_ONLY, 0);
    ...
    retcode = SQLGetStmtAttr(stmtHandle, SQL_ATTR_CONCURRENCY, &concurrency, 0, NULL);
    if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
    {
        return 1;
    }
