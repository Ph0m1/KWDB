...
// Define row operation callback function
SQLRETURN SQL_API MyRowOperationCallback(SQLHSTMT hstmt, SQLUSMALLINT operation, SQLUSMALLINT row)
{
    // Your implementation here
    printf("Row operation callback called with operation %d on row %d\n", operation, row);
    return SQL_SUCCESS;
}
...
int main()
{
    ...
    SQLINTEGER timeout = 30; // Connection timeout in seconds
    retcode = SQLSetConnectAttr(connHandle, SQL_ATTR_CONNECTION_TIMEOUT, (SQLPOINTER)timeout, SQL_IS_UINTEGER);
    if (SQL_SUCCESS != retcode)
    {
        // handle error
        return 1;
    ...
    {
        printf("set SQLSetStmtAttr(SQL_ATTR_CONNECTION_TIMEOUT) successfully\n");
    }
    ...
    retcode = SQLSetStmtAttr(stmtHandle, SQL_ATTR_ROW_OPERATION_PTR, (SQLPOINTER)MyRowOperationCallback, 0);
    if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
    {
        return 1;
    }
    printf("set SQLSetStmtAttr(SQL_ATTR_ROW_OPERATION_PTR) successfully\n");
    handle_error(stmtHandle, retcode);
    printf("%d\n", retcode);
