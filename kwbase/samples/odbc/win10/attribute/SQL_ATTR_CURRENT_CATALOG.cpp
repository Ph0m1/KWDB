...
int main()
{
    ...
    SQLCHAR currentCatalog[256] = "defaultdb";
    // Set SQL_ATTR_CURRENT_CATALOG
    retcode = SQLSetConnectAttr(connHandle, SQL_ATTR_CURRENT_CATALOG, currentCatalog, SQL_NTS);
    ...
    {
        printf("Successed to SQL_ATTR_CURRENT_CATALOG: %d\n", retcode);
    }

    // Get current setting of SQL_ATTR_CURRENT_CATALOG
    SQLCHAR retrievedCatalog[256];
    retcode = SQLGetConnectAttr(connHandle, SQL_ATTR_CURRENT_CATALOG, retrievedCatalog, sizeof(retrievedCatalog), NULL);
    if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
    {
        return 1;
    }
    ...
    {
        printf("Successed to SQL_ATTR_CURRENT_CATALOG: %s\n", retrievedCatalog);
    }
    // alloc handle
    ...
    retcode = SQLSetStmtAttr(stmtHandle, SQL_ATTR_MAX_ROWS, (SQLPOINTER)100, SQL_IS_UINTEGER);
    if (SQL_SUCCESS != retcode)
    {
        // handle error
        printf("Failed to allocate statement handle: %d\n", retcode);
    ...
    {
        printf("Successed to SQL_ATTR_MAX_ROWS: %d\n", retcode);
    }
