...
int main()
{
    ...
    retcode = SQLSetStmtAttr(hstmt, SQL_ATTR_IMP_ROW_DESC, (SQLPOINTER)imp_row_desc, SQL_IS_INTEGER);
    if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
    {
        wprintf(L"Error setting statement attribute for implicit row descriptor\n");
    ...
    SQLWCHAR sqlStatement[] = L"SELECT * FROM t12";
    // Execute a query to retrieve data
    retcode = SQLExecDirectW(hstmt, sqlStatement, SQL_NTS);
    if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
    {
        wprintf(L"Error executing statement to retrieve data\n");
        show_error(hstmt, SQL_HANDLE_STMT);
        SQLFreeHandle(SQL_HANDLE_STMT, hstmt);
        SQLDisconnect(hdbc);
        SQLFreeHandle(SQL_HANDLE_DBC, hdbc);
    ...
    // (Code to fetch and display data goes here)
