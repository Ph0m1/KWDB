...
int main()
{
    ...
    retcode = SQLSetConnectAttr(connHandle, SQL_ATTR_AUTOCOMMIT, (SQLPOINTER)SQL_AUTOCOMMIT_OFF, SQL_IS_UINTEGER);
    ...
    {
        printf("Successed to SQL_ATTR_AUTOCOMMIT: %d\n", retcode);
    }
    SQLINTEGER autoCommit;
    // Get current setting of SQL_ATTR_AUTOCOMMIT
    retcode = SQLGetConnectAttr(connHandle, SQL_ATTR_AUTOCOMMIT, &autoCommit, 0, NULL);
    if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
    {
        return 1;
    }
