...
int main()
{
    ...
    SQLCHAR translateLib[1024] = "YourTranslateLib.so";
    SQLINTEGER translateOption = 1;
    retcode = SQLSetConnectAttr(stmtHandle, SQL_ATTR_TRANSLATE_LIB, (SQLPOINTER)translateLib, SQL_NTS);
    if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO)
    {
        printf("SQL_ATTR_TRANSLATE_LIB set successfully to %s\n", translateLib);

        retcode = SQLSetConnectAttr(stmtHandle, SQL_ATTR_TRANSLATE_OPTION, (SQLPOINTER)&translateOption, 0);
        if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO)
        {
            printf("SQL_ATTR_TRANSLATE_OPTION set successfully to %d\n", translateOption);
        }
        else
        {
            printf("SQLSetConnectAttr(SQL_ATTR_TRANSLATE_OPTION) failed\n");
        }
    }
    else
    {
        printf("SQLSetConnectAttr(SQL_ATTR_TRANSLATE_LIB) failed\n");
    }
