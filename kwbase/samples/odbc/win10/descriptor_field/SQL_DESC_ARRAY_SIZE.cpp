...
int main()
{
    ...
    retcode = SQLSetDescFieldW(hdesc, 1, SQL_DESC_ARRAY_SIZE, dataPtr, SQL_IS_POINTER);
    if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
    {
        wprintf(L"Error setting SQL_DESC_ARRAY_SIZE\n");
        show_error(hstmt, SQL_HANDLE_STMT);
        SQLFreeHandle(SQL_HANDLE_STMT, hstmt);
        SQLDisconnect(hdbc);
        SQLFreeHandle(SQL_HANDLE_DBC, hdbc);
    ...
    {
        wprintf(L"Success setting SQL_DESC_ARRAY_SIZE\n");
    }
