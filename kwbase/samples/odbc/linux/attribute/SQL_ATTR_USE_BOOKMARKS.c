...
int main()
{
    ...
    // Set statement attribute for using bookmarks in result set
    SQLUINTEGER useBookmarks = SQL_UB_VARIABLE; // Set the value for using bookmarks
    ret = SQLSetStmtAttr(hstmt, SQL_ATTR_USE_BOOKMARKS, (SQLPOINTER)useBookmarks, 0);
    if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
    {
        printf("Error setting statement attribute for using bookmarks\n");
    ...
    printf("set SQLSetStmtAttr(SQL_ATTR_USE_BOOKMARKS) successfully\n");
