...
int main()
{
    ...
    // Set SQL_ATTR_TXN_ISOLATION
    ret = SQLSetConnectAttr(hdbc, SQL_ATTR_TXN_ISOLATION, (SQLPOINTER)SQL_TXN_READ_COMMITTED, 0);
    if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
    {
        handleOdbcError(hdbc, SQL_HANDLE_DBC, ret);
    ...
    ret = SQLGetConnectAttr(hdbc, SQL_ATTR_TXN_ISOLATION, &txnIsolationLevel, 0, NULL);
    if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
    {
        handleOdbcError(hdbc, SQL_HANDLE_DBC, ret);
        SQLDisconnect(hdbc);
    ...
    printf("Transaction Isolation Level: %lu\n", txnIsolationLevel);
