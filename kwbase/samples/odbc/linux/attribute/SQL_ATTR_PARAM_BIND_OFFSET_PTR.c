...
int main()
{
    ...
    // prepare sql
    SQLCHAR *sql_query = (SQLCHAR *)"INSERT INTO d1.t1 (id) VALUES (?)";

    // binding Parameters
    SQLINTEGER param1 = 123;
    SQLLEN param1_len = SQL_NTS;
    SQLLEN param_bind_offset = 0;

    ret = SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_SLONG, SQL_INTEGER, 0, 0, &param1, 0, &param1_len);
    handle_error(hstmt, ret);

    // set SQL_ATTR_PARAM_BIND_OFFSET_PTR
    ret = SQLSetStmtAttr(hstmt, SQL_ATTR_PARAM_BIND_OFFSET_PTR, &param_bind_offset, 0);
    handle_error(hstmt, ret);
