...
int main()
{
    ...
    // Set statement attribute for maximum column length
    SQLULEN maxLength = 100; // Set maximum column length to 100
    ret = SQLSetStmtAttr(hstmt, SQL_ATTR_MAX_LENGTH, (SQLPOINTER)maxLength, 0);
    if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
    {
        printf("Error setting statement attribute for maximum column length\n");
    ...
    printf("set SQLSetStmtAttr(SQL_ATTR_MAX_LENGTH) successfully\n");
