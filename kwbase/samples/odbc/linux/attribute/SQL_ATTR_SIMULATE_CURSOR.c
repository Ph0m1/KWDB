...
int main()
{
    ...
    // Set statement attribute for simulating cursor behavior
    SQLUINTEGER simulateCursor = SQL_SC_NON_UNIQUE; // Set the value for simulating cursor behavior
    ret = SQLSetStmtAttr(hstmt, SQL_ATTR_SIMULATE_CURSOR, (SQLPOINTER)simulateCursor, 0);
    if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
    {
        printf("Error setting statement attribute for simulating cursor behavior\n");
    ...
    printf("set SQLSetStmtAttr(SQL_ATTR_SIMULATE_CURSOR) successfully\n");
