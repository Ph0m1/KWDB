...
int main()
{
    ...
    // Set statement attribute for maximum number of rows
    SQLULEN maxRows = 100; // Set maximum number of rows to 100
    ret = SQLSetStmtAttr(hstmt, SQL_ATTR_MAX_ROWS, (SQLPOINTER)maxRows, 0);
    if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
    {
        printf("Error setting statement attribute for maximum number of rows\n");
    ...
    printf("set SQLSetStmtAttr(SQL_ATTR_MAX_ROWS) successfully\n");
