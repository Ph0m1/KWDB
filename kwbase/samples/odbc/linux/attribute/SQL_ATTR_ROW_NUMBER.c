...
int main()
{
    ...
    // Set statement attribute for row number
    SQLULEN rowNumber = 1; // Set row number to 1
    ret = SQLSetStmtAttr(hstmt, SQL_ATTR_ROW_NUMBER, (SQLPOINTER)rowNumber, 0);
    if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
    {
        printf("Error setting statement attribute for row number\n");
    ...
    printf("set SQLSetStmtAttr(SQL_ATTR_ROW_NUMBER) successfully\n");

    SQLHSTMT hstmt1;
    ret = SQLGetStmtAttr(hstmt, SQL_ATTR_ROW_NUMBER, &hstmt1, SQL_IS_POINTER, NULL);
    if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
    {
        printf("Error getting statement attribute for row number\n");
        handleOdbcError(hstmt, SQL_HANDLE_STMT, ret);
        SQLFreeHandle(SQL_HANDLE_STMT, hstmt);
        SQLDisconnect(hdbc);
        SQLFreeHandle(SQL_HANDLE_DBC, hdbc);
