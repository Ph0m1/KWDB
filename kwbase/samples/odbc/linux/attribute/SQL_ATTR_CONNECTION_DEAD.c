...
int main()
{
    ...
    // For demonstration purposes, we are not actually disconnecting the connection here
    ...
    // Get current setting of SQL_ATTR_CONNECTION_DEAD
    ret = SQLGetConnectAttr(hdbc, SQL_ATTR_CONNECTION_DEAD, &connectionDead, 0, NULL);
    if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
    {
        handleOdbcError(hdbc, SQL_HANDLE_DBC, ret);
