...
int main()
{
    ...
    // Set SQL_ATTR_CONNECTION_TIMEOUT
    // ret = SQLSetConnectAttr(hdbc, SQL_ATTR_CONNECTION_TIMEOUT, (SQLPOINTER)connectionTimeout, 0);
    // if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO) {
    //     handleOdbcError(hdbc, SQL_HANDLE_DBC, ret);
    ...
    // Get current setting of SQL_ATTR_CONNECTION_TIMEOUT
    SQLULEN retrievedConnectionTimeout;
    ret = SQLGetConnectAttr(hdbc, SQL_ATTR_CONNECTION_TIMEOUT, &retrievedConnectionTimeout, 0, NULL);
    if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
    {
        handleOdbcError(hdbc, SQL_HANDLE_DBC, ret);
        SQLFreeHandle(SQL_HANDLE_DBC, hdbc);
    ...
    printf("Retrieved connection timeout: %lu seconds\n", retrievedConnectionTimeout);
