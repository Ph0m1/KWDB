...
int main()
{
    ...
    // Get SQL_ATTR_ROW_STATUS_PTR attribute to verify
    SQLPOINTER rowStatusPtr;
    ret = SQLGetStmtAttr(hstmt, SQL_ATTR_ROW_STATUS_PTR, &rowStatusPtr, 0, NULL);
    if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
    {
        // Error handling
        handle_error(hstmt, ret);
    ...
    if (rowStatusPtr == &rowStatus)
    {
        printf("SQL_ATTR_ROW_STATUS_PTR is set correctly.\n");
    }
    else
    {
        printf("SQL_ATTR_ROW_STATUS_PTR is not set correctly.\n");
    }
