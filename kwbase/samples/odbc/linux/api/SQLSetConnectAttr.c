...
int main()
{
    ...
    SQLHDBC hdbc;
    SQLRETURN ret;
    SQLCHAR currentCatalog[256] = "d1"; // Set the current catalog to "my_database"
    ...
    ret = SQLSetConnectAttr(hdbc, SQL_ATTR_CURRENT_CATALOG, currentCatalog, SQL_NTS);
    if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
    {
        handleOdbcError(hdbc, SQL_HANDLE_DBC, ret);
    ...
    ret = SQLGetConnectAttr(hdbc, SQL_ATTR_CURRENT_CATALOG, retrievedCatalog, sizeof(retrievedCatalog), NULL);
    if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
    {
        handleOdbcError(hdbc, SQL_HANDLE_DBC, ret);
        SQLFreeHandle(SQL_HANDLE_DBC, hdbc);
    ...
    printf("Retrieved current catalog: %s\n", retrievedCatalog);
