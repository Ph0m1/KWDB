...
int main()
{
    ...
    for (int i = 1; i <= colCount; ++i)
    {
        ...
        // Get SQL_DESC_LABEL attribute
        ret = SQLColAttribute(hstmt, i, SQL_DESC_LABEL, label, sizeof(label), &labelLen, NULL);
        if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
        {
            printf("Error getting SQL_DESC_LABEL attribute for column %s\n", colName);
            continue;
        }

        printf("Column: %s, Label: %s\n", colName, label);
    }
