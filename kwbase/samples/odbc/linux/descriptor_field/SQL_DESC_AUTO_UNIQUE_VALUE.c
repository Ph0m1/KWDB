...
int main()
{
    ...
    // Set the SQL_DESC_AUTO_UNIQUE_VALUE field for the first column in the result set
    SQLLEN caseSensitive;
    // Get the case sensitivity attribute for the column
    retcode = SQLColAttribute(hstmt, 1, SQL_DESC_AUTO_UNIQUE_VALUE, NULL, 0, NULL, &caseSensitive);
    if (retcode == SQL_SUCCESS)
    {
        printf("set SQLColAttribute(SQL_DESC_AUTO_UNIQUE_VALUE) successfully\n");
    }
    else
    {
        printf("set SQLColAttribute(SQL_DESC_AUTO_UNIQUE_VALUE) failed\n");
    }
