...
int main()
{
    ...
    ret = SQLColAttribute(hstmt, 1, SQL_DESC_SCHEMA_NAME, schemaName, sizeof(schemaName), &schemaNameLen, NULL);
    if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
    {
        printf("Error getting SQL_DESC_SCHEMA_NAME attribute\n");
    }
    else
    {
        printf("SQL_DESC_SCHEMA_NAME value: %s\n", schemaName);
    }
