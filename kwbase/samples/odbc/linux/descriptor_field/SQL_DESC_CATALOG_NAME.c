...
int main()
{
    ...
    if (ret == SQL_SUCCESS)
    {
        printf("Catalog Name of the column: %s\n", colCatalogName);
    }
    else
    {
        printf("Failed to get catalog name for column \n");
    }
    ...
    //     // Get SQL_DESC_CATALOG_NAME attribute
    //     ret = SQLColAttribute(hstmt, i, SQL_DESC_CATALOG_NAME, catalogName, sizeof(catalogName), &catalogNameLen, NULL);
    //     if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO) {
    //         printf("Error getting SQL_DESC_CATALOG_NAME attribute for column %s\n", colName);
    //         continue;
    //     }

    //     printf("Column: %s, Catalog Name: %s\n", colName, catalogName);
    // }
