...
int main()
{
    ...
    // Set the SQL_DESC_INDICATOR_PTR field for the first column in the result set
    SQLPOINTER dataPtr = NULL; // Replace someDataPtr with the actual data pointer
    retcode = SQLSetDescField(hdesc, 1, SQL_DESC_INDICATOR_PTR, dataPtr, SQL_IS_POINTER);
    if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
    {
        printf("set the SQL_DESC_INDICATOR_PTR field for column failed\n");
        show_error(hstmt, SQL_HANDLE_STMT);
        SQLFreeHandle(SQL_HANDLE_STMT, hstmt);
        SQLDisconnect(hdbc);
        SQLFreeHandle(SQL_HANDLE_DBC, hdbc);
    ...
    {
        printf("set the SQL_DESC_INDICATOR_PTR field for column successfully\n");
    }
