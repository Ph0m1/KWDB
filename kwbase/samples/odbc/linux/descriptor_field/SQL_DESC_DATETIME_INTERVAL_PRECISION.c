...
int main()
{
    ...
    // Set the SQL_DESC_DATETIME_INTERVAL_PRECISION field for the first column in the result set
    SQLPOINTER dataPtr = NULL; // Replace someDataPtr with the actual data pointer
    retcode = SQLSetDescField(hdesc, 1, SQL_DESC_DATETIME_INTERVAL_PRECISION, dataPtr, SQL_IS_POINTER);
    if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
    {
