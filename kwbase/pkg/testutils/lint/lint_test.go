...
const kwbaseDB = "gitee.com/kwbasedb/kwbase"
...
func TestLint(t *testing.T) {
	kwdb, err := build.Import(kwbaseDB, "", build.FindOnly)
	if err != nil {
		t.Skip(err)
	}
	pkgDir := filepath.Join(kwdb.Dir, "pkg")
	...
	t.Run("TestLowercaseFunctionNames", func(t *testing.T) {
		...
		cmd, stderr, filter, err := dirCmd(kwdb.Dir,
	...
	t.Run("TestEnvutil", func(t *testing.T) {
		...
		for _, tc := range []struct {
			...
			{re: `\bos\.(Getenv|LookupEnv)\("KWBASE`},
			{
	...
	t.Run("TestImportNames", func(t *testing.T) {
		...
		if err := stream.ForEach(stream.Sequence(
			...
			t.Errorf("\n%s <- forbidden; import 'database/sql' as 'gosql' to avoid confusion with 'kwbase/sql'", s)
		}); err != nil {
			t.Error(err)
		}
	...
	t.Run("TestForbiddenImports", func(t *testing.T) {
		...
		forbiddenImports := map[string]string{
			...
			"golang.org/x/sync/singleflight":              "gitee.com/kwbasedb/kwbase/pkg/util/syncutil/singleflight",
			"syscall":                                     "sysutil",
			"github.com/cockroachdb/errors/assert":        "github.com/kwbasedb/errors",
			"github.com/cockroachdb/errors/barriers":      "github.com/kwbasedb/errors",
			"github.com/cockroachdb/errors/contexttags":   "github.com/kwbasedb/errors",
			"github.com/cockroachdb/errors/domains":       "github.com/kwbasedb/errors",
			"github.com/cockroachdb/errors/errbase":       "github.com/kwbasedb/errors",
			"github.com/cockroachdb/errors/errutil":       "github.com/kwbasedb/errors",
			"github.com/cockroachdb/errors/issuelink":     "github.com/kwbasedb/errors",
			"github.com/cockroachdb/errors/markers":       "github.com/kwbasedb/errors",
			"github.com/cockroachdb/errors/report":        "github.com/kwbasedb/errors",
			"github.com/cockroachdb/errors/safedetails":   "github.com/kwbasedb/errors",
			"github.com/cockroachdb/errors/secondary":     "github.com/kwbasedb/errors",
			"github.com/cockroachdb/errors/telemetrykeys": "github.com/kwbasedb/errors",
			"github.com/cockroachdb/errors/withstack":     "github.com/kwbasedb/errors",
		}
		...
		filter := stream.FilterFunc(func(arg stream.Arg) error {
			...
			outer:
				for path := range buildutil.ExpandPatterns(&buildContext, []string{filepath.Join(kwbaseDB, pkgScope)}) {
					importPkg, err := buildContext.Import(path, kwdb.Dir, 0)
					switch err.(type) {
					case nil:
		...
		settingsPkgPrefix := `gitee.com/kwbasedb/kwbase/pkg/settings`
		if err := stream.ForEach(stream.Sequence(
			filter,
			stream.Sort(),
			stream.Uniq(),
			stream.Grep(`^`+settingsPkgPrefix+`: | `+grepBuf.String()),
			stream.GrepNot(`kwbase/pkg/cmd/`),
			stream.GrepNot(`kwbase/pkg/testutils/lint: log$`),
			stream.GrepNot(`kwbase/pkg/util/sysutil: syscall$`),
			stream.GrepNot(`kwbase/pkg/(base|security|util/(log|randutil|stop)): log$`),
			stream.GrepNot(`kwbase/pkg/(server/serverpb|ts/tspb): github\.com/golang/protobuf/proto$`),

			stream.GrepNot(`kwbase/pkg/util/uuid: github\.com/satori/go\.uuid$`),
		), func(s string) {
			...
			// Test that the settings package does not import KWDB dependencies.
			if importingPkg == settingsPkgPrefix && strings.HasPrefix(importedPkg, kwbaseDB) {
				switch {
				...
				default:
					t.Errorf("%s <- please don't add KWDB dependencies to settings pkg", s)
				}
			}
		}); err != nil {
			t.Error(err)
	...
	t.Run("TestReturnCheck", func(t *testing.T) {
		...
		cmd, stderr, filter, err := dirCmd(kwdb.Dir, "returncheck", pkgScope)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("TestGolint", func(t *testing.T) {
		t.Parallel()
		cmd, stderr, filter, err := dirCmd(kwdb.Dir, "golint", pkgScope)
		if err != nil {
			t.Fatal(err)
		}
	...
	// 			// ignore go generate file
	// 			// stream.grepnot(`pkg/col/coltypes/*.go`),
	// 			stream.grepnot(`pkg/cli/zip.go`),
	// 			stream.grepnot(`pkg/cli/zip_test.go`),
	// 		), func(s string) {
	// 			t.errorf("\n%s", s)
	// 		}); err != nil {
	// 		t.error(err)
	...
	// TODO(mike): Not enough resources to run, skip temporarily
