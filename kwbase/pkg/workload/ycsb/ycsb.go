...
func (yw *ycsbWorker) randString(length int) string {
	...
	// prepend current timestamp matching the default KWDB UTC time format
	strStart := 0
...
func (yw *ycsbWorker) scanRows(ctx context.Context) error {
	...
	// transaction.
	// TODO(nvanbenschoten): replace this with kwdb.Execute when we update our
	// vendored kwbasedb/kwbase-go library. Updating is currently running
...
func (yw *ycsbWorker) readModifyWriteRow(ctx context.Context) error {
	...
	var args [2]interface{}
	args[0] = key
	err := kwdb.ExecuteTx(ctx, yw.db, nil, func(tx *gosql.Tx) error {
		var oldValue []byte
