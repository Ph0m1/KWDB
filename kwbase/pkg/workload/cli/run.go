...
func CmdHelper(
	...
	const kwdbDefaultURL = `postgres://root@localhost:26257?sslmode=disable`
...
// read / write blocks of random data into a table in kwbase DB. The function
...
func runInit(gen workload.Generator, urls []string, dbName string) error {
	...
	initDB, err := gosql.Open(`kwbase`, strings.Join(urls, ` `))
	if err != nil {
		return err
	}
...
func startPProfEndPoint(ctx context.Context) {
	b := envutil.EnvOrDefaultInt64("KWBASE_BLOCK_PROFILE_RATE",
		10000000 /* 1 sample per 10 milliseconds spent blocking */)

	m := envutil.EnvOrDefaultInt("KWBASE_MUTEX_PROFILE_RATE",
...
func runRun(gen workload.Generator, urls []string, dbName string) error {
	...
	log.Infof(ctx, "creating load generator... done (from %v, took %s)", prepareStart, timeutil.Now().Sub(prepareStart))
