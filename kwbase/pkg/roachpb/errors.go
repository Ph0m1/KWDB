...
func (e *Error) MaybeUpdateWriteKeyCount(o *Transaction) {
	if o == nil || e.UnexposedTxn == nil {
		return
	}
	e.UnexposedTxn.BatchWriteKeyCount += o.BatchWriteKeyCount
}
...
func (e *LeaseHolderExpiredError) Error() string {
	return e.message(nil)
}

func (e *LeaseHolderExpiredError) message(_ *Error) string {
	const prefix = "[LeaseHolderExpiredError] "
	if e.CustomMsg != "" {
		return prefix + e.CustomMsg
	}
	var buf strings.Builder
	buf.WriteString(prefix)
	fmt.Fprintf(&buf, "r%d: ", e.RangeID)
	fmt.Fprintf(&buf, "lease status is %s: ", e.LeaseState)
	if e.Replica != (ReplicaDescriptor{}) {
		fmt.Fprintf(&buf, "current replica is %s", e.Replica)
	}
	if e.LeaseHolder == nil {
		fmt.Fprint(&buf, "last lease holder unknown")
	} else if e.Lease != nil {
		fmt.Fprintf(&buf, "last lease holder is %s", e.Lease)
	}
	return buf.String()
}

var _ ErrorDetailInterface = &LeaseHolderExpiredError{}
...
func (e *DefaultReplicaReceiveTSRequestError) Error() string {
	return e.message(nil)
}

func (e *DefaultReplicaReceiveTSRequestError) message(pErr *Error) string {
	return fmt.Sprintf("DefaultReplicaReceiveTSRequestError: %s", e.ExtraMsg)
}

var _ ErrorDetailInterface = &DefaultReplicaReceiveTSRequestError{}
