...
// NodeID is a custom type for a kwbase node ID. (not a raft node ID)
...
// StoreID is a custom type for a kwbase store ID.
type StoreID int32
...
func (r *RangeDescriptor) SetRangeType(rangeType RangeType) {
	r.RangeType = &rangeType
}

// GetRangeType get range type
func (r RangeDescriptor) GetRangeType() RangeType {
	if r.RangeType == nil {
...
func (sc StoreCapacity) String() string {
	return fmt.Sprintf("disk (capacity=%s, available=%s, totalUsed=%s, "+
		"relationalUsed=%s, tsdbUsed=%s, logicalBytes=%s), "+
		...
		humanizeutil.IBytes(sc.Used), humanizeutil.IBytes(sc.RelationalUsed),
		humanizeutil.IBytes(sc.TsdbUsed), humanizeutil.IBytes(sc.LogicalBytes),
...
func (sc StoreCapacity) FractionUsed() float64 {
	...
	// capacity is really just the available space plus kwbase's usage.
	//
