...
const (
	...
	networkPrefix = "kwbasedb_acceptance"
)
...
// binary.
const CockroachBinaryInContainer = "/kwbase/kwbase"

var kwbaseImage = flag.String("i", defaultImage, "the docker image to run")
var kwbaseEntry = flag.String("e", "", "the entry point for the image")
...
var CockroachBinary = flag.String("b", func() string {
	rootPkg, err := build.Import("gitee.com/kwbasedb/kwbase", "", build.FindOnly)
	if err != nil {
		panic(err)
	}
	// NB: This is the binary produced by our linux-gnu build target. Changes
	// to the Makefile must be reflected here.
	return filepath.Join(rootPkg.Dir, "kwbase-linux-2.6.32-gnu-amd64")
...
// DockerCluster manages a local kwbase cluster running on docker. The
// cluster is composed of a "volumes" container which manages the
// persistent volumes used for certs and node data and N kwbase nodes.
...
// CreateDocker creates a Docker-based kwbase cluster. The stopper is used to
...
// specified volumesDir, including logs and kwbase stores. If volumesDir is
...
func CreateDocker(
	...
	if *kwbaseImage == defaultImage && !exists(*CockroachBinary) {
	...
	if volumesDir == "" {
		volumesDir, err = ioutil.TempDir("", fmt.Sprintf("kwbase-acceptance-%s", clusterIDS))
		maybePanic(err)
	} else {
...
func (l *DockerCluster) initCluster(ctx context.Context) {
	...
	binds := []string{
		...
		filepath.Join(pwd, "..") + ":/go/src/gitee.com/kwbasedb/kwbase",
		filepath.Join(l.volumesDir, "logs") + ":/logs",
	}

	if *kwbaseImage == defaultImage {
...
// kwbaseEntryPoint returns the value to be used as
// container.Config.Entrypoint for a container running the kwbase
// binary under test.
// TODO(bdarnell): refactor this to minimize globals
func kwbaseEntrypoint() []string {
	var entrypoint []string
	if *kwbaseImage == defaultImage {
		entrypoint = append(entrypoint, CockroachBinaryInContainer)
	} else if *kwbaseEntry != "" {
		entrypoint = append(entrypoint, *kwbaseEntry)
	}
	return entrypoint
}
...
func (l *DockerCluster) startNode(ctx context.Context, node *testNode) {
	...
	env := []string{
		"KWBASE_SCAN_MAX_IDLE_TIME=200ms",
  ...
  logs:      %[3]s/kwbase.INFO
  pprof:     docker exec -it %[4]s pprof https+insecure://$(hostname):%[5]s/debug/pprof/heap
  kwbase: %[6]s

  cli-env:   KWBASE_INSECURE=false KWBASE_CERTS_DIR=%[7]s KWBASE_HOST=%s:%d`,
...
// RunInitCommand runs the `kwbase init` command. Normally called
...
// ExecCLI runs ./kwbase <args> with sane defaults.
