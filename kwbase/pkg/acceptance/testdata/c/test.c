...
int main(int argc, char *argv[]) {
	...
	const char *version = PQparameterStatus(conn, "kwdb_version");
	if (version == NULL) {
		fprintf(stderr, "ERROR PQparameterStatus: kwdb_version not reported: %s\n", PQgeterror());
		return 1;
	}
	if (strncmp(version, "CockroachDB ", strlen("CockroachDB ")) != 0) {
		fprintf(stderr, "kwdb_version mismatch: '%s' doesn't start with 'CockroachDB '\n", version);
		return 1;
	}
