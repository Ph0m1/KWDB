...
func repoRoot() string {
	root, err := build.Import("gitee.com/kwbasedb/kwbase", "", build.FindOnly)
	if err != nil {
		panic(fmt.Sprintf("must run from within the kwbase repository: %s", err))
	}
	return root.Dir
}

// SourceBinary returns the path of the kwbase binary that was built with the
// local source.
func SourceBinary() string {
	return filepath.Join(repoRoot(), "kwbase")
}

const listeningURLFile = "kwbasedb-url"
...
type ClusterConfig struct {
	...
	Binary      string             // path to kwbase, defaults go <kwbase_repo>/kwbase
	AllNodeArgs []string           // args to pass to ./kwbase on all nodes
...
type NodeConfig struct {
	...
	ExtraArgs         []string // extra arguments for ./kwbase start
...
func (c *Cluster) isReplicated() (bool, string) {
	db := c.Nodes[0].DB()
	rows, err := db.Query(`SELECT range_id, start_key, end_key, array_length(replicas, 1) FROM kwdb_internal.ranges`)
	if err != nil {
...
func (n *Node) startAsyncInnerLocked(ctx context.Context, joins ...string) error {
	...
	n.cmd.Env = append(n.cmd.Env, "KWBASE_SCAN_MAX_IDLE_TIME=5ms") // speed up rebalancing
...
func (n *Node) advertiseAddrFile() string {
	return filepath.Join(n.Cfg.DataDir, "kwbase.advertise-addr")
}

func (n *Node) httpAddrFile() string {
	return filepath.Join(n.Cfg.DataDir, "kwbase.http-addr")
}
	...
	for r := retry.Start(opts); r.Next(); {
...

		{
			...
			if err := n.db.QueryRow(
				`SELECT value FROM kwdb_internal.node_runtime_info WHERE component='UI' AND field = 'URL'`,
			).Scan(&uiStr); err != nil {
				log.Info(ctx, err)
				return nil
			}
