...
type SpanStats interface {
	...
	// TsStats key is processorID, value is the stats that the object represents as a map
...
type span struct {
	...
	mu struct {
		...
		// tsTags for explain analyse
		tsTags map[int32]opentracing.Tags
...
// set tag of time series for explain analyse
func (s *span) setTsTags() {
	if s.mu.tsTags == nil {
		s.mu.tsTags = make(map[int32]opentracing.Tags)
	}
	for processid, stats := range s.mu.stats.TsStats() {
		tsTag := make(opentracing.Tags)
		for key, value := range stats {
			tsTag[key] = value
		}
		s.mu.tsTags[processid] = tsTag
	}
}
...
func (s *span) getRecording() RecordedSpan {
	...
	addTsTag := func(id int32, tags opentracing.Tags) {
		if rs.TsTags == nil {
			rs.TsTags = make(map[int32]*MapTag)
		}
		mapTag := make(map[string]string)
		for k, v := range tags {
			mapTag[k] = fmt.Sprint(v)
		}
		rs.TsTags[id] = &MapTag{mapTag}
	}
	...
	if len(s.mu.tsTags) > 0 {
		for k, v := range s.mu.tsTags {
			// We encode the ts_tag values as strings.
			addTsTag(k, v)
		}
	}
