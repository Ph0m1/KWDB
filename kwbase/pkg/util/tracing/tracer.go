...
const (
	prefixTracerState = "kwdb-tracer-"
	prefixBaggage     = "kwdb-baggage-"
	// prefixShadow is prepended to the keys for the context of the shadow tracer
	// (e.g. LightStep).
	prefixShadow = "kwdb-shadow-"
...
var lightstepToken = settings.RegisterPublicStringSetting(
	...
	envutil.EnvOrDefaultString("KWBASE_TEST_LIGHTSTEP_TOKEN", ""),
)
...
var zipkinCollector = settings.RegisterPublicStringSetting(
	...
	envutil.EnvOrDefaultString("KWBASE_TEST_ZIPKIN_COLLECTOR", ""),
)
...
// seed is used to enhance randomness, it can be the processorID or nodeID.
func StartChildSpan(
	opName string,
	...
	// getRanddom randomly obtain a character from opName,
	// convert it to an int value, then add the length of opName,
	// the result as a random number to enhance randomness.
	getRandom := func() int64 {
		rand.Seed(time.Now().UnixNano())
		if len(opName) > 0 {
			randomIndex := rand.Intn(len(opName))
			randomChar := opName[randomIndex]
			randomNum := rand.Intn(int(randomChar))
			return int64(randomNum + len(opName))
		}
		return 1
	}
	...
	// Create random numbers, add seed to enhance randomness,3600000000000 is 1 hour.
	rand.Seed(time.Now().UnixNano() + int64(seed)*3600000000000*randomNum)
	s.SpanID = uint64(rand.Int63())
...
// closed via FinishSpan.
// seed is used to enhance randomness, it can be the processorID or nodeID.
func ChildSpan(ctx context.Context, opName string, seed int32) (context.Context, opentracing.Span) {
	return childSpan(ctx, opName, false /* separateRecording */, seed)
}
...
func ChildSpanSeparateRecording(
	ctx context.Context, opName string, seed int32,
) (context.Context, opentracing.Span) {
	return childSpan(ctx, opName, true /* separateRecording */, seed)
}

// seed is used to enhance randomness, it can be the processorID or nodeID.
func childSpan(
	ctx context.Context, opName string, separateRecording bool, seed int32,
) (context.Context, opentracing.Span) {
	...
	newSpan := StartChildSpan(opName, span, logtags.FromContext(ctx), separateRecording, seed)
	return opentracing.ContextWithSpan(ctx, newSpan), newSpan
}
