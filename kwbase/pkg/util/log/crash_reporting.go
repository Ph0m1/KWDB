...
var (
	...
	// env var KWBASE_SKIP_ENABLING_DIAGNOSTIC_REPORTING.
	//
	...
	DiagnosticsReportingEnabled = settings.RegisterPublicBoolSetting(
		...
		"enable reporting diagnostic metrics to kwbase labs",
		false,
	)
	...
	ReportSensitiveDetails = envutil.EnvOrDefaultBool("KWBASE_REPORT_SENSITIVE_DETAILS", false)
)
...
var crashReportURL = func() string {
	...
	if build.SeemsOfficial() {
		defaultURL = "https://ignored:ignored@errors.kwbasedb.com/sentry"
	}
	return envutil.EnvOrDefaultString("KWBASE_CRASH_REPORTS", defaultURL)
}()
