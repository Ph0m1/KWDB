...
func TestRouterDiskSpill(t *testing.T) {
	...
	for i := 0; ; i++ {
		...
		if meta != nil {
			...
			rowsRouted := getIntTagValue("kwbase.stat.routeroutput.rows_routed")
			memMax := getIntTagValue("kwbase.stat.routeroutput.mem.max")
			diskMax := getIntTagValue("kwbase.stat.routeroutput.disk.max")
