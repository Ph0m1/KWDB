...
func makeOrderedSync(
	...
	base := &serialUnorderedSynchronizer{
		state:   notInitialized,
		sources: make([]srcInfo, len(sources)),
		types:   sources[0].OutputTypes(),
	}
...
type serialUnorderedSynchronizer struct {
	...
	resChan chan execinfra.RowChannelMsg
	isClose bool
	nums    int32
}
...
func (s *serialUnorderedSynchronizer) startReceiveFromSources(
	source execinfra.RowSource, msgs chan execinfra.RowChannelMsg,
) {
	for {
		row, meta := source.Next()
		if meta != nil {
			msgs <- execinfra.RowChannelMsg{Meta: meta}
			break
		}
		if row == nil {
			break
		}
		msgs <- execinfra.RowChannelMsg{Row: row, Meta: meta}
	}
	i := atomic.AddInt32(&s.nums, -1)
	if i == 0 {
		close(s.resChan)
	}
}

func (s *serialUnorderedSynchronizer) Next() (sqlbase.EncDatumRow, *execinfrapb.ProducerMetadata) {
	for s.srcInx < len(s.sources) {
		row, metadata := s.sources[s.srcInx].src.Next()

		// If we're draining only return metadata.
		if s.state == draining && row != nil {
			continue
		}
...
func (s *serialUnorderedSynchronizer) ConsumerDone() {
	if s.state != draining {
		for i := range s.sources {
...
func (s *serialUnorderedSynchronizer) ConsumerClosed() {
	for i := range s.sources {
