	...
	for i := range rb.outputs {
		go func(ctx context.Context, rb *routerBase, ro *routerOutput, wg *sync.WaitGroup) {
			...
			if rb.statsCollectionEnabled {
				ctx, span = execinfra.ProcessorSpan(ctx, "router output", int32(rb.outputs[i].streamID))
...
func (mr *mirrorRouter) GetCols() int {
	//TODO unimplemented
	panic("unimplemented")
}
...
func (mr *mirrorRouter) PushPGResult(ctx context.Context, res []byte) error {
	return nil
}
...
func (ros *RouterOutputStats) TsStats() map[int32]map[string]string {
	return nil
}
...
func (ros *RouterOutputStats) TsStatsForQueryPlan() map[int32][]string {
	return nil
}
