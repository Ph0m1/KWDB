...
func DecodeTableValue(a *DatumAlloc, valType *types.T, b []byte) (tree.Datum, []byte, error) {
	...
	return decodeUntaggedDatumWithType(a, valType, b, typ)
}

// decodeUntaggedDatumWithType is used to decode a Datum whose type is known,
...
// the tag directly.
func decodeUntaggedDatumWithType(
	a *DatumAlloc, t *types.T, buf []byte, typ encoding.Type,
) (tree.Datum, []byte, error) {
	switch t.Family() {
	case types.IntFamily:
	...
	case types.DecimalFamily:
		if typ == encoding.Int {
			...
			val := a.NewDDecimal(tree.DDecimal{})
			val.SetInt64(i)
			return val, b, nil
			...
			val := a.NewDDecimal(tree.DDecimal{})
			err = val.SetString(strconv.FormatFloat(data, 'f', -1, 64))
			if err != nil {
				return nil, b, err
			}
			return val, b, nil
