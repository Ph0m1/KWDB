...
func TestTransitions(t *testing.T) {
	...
	tests := []test{
		...
		{
			// The txn is starting again (ROLLBACK TO SAVEPOINT <not kwbase_restart> while in Aborted).
			name: "Aborted->Open",
			init: func() (fsm.State, *txnState, error) {
				s, ts := testCon.createAbortedState()
				return s, ts, nil
		...
		{
			// The txn is starting again (ROLLBACK TO SAVEPOINT kwbase_restart while in Aborted).
