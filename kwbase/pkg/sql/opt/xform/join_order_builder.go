...
type JoinOrderBuilder struct {
	...
	// needReconstructAllJoins is true when filter was not pushed down as far as possible,
	// all joins apart from the root will have to be reconstructed.
	needReconstructAllJoins bool

	// relatedEdges records the edges corresponding to the vertexSet, used add join
	relatedEdges map[vertexSet]edgeSet

	// fds is used to keep track of equivalence relations when assembling filters.
...
func (jb *JoinOrderBuilder) Init(c *CustomFuncs) {
	...
	*jb = JoinOrderBuilder{
		...
		relatedEdges:  make(map[vertexSet]edgeSet),
	}
}
...
// In this case, should delete all joins from plans of JoinOrderBuilder and then
// reconstruct all joins later.
func (jb *JoinOrderBuilder) ensureClosure(join memo.RelExpr) {
	...
	for col, ok := reps.Next(0); ok; col, ok = reps.Next(col + 1) {
		...
		if checkValid(equivGroup, join.Relational().NotNullCols) {
			continue
		}
...
func (jb *JoinOrderBuilder) validateEdges() {
	for i := range jb.edges {
		if jb.needReconstructAllJoins {
			break
		}
		e := &jb.edges[i]
		if e.op.joinType == opt.InnerJoinOp {
			jb.needReconstructAllJoins = !e.checkInnerJoin(e.op.leftVertexes, e.op.rightVertexes)
		} else {
			jb.needReconstructAllJoins = !e.checkNonInnerJoin(e.op.leftVertexes, e.op.rightVertexes)
		}
	}
	if jb.needReconstructAllJoins {
...
// checkValid check if the join need ensure the implicit edge.
func checkValid(equivGroup, notNullCols opt.ColSet) bool {
	if !equivGroup.Empty() && !equivGroup.Intersects(notNullCols) {
...
// initRelatedEdges initializes applicableEdges with all edges that must show
// up in any join tree that is constructed for the given set of vertexes. See
// checkRelatedEdges for how this information is used.
func (jb *JoinOrderBuilder) initRelatedEdges(s vertexSet) {
	for i := range jb.edges {
		if jb.edges[i].tes.isSubsetOf(s) {
			if relatedEdges, ok := jb.relatedEdges[s]; ok {
				relatedEdges.Add(i)
				jb.relatedEdges[s] = relatedEdges
			} else {
				relatedEdges := edgeSet{}
				relatedEdges.Add(i)
				jb.relatedEdges[s] = relatedEdges
			}
		}
...
func (jb *JoinOrderBuilder) addJoins(s1, s2 vertexSet) {
	...
	var relatedEdges edgeSet

	jb.fds.ResetDeps()
	jb.fds.AddEquivFrom(&jb.plans[s1].Relational().FuncDeps)
	jb.fds.AddEquivFrom(&jb.plans[s2].Relational().FuncDeps)
	...
	for i, ok := jb.innerEdges.Next(0); ok; i, ok = jb.innerEdges.Next(i + 1) {
		...
		if e.checkInnerJoin(s1, s2) {
			...
			relatedEdges.Add(i)
			if areFiltersRedundant(&jb.fds, e.filters) {
			...
			if !joinIsRedundant {
				...
				joinIsRedundant = jb.joinIsRedundant(s1, s2, relatedEdges)
			}
			getEquivFDs(&jb.fds, e.filters)
	...
	for i, ok := jb.nonInnerEdges.Next(0); ok; i, ok = jb.nonInnerEdges.Next(i + 1) {
		...
		if e.checkNonInnerJoin(s1, s2) {
			...
			// inner join filters.
			if jb.checkRelatedEdges(s1, s2, relatedEdges) {
				jb.addJoin(e.op.joinType, s1, s2, e.filters, innerJoinFilters, jb.joinIsRedundant(s1, s2, relatedEdges))
			}
		...
		if e.checkNonInnerJoin(s2, s1) {
			...
			// right.
			if jb.checkRelatedEdges(s2, s1, relatedEdges) {
				jb.addJoin(e.op.joinType, s2, s1, e.filters, innerJoinFilters, jb.joinIsRedundant(s2, s1, relatedEdges))
			}
			return
		}
	}
...
// checkRelatedEdges checks that each join plan includes every edge for which
...
// longer true for the `JoinOrderBuilder`. checkRelatedEdges corrects for this
// by explicitly checking that all applicable edges have been applied when a
// join plan is considered.
func (jb *JoinOrderBuilder) checkRelatedEdges(s1, s2 vertexSet, appliedEdges edgeSet) bool {
	leftEdges, rightEdges := jb.relatedEdges[s1], jb.relatedEdges[s2]
	allRelatedEdges := appliedEdges.Union(leftEdges).Union(rightEdges)
	return allRelatedEdges.Equals(jb.relatedEdges[s1.union(s2)])
}
...
func (jb *JoinOrderBuilder) makeTransitiveEdge(col1, col2 opt.ColumnID) {
	...
	// check filter and set flag.
	jb.checkNeedReconstructJoins(op, col1, col2)
...
// checkNeedReconstructJoins check if the filter was not pushed down as far as possible,
// and set the needReconstructAllJoins of JoinOrderBuilder.
func (jb *JoinOrderBuilder) checkNeedReconstructJoins(op *operator, c1, c2 opt.ColumnID) {
	vertexSet := op.leftVertexes.union(op.rightVertexes)
	originalJoin, ok := jb.plans[vertexSet]
	if !ok {
		panic(errors.AssertionFailedf("failed to find expected join plan"))
	}
	if !originalJoin.Relational().FuncDeps.AreColsEquiv(c1, c2) {
		...
		jb.needReconstructAllJoins = true
	}
}
