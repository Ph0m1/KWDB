...
func TestDetachMemo(t *testing.T) {
	...
	catalog := testcat.New()
	if _, err := catalog.ExecuteDDL("CREATE TABLE abc (a INT8 PRIMARY KEY, b INT, c STRING, INDEX (c))"); err != nil {
		t.Fatal(err)
	}
...
func TestDetachMemoRace(t *testing.T) {
	...
	_, err := catalog.ExecuteDDL("CREATE TABLE abc (a INT8, b INT8, c INT8, d INT8)")
	if err != nil {
		t.Fatal(err)
	}
	var o xform.Optimizer
