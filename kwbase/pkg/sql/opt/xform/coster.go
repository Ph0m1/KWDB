...
func (c *coster) ComputeCost(candidate memo.RelExpr, required *physical.Required) memo.Cost {
	...
	case opt.TSScanOp:
		cost = c.computeTsScanCost(candidate.(*memo.TSScanExpr))
...
func (c *coster) computeScanCost(scan *memo.ScanExpr, required *physical.Required) memo.Cost {
	...
	if scan.Flags.HintType != keys.NoScanHint {
		if (scan.Flags.HintType == keys.ForceTableScan || scan.Flags.HintType == keys.UseTableScan) &&
			scan.Index != cat.PrimaryIndex {
			return hugeCost
		}
		if (scan.Flags.HintType == keys.ForceIndexScan || scan.Flags.HintType == keys.ForceIndexOnly) &&
			scan.Flags.HintIndexes[0] != scan.Index {
			return hugeCost
		}
		if scan.Flags.HintType == keys.UseIndexScan || scan.Flags.HintType == keys.UseIndexOnly {
			find := false
			for _, index := range scan.Flags.HintIndexes {
				if scan.Index == index {
					find = true
					break
				}
		...
		outputCols := scan.ScanPrivate.Cols
		indexCols := c.mem.Metadata().TableMeta(scan.ScanPrivate.Table).IndexColumns(scan.Index)
		noCover := !(outputCols.SubsetOf(indexCols))
		if (scan.Flags.HintType == keys.ForceIndexOnly || scan.Flags.HintType == keys.UseIndexOnly) && noCover {
			return hugeCost
		}
		if scan.Flags.HintType == keys.IgnoreTableScan && scan.Index == cat.PrimaryIndex {
			return hugeCost
		}
		if scan.Flags.HintType == keys.IgnoreIndexScan || scan.Flags.HintType == keys.IgnoreIndexOnly {
			for _, index := range scan.Flags.HintIndexes {
				if scan.Index == index {
					return hugeCost
				}
...
// computeTsScanCost return the cost of tsscan
func (c *coster) computeTsScanCost(tsScan *memo.TSScanExpr) memo.Cost {
	...
	// Obtain the pTag ID and then obtain pTag statistics information through the pTag ID.
	var pTagColIDs opt.ColSet
	for i := 0; i < table.ColumnCount(); i++ {
		col := table.Column(i)
		if col.IsPrimaryTagCol() {
			pTagColIDs.Add(opt.ColumnID(col.ColID()))
		}
	}
...
func (c *coster) computeGroupingCost(grouping memo.RelExpr, required *physical.Required) memo.Cost {
	...
	groupbyInput, ok := grouping.(*memo.GroupByExpr)
	...
	if groupingColCount > 0 {
		...
		var isGapFill bool
		if ok {
			for i := 0; i < len(groupbyInput.Aggregations); i++ {
				switch groupbyInput.Aggregations[i].Agg.(type) {
				case *memo.ImputationExpr:
					isGapFill = true
				case *memo.TimeBucketGapfillExpr:
					isGapFill = true
				}
			}
		}
