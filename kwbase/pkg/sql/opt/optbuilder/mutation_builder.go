...
func (mb *mutationBuilder) addSynthesizedCols(
	...
	if projectionsScope != nil {
		...
		// time series table need add projection to handle order by for time series insert select.
		// This logic will not be executed in ts insert, and only ts insert select.
		// in ts insert select need add project to pass type check, otherwise return error
...
// decimals) with a call to the kwdb_internal.round_decimal_values function, if
...
func (mb *mutationBuilder) roundDecimalValues(scopeOrds []scopeOrdinal, roundComputedCols bool) {
	...
	for i, ord := range scopeOrds {
		...
		private := &memo.FunctionPrivate{
			Name:       "kwdb_internal.round_decimal_values",
		...
		mb.b.populateSynthesizedColumn(&projectionsScope.cols[ord], fn, ScopeTSPropDefault)
	}
...
//
// NOTE: KWDB does not allow nested array storage types, so only one level of
...
func findRoundingFunction(typ *types.T, precision int) (*tree.FunctionProperties, *tree.Overload) {
	...
	props, overloads := builtins.GetBuiltinProperties("kwdb_internal.round_decimal_values")
