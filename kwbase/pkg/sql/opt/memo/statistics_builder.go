...
func (sb *statisticsBuilder) colStat(colSet opt.ColSet, e RelExpr) *props.ColumnStatistic {
	...
	case opt.TSScanOp:
		return sb.colStatTSScan(colSet, e.(*TSScanExpr))
		...
		if tableType == tree.TimeseriesTable {
			for i := 0; i < tab.DeletableColumnCount(); i++ {
				...
				if col.IsPrimaryTagCol() {
					primaryTagCols.Add(opt.ColumnID(col.ColID()))
				}
			}
			stats.PTagCount = defaultPTagCount
			stats.RowCount = unknownTSRowCount
...
func (sb *statisticsBuilder) buildTSScan(scan *TSScanExpr, relProps *props.Relational) {
	s := &relProps.Stats
	if zeroCardinality := s.Init(relProps); zeroCardinality {
		// Short cut if cardinality is 0.
		return
...
func (sb *statisticsBuilder) buildTSInsertSelect(
	scan *TSInsertSelectExpr, relProps *props.Relational,
) {
	s := &relProps.Stats
	if zeroCardinality := s.Init(relProps); zeroCardinality {
		// Short cut if cardinality is 0.
	...
	inputStats := sb.makeTableStatistics(scan.STable)
	s.RowCount = inputStats.RowCount
...
func (sb *statisticsBuilder) colStatTSScan(
	colSet opt.ColSet, scan *TSScanExpr,
) *props.ColumnStatistic {
	relProps := scan.Relational()
	s := &relProps.Stats
...
func (sb *statisticsBuilder) buildGroupBy(
	groupNode RelExpr, relProps *props.Relational, groupingColSet opt.ColSet, ErrorOnDup bool,
) {
	s := &relProps.Stats
	if zeroCardinality := s.Init(relProps); zeroCardinality {
		// Short cut if cardinality is 0.
...
func (sb *statisticsBuilder) finalizeFromTableTotalCardinality(
	totalCardinality float64, relProps, inputProps *props.Relational,
) {
	s := &relProps.Stats
	...
	if totalCardinality < 0 {
		panic(pgerror.New(pgcode.ExternalApplyCardinalityScan, "stmt hint err: cardinality must not be lower than zero"))
	}
	if totalCardinality < float64(relProps.Cardinality.Min) ||
		totalCardinality > float64(relProps.Cardinality.Max) {
		panic(pgerror.Newf(pgcode.ExternalApplyCardinalityScan, "stmt hint err: cardinality must be between %d and %d", relProps.Cardinality.Min, relProps.Cardinality.Max))
	} else {
		s.RowCount = totalCardinality
		if inputProps.Stats.RowCount < totalCardinality {
			inputProps.Stats.RowCount = totalCardinality
		}
...
func (sb *statisticsBuilder) finalizeFromJoinTotalCardinality(
	totalCardinality float64, relProps *props.Relational,
) {
	s := &relProps.Stats
