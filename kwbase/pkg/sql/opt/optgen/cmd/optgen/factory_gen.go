...
func (g *factoryGen) generate(compiled *lang.CompiledExpr, w io.Writer) {
	...
	g.w.writeIndent("\"gitee.com/kwbasedb/kwbase/pkg/sql/opt/props/physical\"\n")
	g.w.writeIndent("\"gitee.com/kwbasedb/kwbase/pkg/sql/sem/tree\"\n")
	g.w.writeIndent("\"gitee.com/kwbasedb/kwbase/pkg/sql/types\"\n")
...
func (g *factoryGen) genConstructFuncs() {
	...
	for _, define := range defines {
		...
		for _, field := range fields {
			//ConstructFiltersItem produces only one function
			if define.Name == "FiltersItem" && field.Name == "Stats" {
				continue
			}
			if define.Name == "FiltersItem" && field.Name == "scalar" {
				continue
			}
	...
	for _, define := range g.compiled.Defines.WithTag("List") {
		...
		if itemTyp.isGenerated {
			...
			for i, field := range g.md.childAndPrivateFields(itemDefine) {
				...
				// Construct method.
				fieldName := g.md.fieldName(field)
				if itemDefine.Name == "FiltersItem" && fieldName == "Stats" {
					continue
				}
				fieldTyp := g.md.typeOf(field)
		...
		if define.Tags.Contains("Relational") || len(childFields) != 0 {
			if len(childFields) != 0 {
				if define.Tags.Contains("WithBinding") {
					...
					g.w.unnest("}\n")
					if define.Tags.Contains("Relational") {
						g.w.nestIndent("ret := f.Construct%s(\n", define.Name)
					} else {
				...
				g.w.unnest(")\n")
				if define.Tags.Contains("Relational") {
					g.w.writeIndent("copyPushFlag(src.(memo.RelExpr),ret.(memo.RelExpr))\n")
