...
func (g *exprsGen) generate(compiled *lang.CompiledExpr, w io.Writer) {
	...
	fmt.Fprintf(g.w, "  \"gitee.com/kwbasedb/kwbase/pkg/sql/opt/props\"\n")
	fmt.Fprintf(g.w, "  \"gitee.com/kwbasedb/kwbase/pkg/sql/opt/props/physical\"\n")
	fmt.Fprintf(g.w, "  \"gitee.com/kwbasedb/kwbase/pkg/sql/sem/tree\"\n")
	fmt.Fprintf(g.w, "  \"gitee.com/kwbasedb/kwbase/pkg/sql/types\"\n")
...
func (g *exprsGen) genExprFuncs(define *lang.DefineExpr) {
	...
	fmt.Fprintf(g.w, "func (e *%s) Walk(param opt.ExprParam) bool {\n", opTyp.name)
	fmt.Fprintf(g.w, "    ret := true\n")
	fmt.Fprintf(g.w, "    if param != nil {\n")
	fmt.Fprintf(g.w, "        if param.IsTargetExpr(e) {\n")
	fmt.Fprintf(g.w, "            if param.NeedToHandleChild() {\n")
	fmt.Fprintf(g.w, "                for i := 0; i < e.ChildCount(); i++ {\n")
	fmt.Fprintf(g.w, "                    if !param.HandleChildExpr(e, e.Child(i)) {\n")
	fmt.Fprintf(g.w, "                        ret = false\n")
	fmt.Fprintf(g.w, "    		                 break\n")
	fmt.Fprintf(g.w, "                    }\n")
	fmt.Fprintf(g.w, "                }\n")
	fmt.Fprintf(g.w, "            }\n")
	fmt.Fprintf(g.w, "        } else {\n")
	fmt.Fprintf(g.w, "            for i := 0; i < e.ChildCount(); i++ {\n")
	fmt.Fprintf(g.w, "                if !e.Child(i).Walk(param) {\n")
	fmt.Fprintf(g.w, "                    ret = false\n")
	fmt.Fprintf(g.w, "    		             break\n")
	fmt.Fprintf(g.w, "                }\n")
	fmt.Fprintf(g.w, "            }\n")
	...
	if define.Tags.Contains("Scalar") {
		...
		// Generate the bestProps method.
		if opTyp.name == "TSInsertSelectExpr" {
		...
		// Generate the GetAddSynchronizer method.
		fmt.Fprintf(g.w, "func (e *%s) GetAddSynchronizer() bool {\n", opTyp.name)
		fmt.Fprintf(g.w, "  return e.addSynchronizer\n")
		fmt.Fprintf(g.w, "}\n\n")
	...
	fmt.Fprintf(g.w, "func (e *%s) IsTSEngine() bool {\n", opTyp.name)
	fmt.Fprintf(g.w, "  return e.engine == tree.EngineTypeTimeseries\n")
	fmt.Fprintf(g.w, "}\n\n")

	// Generate the SetEngineTS method.
	fmt.Fprintf(g.w, "func (e *%s) SetEngineTS() {\n", opTyp.name)
	fmt.Fprintf(g.w, "  e.engine = tree.EngineTypeTimeseries\n")
	fmt.Fprintf(g.w, "}\n\n")

	// Generate the SimpleString method.
	fmt.Fprintf(g.w, "func (e *%s) SimpleString(flags int) string {\n", opTyp.name)
	if define.Tags.Contains("Scalar") {
		fmt.Fprintf(g.w, "  f := MakeExprFmtCtx(ExprFmtFlags(flags), nil, nil)\n")
	} else {
		fmt.Fprintf(g.w, "  f := MakeExprFmtCtx(ExprFmtFlags(flags), e.Memo(), nil)\n")
	}
	fmt.Fprintf(g.w, "  f.FormatExpr(e)\n")
	fmt.Fprintf(g.w, "  return f.Buffer.String()\n")
	fmt.Fprintf(g.w, "}\n\n")
...
func (g *exprsGen) genEnforcerFuncs(define *lang.DefineExpr) {
	...
	fmt.Fprintf(g.w, "    if param.IsTargetExpr(e) {\n")
	fmt.Fprintf(g.w, "        if param.NeedToHandleChild() {\n")
	fmt.Fprintf(g.w, "            ret = param.HandleChildExpr(e, e.Input)\n")
	fmt.Fprintf(g.w, "        }\n")
	fmt.Fprintf(g.w, "    } else {\n")
...
func (g *exprsGen) genListExprFuncs(define *lang.DefineExpr) {
	...
	fmt.Fprintf(g.w, "func (e *%s) IsTSEngine() bool {\n", opTyp.name)
	fmt.Fprintf(g.w, "  for i := 0; i < e.ChildCount(); i++ {\n")
	fmt.Fprintf(g.w, "    if !e.Child(i).IsTSEngine() {\n")
	...
	fmt.Fprintf(g.w, "func (e *%s) SetEngineTS() {\n", opTyp.name)
	fmt.Fprintf(g.w, "  for i := 0; i < e.ChildCount(); i++ {\n")
	fmt.Fprintf(g.w, "    e.Child(i).SetEngineTS()\n")
	fmt.Fprintf(g.w, "  }\n")
	fmt.Fprintf(g.w, "}\n\n")
	...
	for _, define := range defines {
		...
		if define.Tags.Contains("Scalar") {
			...
			if define.Name == "Const" {
				fmt.Fprintf(g.w, "   IsConstForLogicPlan: true,\n")
			}
			fmt.Fprintf(g.w, "  }\n")
			if g.needsDataTypeField(define) {
		...
		if define.Tags.Contains("Relational") {
			if define.Name == "Synchronizer" || define.Name == "TSScan" {
				fmt.Fprintf(g.w, "e.engine = tree.EngineTypeTimeseries\n")
			}
			if define.Name == "Limit" || define.Name == "Offset" {
				fmt.Fprintf(g.w, "  if e.Input.IsTSEngine() {\n")
				fmt.Fprintf(g.w, "    e.SetEngineTS()\n")
				fmt.Fprintf(g.w, "  }\n")
			}
		}
...
func (g *exprsGen) genInternFuncs() {
	...
	for _, define := range g.compiled.Defines.WithoutTag("Enforcer").WithoutTag("Private") {
		...
		if define.Name == "TSInsert" || define.Name == "TSDelete" || define.Name == "TSUpdate" || define.Name == "TSInsertSelect" {
