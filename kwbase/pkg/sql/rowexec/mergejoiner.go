...
func (m *mergeJoiner) nextRow() (sqlbase.EncDatumRow, *execinfrapb.ProducerMetadata) {
	...
	for {
		...
		// for semi-joins.
		m.leftRows, m.rightRows, meta = m.streamMerger.NextBatch(m.PbCtx(), m.EvalCtx)
		if meta != nil {
			return nil, meta
		}
...
func (mjs *MergeJoinerStats) TsStats() map[int32]map[string]string {
	return nil
}
...
func (mjs *MergeJoinerStats) TsStatsForQueryPlan() map[int32][]string {
	return nil
}
...
func (m *mergeJoiner) outputStatsToTrace() {
	...
	if sp := opentracing.SpanFromContext(m.PbCtx()); sp != nil {
