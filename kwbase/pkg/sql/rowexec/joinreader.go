...
func (jr *joinReader) Next() (sqlbase.EncDatumRow, *execinfrapb.ProducerMetadata) {
	...
	for jr.State == execinfra.StateRunning {
		...
		default:
			log.Fatalf(jr.PbCtx(), "unsupported state: %d", jr.runningState)
		}
		if row == nil && meta == nil {
			continue
		}
...
func (jr *joinReader) readInput() (joinReaderState, *execinfrapb.ProducerMetadata) {
	...
	if len(jr.inputRows) == 0 {
		log.VEventf(jr.PbCtx(), 1, "no more input rows")
		// We're done.
		jr.MoveToDraining(nil)
		return jrStateUnknown, jr.DrainHelper()
	}
	log.VEventf(jr.PbCtx(), 1, "read %d input rows", len(jr.inputRows))
	...
	// phase.
	sort.Sort(spans)
	log.VEventf(jr.PbCtx(), 1, "scanning %d spans", len(spans))
	err := jr.fetcher.StartScan(
		jr.PbCtx(), jr.FlowCtx.Txn, spans, jr.shouldLimitBatches, 0, /* limitHint */
		jr.FlowCtx.TraceKV)
	if err != nil {
		jr.MoveToDraining(err)
		return jrStateUnknown, jr.DrainHelper()
...
func (jr *joinReader) performLookup() (joinReaderState, *execinfrapb.ProducerMetadata) {
	...
	log.VEventf(jr.PbCtx(), 1, "joining rows")
	...
	for ; ; lookedUpRowIdx++ {
		...
		lookedUpRow, _, _, err := jr.fetcher.NextRow(jr.PbCtx())
		if err != nil {
		...
		if !isJoinTypePartialJoin {
			...
			if err := jr.lookedUpRows.AddRow(jr.PbCtx(), lookedUpRow); err != nil {
				jr.MoveToDraining(err)
				return jrStateUnknown, jr.DrainHelper()
			}
		}
	...
	log.VEventf(jr.PbCtx(), 1, "done joining rows (%d loop iterations)", lookedUpRowIdx)
...
func (jr *joinReader) emitRow() (
	...
	*execinfrapb.ProducerMetadata,
	...
	if jr.emitCursor.inputRowIdx >= len(jr.inputRowIdxToLookedUpRowIdx) {
		log.VEventf(jr.PbCtx(), 1, "done emitting rows")
		...
		if err := jr.lookedUpRows.UnsafeReset(jr.PbCtx()); err != nil {
			jr.MoveToDraining(err)
	...
	lookedUpRow, err := jr.lookedUpRows.GetRow(jr.PbCtx(), lookedUpRowIdx)
	if err != nil {
		jr.MoveToDraining(err)
		return jrStateUnknown, nil, jr.DrainHelper()
	}
...
func (jr *joinReader) close() {
	if jr.InternalClose() {
		if jr.lookedUpRows != nil {
			jr.lookedUpRows.Close(jr.PbCtx())
		}
		jr.MemMonitor.Stop(jr.PbCtx())
		if jr.diskMonitor != nil {
			jr.diskMonitor.Stop(jr.PbCtx())
		}
	}
}
...
func (jrs *JoinReaderStats) TsStats() map[int32]map[string]string {
	return nil
}
...
func (jrs *JoinReaderStats) TsStatsForQueryPlan() map[int32][]string {
	return nil
}
...
func (jr *joinReader) outputStatsToTrace() {
	...
	if sp := opentracing.SpanFromContext(jr.PbCtx()); sp != nil {
