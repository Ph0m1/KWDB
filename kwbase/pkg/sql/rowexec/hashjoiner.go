...
func (h *hashJoiner) build() (hashJoinerState, sqlbase.EncDatumRow, *execinfrapb.ProducerMetadata) {
	...
	for {
		...
		if leftUsage >= h.initialBufferSize && rightUsage >= h.initialBufferSize {
			...
			log.VEventf(h.PbCtx(), 1, "buffer phase found no short stream with buffer size %d", h.initialBufferSize)
		...
		if err := h.rows[side].AddRow(h.PbCtx(), row); err != nil {
			...
			if sqlbase.IsOutOfMemoryError(err) {
				if !h.useTempStorage {
					...
					addErr := h.storedRows.AddRow(h.PbCtx(), row)
					if addErr == nil {
						return hjConsumingStoredSide, nil, nil
					}
	...
	for {
		...
		if row == nil {
			...
			if rc, ok := h.storedRows.(*rowcontainer.HashMemRowContainer); ok {
				...
				err = rc.ReserveMarkMemoryMaybe(h.PbCtx())
			} else if hdbrc, ok := h.storedRows.(*rowcontainer.HashDiskBackedRowContainer); ok {
				err = hdbrc.ReserveMarkMemoryMaybe(h.PbCtx())
			} else {
		...
		err = h.storedRows.AddRow(h.PbCtx(), row)
...
func (h *hashJoiner) readProbeSide() (
	...
	*execinfrapb.ProducerMetadata,
	...
	if h.probingRowState.iter == nil {
		i, err := h.storedRows.NewBucketIterator(h.PbCtx(), row, h.eqCols[side])
		if err != nil {
			h.MoveToDraining(err)
			return hjStateUnknown, nil, h.DrainHelper()
		}
		...
		if err := h.probingRowState.iter.Reset(h.PbCtx(), row); err != nil {
			h.MoveToDraining(err)
			return hjStateUnknown, nil, h.DrainHelper()
		}
	}
...
func (h *hashJoiner) probeRow() (
	...
	*execinfrapb.ProducerMetadata,
	...
	if shouldMark(h.storedSide, h.joinType) {
		...
		} else if err := i.Mark(h.PbCtx(), true); err != nil {
			h.MoveToDraining(err)
			return hjStateUnknown, nil, h.DrainHelper()
		}
	}
...
func (hjs *HashJoinerStats) TsStats() map[int32]map[string]string {
	return nil
}
...
func (hjs *HashJoinerStats) TsStatsForQueryPlan() map[int32][]string {
	return nil
}
