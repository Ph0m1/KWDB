...
const commitOnReleaseSavepointName = "kwbase_restart"
...
func (ex *connExecutor) execSavepointInOpenState(
	...
	// Sanity check for "SAVEPOINT kwbase_restart".
	commitOnRelease := ex.isCommitOnReleaseSavepoint(s.Name)
	if commitOnRelease {
		// Validate the special savepoint kwbase_restart. It cannot be nested
		...
		// SAVEPOINT kwbase_restart
		//
		// Some of our examples use this pattern, issuing the SAVEPOINT kwbase_restart
		...
		// ROLLBACK TO SAVEPOINT kwbase_restart  -> the savepoint no longer exists here
		//
		// Although it would work for any other savepoint but kwbase_restart. But
		...
		err := func() error {
			...
			// ORM compatibility.
			// See also https://gitee.com/kwbasedb/kwbase/issues/15012.
...
func (ex *connExecutor) execRelease(
	...
	if entry.commitOnRelease {
		...
		if errIsRetriable(err) {
			...
			// kwbase_restart (that's the whole point of commitOnRelease).
			env.push(*entry)
...
func (ex *connExecutor) execRollbackToSavepointInOpenState(
	...
	// are not yet enabled but we still support kwbase_restart. In
	// that case, we can't process ROLLBACK TO SAVEPOINT
	// kwbase_restart using ignored seqnum lists so we need to
...
func (ex *connExecutor) checkRollbackValidity(
	...
	if ex.state.mu.txn.UserPriority() == roachpb.MaxUserPriority {
		...
		// See https://gitee.com/kwbasedb/kwbase/issues/46414
		// for details.
		//
...
type savepoint struct {
	...
	// commitOnRelease is set if the special syntax "SAVEPOINT kwbase_restart"
	...
	// retriable error and issue another "ROLLBACK TO SAVEPOINT kwbase_restart"
