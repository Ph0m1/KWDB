...
`); err != nil {
	...
	if _, err := tx.Exec(
		"SELECT kwdb_internal.force_retry('100s':::INTERVAL)"); !testutils.IsError(
...
`); err != nil {
	...
	go func() {
		...
		if err := txRetry.Commit(); !testutils.IsError(err,
			`TransactionRetryWithProtoRefreshError: cannot publish new versions for tables: \[\{kv1 78 1\}\], old versions still in use`,
		) {
			t.Errorf("err = %v", err)
		}
		wg.Done()
...
func TestIntentOnSystemConfigDoesNotPreventSchemaChange(t *testing.T) {
	...
	getFooVersion := func() (version int) {
		...
		require.NoError(t, tx.QueryRow(
			"SELECT version FROM kwdb_internal.tables WHERE name = 'foo'").
			Scan(&version))
