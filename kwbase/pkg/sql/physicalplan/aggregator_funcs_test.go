...
func runTestFlow(
	...
	for {
		...
		if meta != nil {
			if meta.LeafTxnFinalState != nil || meta.Metrics != nil || meta.Err != nil {
				continue
			}
...
func checkDistAggregationInfo(
		...
		return execinfrapb.ProcessorSpec{
			...
			Output: []execinfrapb.OutputRouterSpec{{
				...
				Streams: []execinfrapb.StreamEndpointSpec{
					{Type: execinfrapb.StreamEndpointType_LOCAL, StreamID: execinfrapb.StreamID(streamID)},
				},
			}},
		}
	}
	...
	rowsNonDist := runTestFlow(
		...
		execinfrapb.ProcessorSpec{
			Input: []execinfrapb.InputSyncSpec{{
				...
				Streams: []execinfrapb.StreamEndpointSpec{
					{Type: execinfrapb.StreamEndpointType_LOCAL, StreamID: 0},
				},
			}},
	...
	for i := 0; i < numParallel; i++ {
		...
		agg := execinfrapb.ProcessorSpec{
			Input: []execinfrapb.InputSyncSpec{{
				...
				ColumnTypes: []types.T{colType},
				Streams: []execinfrapb.StreamEndpointSpec{
					{Type: execinfrapb.StreamEndpointType_LOCAL, StreamID: execinfrapb.StreamID(2 * i)},
				},
			}},
			Core: execinfrapb.ProcessorCoreUnion{Aggregator: &execinfrapb.AggregatorSpec{
			...
			Output: []execinfrapb.OutputRouterSpec{{
				Type: execinfrapb.OutputRouterSpec_PASS_THROUGH,
				Streams: []execinfrapb.StreamEndpointSpec{
					{Type: execinfrapb.StreamEndpointType_LOCAL, StreamID: execinfrapb.StreamID(2*i + 1)},
				},
			}},
		}
	...
	if info.FinalRendering != nil {
		...
		expr, err = MakeExpression(renderExpr, nil, nil, true, false)
		if err != nil {
			t.Fatal(err)
		}
...
func TestDistAggregationTable(t *testing.T) {
	...
	sqlutils.CreateTable(
		t, tc.ServerConn(0), "t",
		"k INT8 PRIMARY KEY, int1 INT8, int2 INT8, bool1 BOOL, bool2 BOOL, dec1 DECIMAL, dec2 DECIMAL, float1 FLOAT, float2 FLOAT, b BYTES",
		numRows,
	...
	for fn, info := range DistAggregationTable {
		...
		// sum int can only use ts engine for dist
		if fn == execinfrapb.AggregatorSpec_SUM_INT ||
