	...
	testData := []struct {
	...
	}{
		{
			...
			getRows: func(t *testing.T, sqlDB *gosql.DB) (*gosql.Rows, error) {
				...
				return sqlDB.Query(
					"SELECT DISTINCT operation AS op FROM kwdb_internal.session_trace " +
		...
		},
		{
			...
			getRows: func(t *testing.T, sqlDB *gosql.DB) (*gosql.Rows, error) {
				...
				rows, err := sqlDB.Query(
					"SELECT count(message) FROM kwdb_internal.session_trace " +
						"WHERE message LIKE '%kwbase.stat.tablereader.input.rows: 0%'",
				)
				if err != nil {
					t.Fatal(err)
				}
			...
			// spans.
			optionalSpans: []string{
				"/kwbase.sql.distsqlrun.DistSQL/SetupFlow",
				"noop",
			},
		},
		{
				...
				t.Run(name, func(t *testing.T) {
					for i := 0; i < numNodes; i++ {
						t.Run(fmt.Sprintf("node-%d", i), func(t *testing.T) {
							...
							row := sqlDB.QueryRow("SELECT count(1) FROM kwdb_internal.session_trace")
							var count int
							if err := row.Scan(&count); err != nil {
								t.Fatal(err)
							}
							if count != 0 {
								t.Fatalf("expected kwdb_internal.session_trace to be empty "+
