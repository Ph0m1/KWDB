...
CREATE TABLE perm_table (a int DEFAULT nextval('a_sequence'), b int); INSERT INTO perm_table VALUES (DEFAULT, 1); `)
	...
	require.NoError(
		...
		kvDB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
			...
			if !strings.Contains(err.Error(), fmt.Sprintf(`delete-temp-SEQUENCE: relation "defaultdb.%s.a_a_seq" does not exist`, tempSchemaName)) &&
				!strings.Contains(err.Error(), fmt.Sprintf(`delete-temp-SEQUENCE: relation "defaultdb.%s.a_b_seq" does not exist`, tempSchemaName)) {
				require.NoError(t, err)
			}
			...
			_, err = s.InternalExecutor().(*InternalExecutor).ExecEx(ctx, "delete-temp-a_sequence", txn, override, fmt.Sprintf("DROP SEQUENCE defaultdb.%s.a_sequence CASCADE", tempSchemaName))
			require.NoError(t, err)
