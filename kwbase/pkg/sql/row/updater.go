...
func (ru *Updater) UpdateRow(
	...
	if txn != nil {
		tblDesc := ru.Helper.TableDesc.TableDesc()
		...
		if txn.ModifyReplTbl && txn.ModifyNonReplTbl {
			return nil, errors.Errorf("Txn don't support modify repl tables and non repl tables!")
		}
	}
	...
	if rowPrimaryKeyChanged {
		if err := ru.rd.DeleteRow(ctx, batch, oldValues, SkipFKs, traceKV, txn); err != nil {
			return nil, err
		}
		if err := ru.ri.InsertRow(
			ctx, batch, ru.newValues, false /* ignoreConflicts */, SkipFKs, traceKV, txn,
		); err != nil {
			return nil, err
		}
