...
import (
	...
	"gitee.com/kwbasedb/kwbase/pkg/util/shuffle"
...
// for rolling back to the kwbase_restart savepoint. See
...
func (rs rollbackStrategy) SQLCommand() string {
	...
	case rollbackToSavepoint:
		return "ROLLBACK TO SAVEPOINT kwbase_restart"
	case declareSavepoint:
		return "SAVEPOINT kwbase_restart"
	}
	panic("unreachable")
}
...
func retryExec(t *testing.T, sqlDB *gosql.DB, rs rollbackStrategy, fn func(*gosql.Tx) bool) {
	...
	if _, err := tx.Exec(
		"SAVEPOINT kwbase_restart; SET TRANSACTION PRIORITY LOW"); err != nil {
		t.Fatal(err)
	}
...
`); err != nil {
	...
	if _, err := tx.Exec(
		"SAVEPOINT kwbase_restart; RELEASE kwbase_restart"); err != nil {
		t.Fatal(err)
	}
...
`); err != nil {
	...
	// txn that had a "retry intent" (SAVEPOINT kwbase_restart), and once in a
	// txn without it.
	testCases := []struct {
		retryIntent bool
	}{
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("retryIntent=%t", tc.retryIntent), func(t *testing.T) {
			...
			if tc.retryIntent {
				if _, err := sqlDB.Exec("SAVEPOINT kwbase_restart"); err != nil {
					t.Fatal(err)
				}
			}
			if _, err := sqlDB.Exec(insertStmt); err != nil {
...
`); err != nil {
	...
	if _, err := tx.Exec("SAVEPOINT kwbase_restart"); err != nil {
		t.Fatal(err)
	}
	// Run a batch of statements to move the txn out of the AutoRetry state,
	// otherwise the INSERT below would be automatically retried.
	...
	if _, err := tx.Exec("RELEASE SAVEPOINT kwbase_restart"); !testutils.IsError(
...
func TestUnexpectedStatementInRestartWait(t *testing.T) {
	...
	if _, err := tx.Exec(
		"SELECT kwdb_internal.force_retry('1s':::INTERVAL)"); !testutils.IsError(
		err, `forced by kwdb_internal\.force_retry\(\)`) {
		t.Fatal(err)
	}
	var state string
...
func TestDistSQLRetryableError(t *testing.T) {
	...
	targetKey := roachpb.Key("\326\211\212")
...
func TestRollbackToSavepointFromUnusualStates(t *testing.T) {
	...
	if _, err := tx.Exec("ROLLBACK TO SAVEPOINT kwbase_restart"); err != nil {
		t.Fatal(err)
	}
	checkState(tx, ts)
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.clientDirectedRetry {
				savepoint = "SAVEPOINT kwbase_restart;"
			}
			...
			sql := fmt.Sprintf(`
				...
				SELECT
					CASE x
          WHEN 10000 THEN kwdb_internal.force_retry('1s')
          ELSE x
					END
