...
func TestDropWhileBackfill(t *testing.T) {
	defer leaktest.AfterTest(t)()
	t.Skip("https://gitee.com/kwbasedb/kwbase/issues/44944")
	// protects backfillNotification
	var mu syncutil.Mutex
	backfillNotification := make(chan struct{})
...
`); err != nil {
	...
	// written in the past when kwbasedb used to write sentinel
...
func TestVisibilityDuringPrimaryKeyChange(t *testing.T) {
	...
	if _, err := sqlDB.Exec(`
CREATE DATABASE t;
CREATE TABLE t.test (x INT8 PRIMARY KEY, y INT8 NOT NULL, z INT8, INDEX i (z));
...
`); err != nil {
	...
	SELECT message FROM [SHOW KV TRACE FOR SESSION] WHERE
		message LIKE 'InitPut /Table/78/2%' ORDER BY message;`)
	if err != nil {
		t.Fatal(err)
	}

	expected := []string{
		"InitPut /Table/78/2/2/0 -> /TUPLE/1:1:Int/1",
		// TODO (rohany): this k/v is spurious and should be removed
		//  when #45343 is fixed.
		"InitPut /Table/78/2/2/1/1 -> /INT/2",
		"InitPut /Table/78/2/2/2/1 -> /TUPLE/3:3:Int/3",
		"InitPut /Table/78/2/2/4/1 -> /INT/6",
	}
	...
	SET TRACING=off;
	SELECT message FROM [SHOW KV TRACE FOR SESSION] WHERE
		message LIKE 'Del /Table/78/2%' ORDER BY message;`)
	if err != nil {
		t.Fatal(err)
	}
	...
	expected = []string{
		"Del /Table/78/2/2/0",
		"Del /Table/78/2/2/1/1",
		"Del /Table/78/2/2/2/1",
		"Del /Table/78/2/2/3/1",
		"Del /Table/78/2/2/4/1",
	}
	require.Equal(t, expected, scanToArray(rows))
	...
	SET TRACING=off;
	SELECT message FROM [SHOW KV TRACE FOR SESSION] WHERE
		message LIKE 'Put /Table/78/2%' OR
		message LIKE 'Del /Table/78/2%' OR
		message LIKE 'CPut /Table/78/2%';`)
	if err != nil {
		t.Fatal(err)
	}
	...
	expected = []string{
		"Del /Table/78/2/2/0",
		"CPut /Table/78/2/3/0 -> /TUPLE/1:1:Int/1 (expecting does not exist)",
		// TODO (rohany): this k/v is spurious and should be removed
		//  when #45343 is fixed.
		"Del /Table/78/2/2/1/1",
		"CPut /Table/78/2/3/1/1 -> /INT/3 (expecting does not exist)",
		"Del /Table/78/2/2/2/1",
		"CPut /Table/78/2/3/2/1 -> /TUPLE/3:3:Int/3 (expecting does not exist)",
		"Del /Table/78/2/2/4/1",
		"CPut /Table/78/2/3/4/1 -> /INT/6 (expecting does not exist)",
	}
	require.Equal(t, expected, scanToArray(rows))
	...
	expected = []string{
		"Del /Table/78/2/3/2/1",
		"CPut /Table/78/2/3/3/1 -> /INT/5 (expecting does not exist)",
		"Del /Table/78/2/3/4/1",
	}
	require.Equal(t, expected, scanToArray(rows))
...
func TestTruncateWhileColumnBackfill(t *testing.T) {
	...
	t.Skip("https://gitee.com/kwbasedb/kwbase/issues/43990")
...
// needs to be retried it gets retried internal to kwbase. This test
...
`); err != nil {
	...
	FROM
	kwdb_internal.check_consistency(false, '', '') as t
...
func TestSchemaChangeJobRunningStatusValidation(t *testing.T) {
	...
	if _, err := sqlDB.Exec(`
CREATE DATABASE t;
CREATE TABLE t.test (k INT8 PRIMARY KEY, v INT8);
...
`); err != nil {
	...
	if _, err := sqlDB.Exec(
		`ALTER TABLE t.test ADD COLUMN a INT8 AS (v - 1) STORED, ADD CHECK (a < v AND a IS NOT NULL)`,
	); err != nil {
		t.Fatal(err)
	}
}
...
SELECT job_id FROM kwdb_internal.jobs WHERE description LIKE '%ALTER TABLE%'
AND descriptor_ids[1] = 'db.t2'::regclass::int`,
	...
	var error string
	tdb.QueryRow(t, "SELECT status, error FROM kwdb_internal.jobs WHERE job_id = $1", jobID).
		Scan(&status, &error)
...
SELECT job_id FROM kwdb_internal.jobs 
