...
func (pb *ProcessorBase) moveToTrailingMeta() {
	...
	if pb.span != nil {
		if trace := GetTraceData(pb.PbCtx()); trace != nil {
	...
	if pb.trailingMetaCallback != nil {
		pb.trailingMeta = append(pb.trailingMeta, pb.trailingMetaCallback(pb.PbCtx())...)
	} else {
		pb.InternalClose()
	}
}
...
func (pb *ProcessorBase) ProcessRowHelper(row sqlbase.EncDatumRow) sqlbase.EncDatumRow {
	outRow, ok, err := pb.Out.ProcessRow(pb.PbCtx(), row)
	if err != nil {
...
func (pb *ProcessorBase) Start(ctx context.Context) context.Context {
	return nil
}

// PbCtx is an accessor method for ctx which is guaranteed to return non-nil
// context even if StartInternal() hasn't been called.
func (pb *ProcessorBase) PbCtx() context.Context {
	if pb.Ctx == nil {
...
func (pb *ProcessorBase) RunTS(ctx context.Context) {
	if pb.Out.output == nil {
		panic("processor output not initialized for emitting rows")
	}
	start := timeutil.Now()
...
func ProcessorSpan(
	ctx context.Context, name string, seed int32,
) (context.Context, opentracing.Span) {
	return tracing.ChildSpanSeparateRecording(ctx, name, seed)
}
...
func (pb *ProcessorBase) StartInternal(ctx context.Context, name string) context.Context {
	pb.origCtx = ctx
	pb.Ctx, pb.span = ProcessorSpan(ctx, name, pb.ProcessorID)
