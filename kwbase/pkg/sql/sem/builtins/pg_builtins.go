...
func initPGBuiltins() {
	...
	// Make kwdb_internal.create_regfoo builtins.
	for _, typ := range []*types.T{types.RegType, types.RegProc, types.RegProcedure, types.RegClass, types.RegNamespace} {
		...
		builtins["kwdb_internal.create_"+typName] = makeCreateRegDef(typ)
	}
}
	...
	return tree.Overload{
		...
		Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
			...
			r, err := ctx.InternalExecutor.QueryRow(
				...
				ctx.Txn,
				"SELECT indexdef FROM pg_catalog.pg_indexes WHERE kwdb_oid = $1", args[0])
			if err != nil {
				return nil, err
			}
