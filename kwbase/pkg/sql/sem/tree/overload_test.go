...
func TestTypeCheckOverloadedExprs(t *testing.T) {
	...
	binaryFloatIntFn := makeTestOverload(types.Float, types.Float, types.Int)
	...
	testData := []struct {
		...
		{nil, []Expr{decConst("1.1"), plus(intConst("1"), intConst("2"))}, []overloadImpl{binaryIntFn, binaryDecimalFn, binaryFloatIntFn}, binaryFloatIntFn, false},
	...
	for i, d := range testData {
		t.Run(fmt.Sprintf("%v/%v", d.exprs, d.overloads), func(t *testing.T) {
			...
			typedExprs, fns, err := typeCheckOverloadedExprs(
				&ctx, desired, d.overloads, false, d.inBinOp, d.exprs...,
			)
			assertNoErr := func() {
				if err != nil {
