...
// DatumWithPlaceHolderIdx is a Datum that
// contains Type information and PlaceholderID.
// ConstPlaceHolderExpr will be converted to
// DatumWithPlaceHolderIdx when generating the plan.
type DatumWithPlaceHolderIdx struct {
	Datum
...
func (v *Placeholder) DatumToExpr() (Expr, error) {
	return nil, pgerror.Newf(pgcode.Warning, "%v type is not supported convert to expr", v.String())
}
...
// Format implements the NodeFormatter interface.
func (v *Placeholder) Format(ctx *FmtCtx) {
	if ctx.placeholderFormat != nil {
		ctx.placeholderFormat(ctx, v)
		return
	}
...
// ResolvedType implements the TypedExpr interface.
func (v *Placeholder) ResolvedType() *types.T {
	if v.typ == nil {
		return types.Any
	}
	return v.typ
...
// IsPushDisabled return true when the node is an udf function.
func (node *FuncExpr) IsPushDisabled() bool {
	return node.fnProps != nil && node.fnProps.ForbiddenExecInTSEngine
}
...
// Reset reset fn and fnProps of FuncExpr.
func (node *FuncExpr) Reset() {
	node.fn = nil
	node.fnProps = nil
}
...
// ChangeToNull return dNull when the expr of CastExpr is dNull.
// ChangeToNull use to replace CastExpr with dNull.
// ex: cast(a as null) -> dNull
func (node *CastExpr) ChangeToNull() TypedExpr {
	if val, ok := node.Expr.(dNull); ok {
		return val
	}
	return node
}
