...
// FormatTags recurses into a node for pretty-printing.
// Flag-driven special cases can hook into this.
func (ctx *FmtCtx) FormatTags(n Tags) {
	for i, tag := range n {
		ctx.WriteString(string(tag.TagName))
		ctx.WriteByte(' ')
		ctx.WriteString(tag.TagType.SQLString())
		if !tag.Nullable {
