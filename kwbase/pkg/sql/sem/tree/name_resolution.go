...
func classifyTablePattern(n *UnresolvedName) (TablePattern, error) {
	...
	// We allow this in e.g. `select * from "".kwdb_internal.tables`.
...
func classifyColumnItem(n *UnresolvedName) (VarName, error) {
	...
	// `select "".kwdb_internal.tables.table_id from "".kwdb_internal.tables`.
	lastCheck := n.NumParts
	if lastCheck > 3 {
		lastCheck = 3
	}
...
type TableNameExistingResolver interface {
	...
	//LookupKWDBObject(ctx context.Context, dbName, scName, tbName string) (
	//	found bool, objMeta NameResolutionResult, err error,
	//)
	LookupSubObject(ctx context.Context, flags ObjectLookupFlags, dbName, scName, tbName string) (
		found bool, objMeta NameResolutionResult, err error,
	)
}
...
func (t *TableName) ResolveExisting(
	...
	updateCTbPrefixName := func(found bool) {
		if found && t.SchemaName == "" && t.CatalogName == "" {
			t.SchemaName = PublicSchemaName
			t.CatalogName = Name(curDb)
		}
		if found && t.CatalogName == "" {
			if t.SchemaName == PublicSchemaName {
				t.CatalogName = Name(curDb)
			} else {
	...
	if t.ExplicitSchema {
		...
		if t.ExplicitCatalog {
			...
			found, objMeta, err := r.LookupObject(ctx, lookupFlags, t.Catalog(), scName, t.Table())
			if found || err != nil {
		...
		// meybe the table is a sub table, find the sub table in ts case.
		if found, objMeta, err := r.LookupSubObject(ctx, lookupFlags, t.Catalog(), t.Schema(), t.Table()); found || err != nil {
			updateCTbPrefixName(found)
			return found, objMeta, err
		}
	...
	// maybe the table is a sub table, find the sub table in ts case.
	if found, objMeta, err := r.LookupSubObject(ctx, lookupFlags, curDb, PublicSchema, t.Table()); found || err != nil {
		updateCTbPrefixName(found)
		return found, objMeta, err
	}
...
// DBObjParse parse database objects without prefix
func (t *TableName) DBObjParse(
	ctx context.Context,
	r TableNameExistingResolver,
	lookupFlags ObjectLookupFlags,
	curDb string,
...
type ObjectLookupFlags struct {
	...
	// used to parse time series objects
	Physicaltype PhysicalLayerType
}
