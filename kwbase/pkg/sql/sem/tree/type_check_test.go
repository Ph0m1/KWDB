...
func TestTypeCheck(t *testing.T) {
	...
	testData := []struct {
		...
		{`ARRAY[NULL]:::int[]`, `ARRAY[NULL]:::INT4[]`},
		{`ARRAY[NULL, NULL]:::int[]`, `ARRAY[NULL, NULL]:::INT4[]`},
...
func TestTypeCheckError(t *testing.T) {
	...
	testData := []struct {
		...
		{`lower(1, 2)`, `unknown signature: lower(INT8, INT8)`},
		{`lower(1)`, `unknown signature: lower(INT8)`},
		...
		{`3:::int[]`, `incompatible type annotation for 3 as int4[], found type: int`},
		...
		{`ARRAY[1]::int[] = ARRAY[1.0]::decimal[]`, `unsupported comparison operator: <int4[]> = <decimal[]>`},
