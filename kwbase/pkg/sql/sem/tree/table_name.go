...
// PhysicalLayerType id Physical object types for different engines
type PhysicalLayerType string

const (
	//Invalid is invlid object
	Invalid PhysicalLayerType = ""

	//TS is time series object
	TS PhysicalLayerType = "timeseries"

	//DB is mixed object
	DB PhysicalLayerType = "db"
)

// LogicPlanType distinguish the types of logical plans.
type LogicPlanType int

const (
	// Default is DB query.
	Default = iota

	// General is TS general query.
	General
)
...
type TableNamePrefix struct {
	...
	// PhysType distinguishs what kind of physical object.
	PhysType PhysicalLayerType
}
...
// MakeDeviceName creates a new table name qualified with just a schema.
func MakeDeviceName(db, sc, tbl Name) TableName {
	return TableName{tblName{
		TableName: tbl,
		TableNamePrefix: TableNamePrefix{
			CatalogName:     db,
