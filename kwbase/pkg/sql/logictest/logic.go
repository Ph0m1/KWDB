...
// its input files are hosted at https://github.com/kwbasedb/sqllogictest)
//
...
//
//  - skipif <mysql/mssql/postgresql/kwbasedb>
//    Skips the following `statement` or `query` if the argument is postgresql
//    or kwbasedb.
//
//  - onlyif <mysql/mssql/postgresql/kwbasedb>
...
func (t *logicTest) setup(cfg testClusterConfig) {
	...
	if cfg.mppMode {
		dir, dirCleanupFn := testutils.TempDir(t.rootT)
		defer dirCleanupFn()
		if params.ServerArgsPerNode == nil {
			params.ServerArgsPerNode = map[int]base.TestServerArgs{}
		}
		for i := 0; i < cfg.numNodes; i++ {
			perNode := params.ServerArgsPerNode[i]
			perNode.ModeFlag = 1
			perNode.StoreSpecs = []base.StoreSpec{{Path: dir + "n" + strconv.Itoa(i)}}
			params.ServerArgsPerNode[i] = perNode
		}
	}
...
var skipLogicTests = envutil.EnvOrDefaultBool("KWBASE_LOGIC_TESTS_SKIP", false)
var logicTestsConfigExclude = envutil.EnvOrDefaultString("KWBASE_LOGIC_TESTS_SKIP_CONFIG", "")
var logicTestsConfigFilter = envutil.EnvOrDefaultString("KWBASE_LOGIC_TESTS_CONFIG", "")
...
func RunLogicTest(t *testing.T, serverArgs TestServerArgs, globs ...string) {
	...
	if skipLogicTests {
		t.Skip("KWBASE_LOGIC_TESTS_SKIP")
	}
