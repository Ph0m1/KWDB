...
func TestPGPrepareFail(t *testing.T) {
	...
	testFailures := map[string]string{
		...
		"CREATE TABLE $1 (id INT8)":                 "pq: at or near \"1\": syntax error",
	...
	if _, err := db.Exec(`CREATE DATABASE d; CREATE TABLE d.t (i INT8, s STRING, d INT8)`); err != nil {
		t.Fatal(err)
	}
...
func TestPGPreparedQuery(t *testing.T) {
	...
	queryTests := []struct {
		...
		{"SELECT descriptor FROM system.descriptor WHERE descriptor != $1 LIMIT 1", []preparedQueryTest{
			baseTest.SetArgs([]byte("abc")).Results([]byte("\x121\n\x06system\x10\x01\x1a\x15\n\t\n\x05admin\x100\n\b\n\x04root\x100\"\x00(\x002\n\b\x00\x10\x00\x1a\x00\"\x00(\x00")),
		}},
		{"SHOW COLUMNS FROM system.users", []preparedQueryTest{
			baseTest.
				Results("username", "STRING", false, gosql.NullBool{}, "", "{primary}", false, false).
				Results("hashedPassword", "BYTES", true, gosql.NullBool{}, "", "{}", false, false).
				Results("isRole", "BOOL", false, false, "", "{}", false, false),
		}},
		{"SHOW DATABASES", []preparedQueryTest{
			baseTest.Results("d", "RELATIONAL").Results("defaultdb", "RELATIONAL").Results("postgres", "RELATIONAL").Results("system", "RELATIONAL"),
		}},
		...
		{"SHOW TABLES FROM system", []preparedQueryTest{
			baseTest.Results("audits", "BASE TABLE").Others(32),
		}},
		{"SHOW SCHEMAS FROM system", []preparedQueryTest{
			baseTest.Results("information_schema").Others(3),
		}},
	...
	initStmt := `
CREATE DATABASE d;
CREATE TABLE d.t (a INT8);
INSERT INTO d.t VALUES (10),(11);
CREATE TABLE d.ts (a TIMESTAMP, b DATE);
CREATE TABLE d.two (a INT8, b INT8);
CREATE TABLE d.intStr (a INT8, s STRING);
CREATE TABLE d.str (s STRING, b BYTES);
CREATE TABLE d.arr (a INT8[], b TEXT[]);
CREATE TABLE d.emptynorows (); -- zero columns, zero rows
CREATE TABLE d.emptyrows (x INT8);
...
func TestPGPreparedExec(t *testing.T) {
	...
	execTests := []struct {
		...
		{
			"CREATE TABLE d.public.t (i INT8, s STRING, d INT)",
			[]preparedExecTest{
				baseTest,
		...
		{
			"CREATE TABLE d.public.types (i int8, f float, s string, b bytes, d date, m timestamp, z timestamp with time zone, n interval, o bool, e decimal)",
			[]preparedExecTest{
				baseTest,
			},
		},
...
func TestPGPrepareNameQual(t *testing.T) {
	...
	statements := []string{
		`CREATE TABLE IF NOT EXISTS f (v INT8)`,
...
func TestPGPrepareInvalidate(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			stmt: `CREATE TABLE IF NOT EXISTS ab (a INT8 PRIMARY KEY, b INT)`,
		},
		{
...
func TestPGCommandTags(t *testing.T) {
	...
	if _, err := db.Exec(`CREATE TABLE testing.tags (k INT8 PRIMARY KEY, v INT)`); err != nil {
		t.Fatal(err)
	}
...
func TestPGWireResultChange(t *testing.T) {
	...
	if _, err := db.Exec(`CREATE TABLE testing.f (v INT8)`); err != nil {
		t.Fatal(err)
	}
