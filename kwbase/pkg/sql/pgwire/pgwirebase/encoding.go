...
func DecodeOidDatum(
	...
	case FormatBinary:
		...
		case oid.T_float4:
			...
			// float4 -> float8 causes prec problem, use float4 -> string -> float8 instead.
			// todo: wsh
			tempStr := strconv.FormatFloat(float64(f), 'f', -1, 32)
			tempFloat, err := strconv.ParseFloat(tempStr, 64)
			if err != nil {
				return nil, err
			}
			return tree.NewDFloat(tree.DFloat(tempFloat)), nil
		case oid.T_float8:
			if len(b) < 8 {
