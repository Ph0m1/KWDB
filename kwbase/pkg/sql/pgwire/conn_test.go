...
func TestConnectionMaxLimit(t *testing.T) {
	defer leaktest.AfterTest(t)()
	s, db, _ := serverutils.StartServer(t, base.TestServerArgs{})
	ctx := context.TODO()
	defer s.Stopper().Stop(ctx)
	...
	if _, err := db.Exec(`SET CLUSTER SETTING server.sql_connections.max_limit = 4`); err != nil {
		t.Fatal(err)
	}
	...
	pgURL1, cleanupFunc1 := sqlutils.PGUrl(
		t, s.ServingSQLAddr(), "testConnectionMaxLimit" /* prefix */, url.User(security.RootUser),
	)
	defer cleanupFunc1()

	pgURL2, cleanupFunc2 := sqlutils.PGUrl(
		t, s.ServingSQLAddr(), "testConnectionMaxLimit" /* prefix */, url.User("testuser"),
	)
	defer cleanupFunc2()

	db1, err := runClient(pgURL2)
	if !testutils.IsError(err, ErrUserConnectionLimited) {
		t.Errorf("unexpected error: %v", err)
	}
	defer db1.Close()

	db2, err := runClient(pgURL1)
	if err != nil {
		t.Fatal(err)
	}
	defer db2.Close()

	db3, err := runClient(pgURL1)
	if err != nil {
		t.Fatal(err)
	}
	defer db3.Close()

	db4, err := runClient(pgURL1)
	if err != nil {
		t.Fatal(err)
	}
	defer db4.Close()

	db5, err := runClient(pgURL1)
	if !testutils.IsError(err, ErrConnectionLimited) {
		t.Fatal(err)
	}
	defer db5.Close()
}

func runClient(pgURL url.URL) (*gosql.DB, error) {
	db, err := gosql.Open("postgres", pgURL.String())
	if err != nil {
		return nil, err
	}
