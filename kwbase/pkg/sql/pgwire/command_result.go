...
type commandResult struct {
	...
	// hasRowDesc is set when conn.writeRowDescription to buff and conn.Flush not called
	hasRowDesc bool
}
...
// AddPGResult is part of the CommandResult interface.
func (r *commandResult) AddPGResult(ctx context.Context, res []byte) error {
	var err error
	_, err = r.conn.writerState.buf.Write(res)
	if err != nil {
		return err
	}
	//fmt.Println(r.conn.writerState.buf.Bytes())
...
// StmtType is part of the RestrictedCommandResult interface.
...
// WarningForExport is part of the CommandResult interface.
func (r *commandResult) WarningForExport(warning error) {
	r.flushBeforeCloseFuncs = append(
		r.flushBeforeCloseFuncs,
		func(ctx context.Context) error { return r.conn.bufferWarning(ctx, warning) },
	)
}
...
func (r *commandResult) SetColumns(ctx context.Context, cols sqlbase.ResultColumns) {
	r.assertNotReleased()
	r.conn.writerState.fi.registerCmd(r.pos)
	if r.descOpt == sql.NeedRowDesc && !r.hasRowDesc {
...
func isExport(cols sqlbase.ResultColumns) bool {
	if len(cols) != len(sqlbase.ExportColumns) {
		return false
	}
	for i := 0; i < len(cols); i++ {
		if cols[i].Name != sqlbase.ExportColumns[i].Name {
			return false
		}
	}
	return true
...
// AddPGResult is part of the CommandResult interface.
func (r *limitedCommandResult) AddPGResult(ctx context.Context, res []byte) error {
	var err error
	_, err = r.conn.writerState.buf.Write(res)
	if err != nil {
		return err
...
// IsLimit is part of the RestrictedCommandResult interface.
func (r *limitedCommandResult) IsLimit() bool {
	return true
}
