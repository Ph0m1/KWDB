...
func (b *writeBuffer) writeTextDatumWithOid(
	ctx context.Context, d tree.Datum, conv sessiondata.DataConversionConfig, Oid oid.Oid,
) {
	if log.V(2) {
		log.Infof(ctx, "pgwire writing TEXT datum of type: %T, %#v", d, d)
	}
	...
	case *tree.DFloat:
		...
		case oid.T_float4:
			// Start at offset 4 because `putInt32` clobbers the first 4 bytes.
			s := strconv.AppendFloat(b.putbuf[4:4], float64(*v), 'f', 6, 32)
			b.putInt32(int32(len(s)))
			b.write(s)
		default:
			// Start at offset 4 because `putInt32` clobbers the first 4 bytes.
