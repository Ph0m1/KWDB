...
var (
	...
	MetaSucConns = metric.Metadata{
		Name:        "sql.suc_conns",
		Help:        "Number of active sql connections that have been successfully established",
		Measurement: "Connections",
		Unit:        metric.Unit_COUNT,
	}
...
var baseSQLMemoryBudget = envutil.EnvOrDefaultInt64("KWBASE_BASE_SQL_MEMORY_BUDGET",
...
func makeServerMetrics(
	...
	return ServerMetrics{
		...
		SucConns:       metric.NewGauge(MetaSucConns),
...
var noteworthySQLMemoryUsageBytes = envutil.EnvOrDefaultInt64("KWBASE_NOTEWORTHY_SQL_MEMORY_USAGE", 100*1024*1024)
...
// usage growth in the log.
var noteworthyConnMemoryUsageBytes = envutil.EnvOrDefaultInt64("KWBASE_NOTEWORTHY_CONN_MEMORY_USAGE", 2*1024*1024)
...
// https://gitee.com/kwbasedb/kwbase/blob/master/docs/RFCS/20160425_drain_modes.md
//
	...
	for {
		...
		case "mode":
			...
			if err != nil {
				return sql.SessionArgs{}, errors.WithSecondaryError(
					pgerror.Newf(pgcode.ProtocolViolation,
						"error parsing mode option value '%s' as integer", value), err)
			}
			args.HasConnectionMode = true
			args.ConnectionMode = mode
