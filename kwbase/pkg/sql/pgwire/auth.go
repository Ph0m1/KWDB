...
func (c *conn) handleAuthentication(
	...
	c.sessionArgs.Roles = []string{}
	if roles, err := c.getRolesOfUser(ctx, execCfg.InternalExecutor, c.sessionArgs.User); err == nil {
		for k := range roles {
			c.sessionArgs.Roles = append(c.sessionArgs.Roles, k)
		}
	} else {
		log.Warningf(ctx, "error when getting roles, err:%s", err)
	}
...
func (c *conn) lookupAuthenticationMethodUsingRules(
	...
	for i := range auth.Entries {
		...
		if err != nil {
			...
			// See: https://gitee.com/kwbasedb/kwbase/issues/43716
			return
		}
		if !connMatch {
