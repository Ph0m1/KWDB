...
func (c *C) check() {
	...
	for e := c.mu.used.next; e != &c.mu.used; e = e.next {
		...
		if e.SQL == "" {
			panic("used entry with empty SQL")
		}
...
// GetPutPlanTimeStamp gets the timestamp when
// the PlanNode of the SQL statement was added to the QueryCache.
func (c *C) GetPutPlanTimeStamp(sql string) (time.Time, bool) {
	c.mu.Lock()
	tmpl, ok := c.mu.m[sql]
	c.mu.Unlock()

	if ok {
		return tmpl.PlanNodeTimestamp, true
	}
