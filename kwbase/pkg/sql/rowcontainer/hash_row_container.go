...
type HashDiskBackedRowContainer struct {
	...
	// orderForWindowFunc add sorting within over.
	orderForWindowFunc sqlbase.ColumnOrdering
}
...
// SetOrder set orderForWindowFunc.
func (h *HashDiskBackedRowContainer) SetOrder(orderForWindowFunc sqlbase.ColumnOrdering) {
	h.orderForWindowFunc = orderForWindowFunc
}
...
func (h *HashDiskBackedRowContainer) Init(
	...
	if len(h.orderForWindowFunc) > 0 {
		ordering = append(ordering, h.orderForWindowFunc...)
	}
