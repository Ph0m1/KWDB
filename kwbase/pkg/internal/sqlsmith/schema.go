...
ORDER BY
	...
	emit := func() error {
		if lastSchema != tree.PublicSchemaName {
			return nil
		}
...
`, t.TableName.Table(), idx))
			...
			if err = row.Scan(&isInverted); err != nil {
				...
				// not present in kwdb_internal.table_indexes vtable (probably because
...
var functions = func() map[tree.FunctionClass]map[oid.Oid][]function {
	...
	for _, def := range tree.FunDefs {
		...
		if strings.Contains(def.Name, "kwdb_internal.force_") {
			continue
		}
