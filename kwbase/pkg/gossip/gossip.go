...
type Gossip struct {
	*server // Embedded gossip RPC server
	...
	storePath         string
	startMode         string
	StartModeCh       chan bool
	//defaultTsPartitionConfig *api.EntityRangeGroup
}
...
// CheckAndSaveClusterStartMode check and save cluster start mode.
func (g *Gossip) CheckAndSaveClusterStartMode(startMode string) error {
	if startMode == "" {
		return nil
	}
	file := filepath.Join(g.storePath, "kwbase.start_mode")

	if _, err := os.Stat(file); os.IsNotExist(err) {
		if err := ioutil.WriteFile(file, []byte(startMode), 0644); err != nil {
			return err
		}
		if g.startMode != startMode {
			g.StartModeCh <- true
			err := fmt.Errorf("ERROR: Check start mode failed! Node start mode is %s, should be %s",
...
func (g *Gossip) updateNodeAddress(key string, content roachpb.Value) {
	...
	// JoinNode save start mode of the cluster's start mode
	if isMeStarted == false {
		if err := g.CheckAndSaveClusterStartMode(desc.StartMode); err != nil {
			fmt.Println(err)
		}
	}
}
