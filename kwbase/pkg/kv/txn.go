...
type Txn struct {
	...
	// ModifyReplTbl is whether current txn modifies replication tables
	ModifyReplTbl bool
	// ModifyNonReplTbl is whether current txn modifies non replication tables
	ModifyNonReplTbl bool

	// NotReplyForTS not replay for ts
	NotReplyForTS bool
...
func NewTxnFromProto(
	...
	txn.ModifyReplTbl = false
	txn.ModifyNonReplTbl = false
	return txn
}
...
// SetUnConsistency set un consistency
func (txn *Txn) SetUnConsistency(unConsistency bool) {
	if txn.typ != RootTxn {
		panic(errors.AssertionFailedf("SetUnConsistency() called on leaf txn"))
	}

	txn.mu.Lock()
	defer txn.mu.Unlock()

	txn.mu.sender.SetUnconsistency(unConsistency)
}
...
// GetWithUnConsistency with un consistency
func (txn *Txn) GetWithUnConsistency(ctx context.Context, key interface{}) (KeyValue, error) {
	b := txn.NewBatch()
	b.Get(key)
	txn.SetUnConsistency(false)
...
// ScanWithUnReadConsistency with un consistency
func (txn *Txn) ScanWithUnReadConsistency(
	ctx context.Context, begin, end interface{}, maxRows int64,
) ([]KeyValue, error) {
...
// https://gitee.com/kwbasedb/kwbase/issues/15012
...
// This method is only valid when called on RootTxns.if
