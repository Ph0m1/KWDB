...
// DB is a database handle to a single kwbase cluster. A DB is safe for
...
// AdminReplicaStatusConsistent check replicas status consistent
func (db *DB) AdminReplicaStatusConsistent(
	ctx context.Context, begin, end interface{},
) (bool, error) {
	b := &Batch{}
	b.adminGetReplicaStatus(begin, end)
	if err := getOneErr(db.Run(ctx, b), b); err != nil {
		return false, err
	}
	responses := b.response.Responses
	if len(responses) == 0 {
		return false, errors.Errorf("unexpected empty responses for AdminGetReplicaStatus")
	}
	resp, ok := responses[0].GetInner().(*roachpb.GetReplicaStatusResponse)
	if !ok {
		return false, errors.Errorf("unexpected response of type %T for AdminGetReplicaStatus",
			responses[0].GetInner())
	}

	rangeStatusMap := make(map[roachpb.RangeID][]roachpb.ReplicaStatus)
	for _, s := range resp.ReplicaStatus {
		rangeID := s.RangeID
		if rangeID != 0 {
			rangeStatusMap[rangeID] = append(rangeStatusMap[rangeID], s)
		} else {
			log.Errorf(ctx, "get replica status failed %v", s)
			return false, errors.Errorf("get replica status failed")
		}
	}

	for rangeID, status := range rangeStatusMap {
		//fmt.Println("raftlog status: %v", status)
		// 1 replica:
		// {55, 15, 15, true, 1, 1}
		if len(status) > 1 {
			applyIndex := status[0].ApplyIndex
			lastIndex := status[0].LastIndex
			if applyIndex != lastIndex {
				log.Errorf(ctx, "range %v applyIndex is not up to date %v : %v", rangeID, applyIndex, lastIndex)
				return false, errors.Errorf("replica applyIndex is not up to date")
			}
			for _, s := range status[1:] {
				if applyIndex != s.ApplyIndex || lastIndex != s.LastIndex {
					log.Errorf(ctx, "replica status is different %v", status)
					return false, errors.Errorf("replica status is different")
				}
			}
		}
	}
...
// AdminReplicaVoterStatusConsistent check replicas(non-learner) status consistent
// check the consistency of no leaner replicas
// Used for state consistency verification after range split, in mode 1/3, single node, two nodes,
// and three nodes perform the same, and can also be used normally in scaling scenarios,
// In a highly available scenario, if the node where the replica is located dies,
// it will result in the inability to obtain the replica status and the inability to pass the consistency check.
func (db *DB) AdminReplicaVoterStatusConsistent(
	ctx context.Context, begin, end interface{},
) (bool, error) {
	b := &Batch{}
	b.adminGetReplicaStatus(begin, end)
	...
	for _, s := range resp.ReplicaStatus {
		rangeID := s.RangeID
		// failed replicate status like
		// {range_id, apply_index, last_index, leader, store_id, node_id}
		// {0, 0, 0, false, 0, 0}
		// 3 replicas, 1 replica failed:
		// {55, 15, 15, true, 1, 1}, {55, 15, 15, false, 2, 2}, {0, 0, 0, false, 0, 0}
		if s.Learner {
			continue
		}
		if rangeID != 0 {
	...
	for _, status := range rangeStatusMap {
		//fmt.Println("raftlog status: %v", status)
		// 1 replica:
		// {55, 15, 15, true, 1, 1}
		if len(status) > 1 {
...
// AdminReplicaLeaseStatusConsistent check replicas(lease holder) status consistent.
// Check whether the lease holder and the raft leader are on the same node to ensure
// that the node status after the manual lease transfer is the same in all scenarios.
func (db *DB) AdminReplicaLeaseStatusConsistent(
	ctx context.Context, begin, end interface{}, targetStore roachpb.StoreID,
) (bool, error) {
	b := &Batch{}
	b.adminGetReplicaStatus(begin, end)
	if err := getOneErr(db.Run(ctx, b), b); err != nil {
	...
	for _, s := range resp.ReplicaStatus {
		...
		// 3 replicas, 1 replica failed:
		// {55, 15, 15, true, 1, 1}, {55, 15, 15, false, 2, 2}, {0, 0, 0, false, 0, 0}
		// only need to check the range of startKey, not endKey.
		if rangeID != 0 && s.StartKey.AsRawKey().Equal(startKey) {
			rangeStatusMap[rangeID] = append(rangeStatusMap[rangeID], s)
		}
	}
	...
	for _, status := range rangeStatusMap {
		...
		for _, s := range status {
			if s.StoreID == targetStore {
				if s.Leader {
					break
				} else {
					log.Errorf(ctx, "leaseholder and leader is different:%v", status)
					return false, errors.Errorf("leaseholder and leader is different")
				}
			}
		}
	}
...
func (db *DB) AddSSTable(
	...
	if shouldLogLogicalOp {
		b.Header.ShouldLogLogicalOp = true
	}
	return getOneErr(db.Run(ctx, b), b)
}
