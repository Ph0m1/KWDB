...
// RawRequests returns the BatchRequests.
...
// InitResult initialize the results, create the buffer.
...
// AppendReqs appends requests to the batch.
...
func (b *Batch) adminSplitTs(
	...
	req := &roachpb.AdminSplitForTsRequest{
		RequestHeader: roachpb.RequestHeader{
			Key: spanKey,
		},
		TableId:         tableID,
...
// adminGetReplicaStatus get replicas status
func (b *Batch) adminGetReplicaStatus(s, e interface{}) {
	begin, err := marshalKey(s)
	if err != nil {
		b.InitResult(0, 0, notRaw, err)
		return
