...
// sendTS. The sendTS time replica processes TsRequest, checks the initialization status of RaftGroup and wakes up the leader;
func (r *Replica) sendTS(
	ctx context.Context, ba *roachpb.BatchRequest,
) (*roachpb.BatchResponse, *roachpb.Error) {
	...
	fn := (*Replica).executeWriteTSBatch
	br, pErr = r.executeTSPutBatchWithConcurrencyRetries(ctx, ba, fn)
...
//
// gitee.com/kwbasedb/kwbase/pkg/storage.(*Replica).sendWithRangeID(0xc420d1a000, 0x64bfb80, 0xc421564b10, 0x15, 0x153fd4634aeb0193, 0x0, 0x100000001, 0x1,
// 0x15, 0x0, ...)
...
func (r *Replica) executeTSBatchWithConcurrencyRetries(
	ctx context.Context, ba *roachpb.BatchRequest, fn batchExecutionFn,
) (br *roachpb.BatchResponse, pErr *roachpb.Error) {
	// Try to execute command; exit retry loop on success.
	var g *concurrency.Guard
	...
	for {
		...
		//br, pErr, _ = fn(r, ctx, ba, status, g)
		br, g, pErr = fn(r, ctx, ba, status, g)
		if pErr == nil {
			// Success.
			return br, nil
...
func (r *Replica) executeTSPutBatchWithConcurrencyRetries(
	ctx context.Context, ba *roachpb.BatchRequest, fn batchExecutionFn,
) (br *roachpb.BatchResponse, pErr *roachpb.Error) {
	// Try to execute command; exit retry loop on success.
	var g *concurrency.Guard
	...
	for {
		...
		if latchSpans == nil {
			var err error
			latchSpans, lockSpans, err = r.collectSpansTS(ba)
			if err != nil {
				return nil, roachpb.NewError(err)
			}
...
func (r *Replica) executeAdminBatch(
	...
	case *roachpb.AdminSplitForTsRequest:
		var reply roachpb.AdminSplitResponse
		reply, pErr = r.AdminSplitForTs(ctx, *tArgs, "manual")
		resp = &reply
	...
	case *roachpb.GetReplicaStatusRequest:
		var reply roachpb.GetReplicaStatusResponse
		reply, pErr = r.GetReplicaStatus(ctx, *tArgs)
		resp = &reply
