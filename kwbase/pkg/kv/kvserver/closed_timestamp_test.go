...
func getTableID(db *gosql.DB, dbName, tableName string) (tableID sqlbase.ID, err error) {
	err = db.QueryRow(`SELECT table_id FROM kwdb_internal.tables WHERE database_name = $1 AND name = $2`,
...
`, targetDuration, closeFraction)); err != nil {
	...
	testutils.SucceedsSoon(t, func() error {
		if err := db0.QueryRow(
			`SELECT range_id, start_key, array_length(replicas, 1) FROM kwdb_internal.ranges WHERE table_name = 'kv' AND database_name = 'cttest'`,
	...
	startKey, err := hex.DecodeString(string(startKey[2:]))
	if err != nil {
		t.Fatal(err)
	}
