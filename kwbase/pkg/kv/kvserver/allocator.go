...
func (ae *allocatorError) Error() string {
	...
	if len(ae.constraints) == 0 {
		if len(ae.preDist) != 0 {
			...
			b.WriteString("; must match preDist [")
			for i := range ae.preDist {
				//if i > 0 {
				//	b.WriteByte(' ')
				//}
				b.WriteByte('{')
				b.WriteString("NodeId:")
				b.WriteString(ae.preDist[i].NodeID.String())
				b.WriteByte(',')
				b.WriteString("StoreId:")
				b.WriteString(ae.preDist[i].StoreID.String())
				b.WriteByte('}')
			}
			b.WriteString("]")
			return b.String()
...
// GetNeededReplicasWithPreDist get need replica with pre dist
func GetNeededReplicasWithPreDist(preDistReplicaCount int, clusterNodes int) int {
	numPreDistReplicas := preDistReplicaCount
	need := numPreDistReplicas

	if clusterNodes < need {
		need = clusterNodes
	}

	if need == numPreDistReplicas {
		return need
	}
	if need%2 == 0 {
		need = need - 1
