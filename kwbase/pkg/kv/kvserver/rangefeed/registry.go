...
func (r *registration) validateEvent(event *roachpb.RangeFeedEvent) {
	...
	case *roachpb.RangeFeedValue:
		...
		if t.Value.RawBytes == nil {
			panic(fmt.Sprintf("unexpected empty RangeFeedValue.Value.RawBytes: %v, t %s", t, t.Value.Timestamp.String()))
		}
...
// isSystemMetaKey detects whether the key is a system table.
// We only check system.descriptor now.
func isSystemMetaKey(key roachpb.Key) bool {
	_, tableID, err := keys.DecodeTablePrefix(key)
	if err != nil {
		return false
	}

	// table system.kwdb_*
	if tableID >= keys.KWDBHAInfoID && tableID < keys.MinNonPredefinedUserDescID {
		return true
	}

	var systemTableID = []uint64{keys.DescriptorTableID, keys.UsersTableID, keys.SettingsTableID, keys.RoleMembersTableID, keys.CommentsTableID, keys.NamespaceTableID, keys.RoleOptionsTableID, keys.KWDBTsTableID}
	flag := false
	for _, v := range systemTableID {
		if tableID == v {
			flag = true
			continue
		}
...
func (r *registration) maybeStripEvent(event *roachpb.RangeFeedEvent) *roachpb.RangeFeedEvent {
	...
	case *roachpb.Commit:
		if isSystemMetaKey(r.span.Key) {
			t = copyOnWrite().(*roachpb.Commit)
			t.SystemSpan = true
		}
	case *roachpb.Abort:
	case *roachpb.InitEOF:
	default:
		panic(fmt.Sprintf("unexpected RangeFeedEvent variant: %v", t))
	}
	return ret
...
func (reg *registry) GetAllSpanSlice() []interval.Range {
	ret := make([]interval.Range, 0)
	matchFn := func(i interval.Interface) (done bool) {
		r := i.(*registration)
		ret = append(ret, r.keys)
...
// GetAll get all the registered spans and merge overlapped spans.
func (reg *registry) GetAll() []interval.Range {
	ret := make([]interval.Range, 0)
	matchFn := func(i interval.Interface) (done bool) {
		r := i.(*registration)
		ret = append(ret, r.keys)
	...
	sort.Slice(ret, func(i, j int) bool {
		if ret[i].Start.Compare(ret[j].Start) < 0 {
			return true
		}
		return false
	})
	ret2 := make([]interval.Range, 0)
	for i := 0; i < len(ret); i++ {
		if len(ret2) == 0 {
			ret2 = append(ret2, ret[i])
		} else {
			if ret[i].Start.Compare(ret2[len(ret2)-1].End) <= 0 {
				if ret[i].End.Compare(ret2[len(ret2)-1].End) > 0 {
					ret2[len(ret2)-1].End = ret[i].End
				}
			} else {
				ret2 = append(ret2, ret[i])
			}
...
func (reg *registry) PublishToOverlapping(span roachpb.Span, event *roachpb.RangeFeedEvent) {
	...
	case *roachpb.Commit, *roachpb.Abort, *roachpb.InitEOF:
	default:
		panic(fmt.Sprintf("unexpected RangeFeedEvent variant: %v", t))
	}
