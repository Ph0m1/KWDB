...
func TestProcessorSlowConsumer(t *testing.T) {
	...
	p.syncEventAndRegistrations() // syc all
	require.Equal(t, 2, p.Len())
	require.Equal(t,
		[]*roachpb.RangeFeedEvent{rangeFeedCheckpoint(
			roachpb.Span{Key: roachpb.Key("a"), EndKey: roachpb.Key("m")},
	...
	toFill := testProcessorEventCCap + 1 // 16
	for i := 0; i < toFill; i++ {
	...
	testutils.SucceedsSoon(t, func() error {
		if act, exp := p.Len(), 2; exp != act { // 2 registrations
...
func TestProcessorInitializeResolvedTimestamp(t *testing.T) {
	defer leaktest.AfterTest(t)()
	t.Skip("Initialize, the catch-up no longer requires testing")
