...
import (
	...
	_ "gitee.com/kwbasedb/kwbase/pkg/keys" // hook up pretty printer
...
func TestRegistryBasic(t *testing.T) {
	...
	if evs := rAB.Events(); len(evs) == 2 {
		require.Equal(t, []*roachpb.RangeFeedEvent{noPrev(ev1), noPrev(ev4)}, evs)
	}
	if evs := rBC.Events(); len(evs) == 2 {
		require.Equal(t, []*roachpb.RangeFeedEvent{ev2, ev4}, evs)
	}
	if evs := rCD.Events(); len(evs) == 1 {
		require.Equal(t, []*roachpb.RangeFeedEvent{ev3}, evs)
	}
	if evs := rAC.Events(); len(evs) == 3 {
		require.Equal(t, []*roachpb.RangeFeedEvent{noPrev(ev1), noPrev(ev2), noPrev(ev4)}, evs)
	}
	...
	require.NoError(t, reg.waitForCaughtUp(all))
	if evs := rAB.Events(); len(evs) == 2 {
		require.Equal(t, []*roachpb.RangeFeedEvent{noPrev(ev4), noPrev(ev1)}, evs)
	}
...
func TestRegistryPublishBeneathStartTimestamp(t *testing.T) {
	defer leaktest.AfterTest(t)()
	t.Skip("TODO(liwei): because, at split, merge, kv is bufferd, after second registry sucsess, bufferd kv is send.")
	reg := makeRegistry()
