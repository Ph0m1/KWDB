...
import (
	...
	"gitee.com/kwbasedb/kwbase/pkg/kv/kvserver/idalloc"
	...
	"gitee.com/kwbasedb/kwbase/pkg/kv/kvserver/txnrecovery"
...
var storeSchedulerConcurrency = envutil.EnvOrDefaultInt(
	...
	"KWBASE_SCHEDULER_CONCURRENCY", min(8*runtime.NumCPU(), 96))

var logSSTInfoTicks = envutil.EnvOrDefaultInt(
	"KWBASE_LOG_SST_INFO_TICKS_INTERVAL", 60)
...
func (sc *StoreConfig) SetDefaults() {
	...
	if sc.concurrentSnapshotApplyLimit == 0 {
		...
		sc.concurrentSnapshotApplyLimit =
			envutil.EnvOrDefaultInt("KWBASE_CONCURRENT_SNAPSHOT_APPLY_LIMIT", 1)
	}
...
func (s *Store) SetDraining(drain bool, reporter func(int, string)) {
	...
	if err := contextutil.RunWithTimeout(ctx, "wait for raft leadership transfer", transferTimeout,
		func(ctx context.Context) error {
			...
			var err error
			// Avoid retry.ForDuration because of https://gitee.com/kwbasedb/kwbase/issues/25091.
...
func (s *Store) Start(
	...
	if envutil.EnvOrDefaultBool("KWBASE_ENABLE_COMPACTOR", true) {
