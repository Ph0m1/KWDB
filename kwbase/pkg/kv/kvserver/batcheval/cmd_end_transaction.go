	...
	if !et.IsParallelCommit() {
		...
		if et.InternalCommitTrigger != nil {
			if st := et.InternalCommitTrigger.SplitTrigger; st != nil {
				...
				// https://gitee.com/kwbasedb/kwbase/issues/14881)
...
func EndTxn(
	...
	var logicalOp storage.MVCCLogicalOpType
	...
	if ok, err := storage.MVCCGetProto(
		...
		if h.Txn.BatchWriteKeyCount != 0 {
			h.Txn.WriteKeyCount += h.Txn.BatchWriteKeyCount
			h.Txn.BatchWriteKeyCount = 0
		}
		if reply.Txn.BatchWriteKeyCount != 0 {
			reply.Txn.WriteKeyCount += reply.Txn.BatchWriteKeyCount
			reply.Txn.BatchWriteKeyCount = 0
		}
		...
		if h.Txn.BatchWriteKeyCount != 0 {
			reply.Txn.WriteKeyCount += h.Txn.BatchWriteKeyCount
			h.Txn.WriteKeyCount += h.Txn.BatchWriteKeyCount
			h.Txn.BatchWriteKeyCount = 0
		}
		if reply.Txn.BatchWriteKeyCount != 0 {
		...
		if h.Txn.WriteKeyCount < reply.Txn.WriteKeyCount {
			h.Txn.WriteKeyCount = reply.Txn.WriteKeyCount
		}
		reply.Txn.WriteKeyCount = h.Txn.WriteKeyCount
		...
		case roachpb.COMMITTED:
			...
			logicalOp = storage.MVCCCommitTxnOpType
			logicalOpDetails := storage.MVCCLogicalOpDetails{
				Txn: reply.Txn.TxnMeta,
			}
			readWriter.LogLogicalOp(logicalOp, logicalOpDetails)
			log.VEventf(ctx, 3, "endxevt committed %s, %d, %d, %d, %d", reply.Txn.ID, h.Txn.WriteKeyCount, h.Txn.BatchWriteKeyCount, reply.Txn.WriteKeyCount, reply.Txn.BatchWriteKeyCount)
		...
		case roachpb.ABORTED:
			if !args.Commit {
				...
				logicalOp = storage.MVCCAbortOpType
				logicalOpDetails := storage.MVCCLogicalOpDetails{
					Txn: reply.Txn.TxnMeta,
				}
				readWriter.LogLogicalOp(logicalOp, logicalOpDetails)
				log.VEventf(ctx, 3, "endxevt aborted %s, %d, %d, %d, %d", reply.Txn.ID, h.Txn.WriteKeyCount, h.Txn.BatchWriteKeyCount, reply.Txn.WriteKeyCount, reply.Txn.BatchWriteKeyCount)
				return res, nil
			}
	...
	if args.Commit {
		if retry, reason, extraMsg := IsEndTxnTriggeringRetryError(reply.Txn, args); retry {
			...
			readWriter.LogLogicalOp(logicalOp, logicalOpDetails)
			log.VEventf(ctx, 3, "endxevt retry %s, %d, %d, %d, %d", reply.Txn.ID, h.Txn.WriteKeyCount, h.Txn.BatchWriteKeyCount, reply.Txn.WriteKeyCount, reply.Txn.BatchWriteKeyCount)
	...
	readWriter.LogLogicalOp(logicalOp, logicalOpDetails)
	log.VEventf(ctx, 3, "endxevt final %s, %d, %d, %d, %d", reply.Txn.ID, h.Txn.WriteKeyCount, h.Txn.BatchWriteKeyCount, reply.Txn.WriteKeyCount, reply.Txn.BatchWriteKeyCount)
	return pd, nil
}
