...
func runTestDBAddSSTable(ctx context.Context, t *testing.T, db *kv.DB, store *kvserver.Store) {
	{
		...
		if err := db.AddSSTable(
			ctx, "d", "e", data, false /* disallowShadowing */, nil /* stats */, false /* ingestAsWrites */, false, /* shouldLogLogicalOp */
		...
		if err := db.AddSSTable(
			ctx, "a", "b", data, false /* disallowShadowing */, nil /* stats */, false /* ingestAsWrites */, false, /* shouldLogLogicalOp */
		); !testutils.IsError(err, "not in request range") {
			t.Fatalf("expected request range error got: %+v", err)
		}
		...
		defer cancel()
		if err := db.AddSSTable(
			ingestCtx, "b", "c", data, false /* disallowShadowing */, nil /* stats */, false /* ingestAsWrites */, false, /* shouldLogLogicalOp */
		); err != nil {
			t.Fatalf("%+v", err)
		}
	...
	// the value returned by Get.
	{
		...
		if err := db.AddSSTable(
			ctx, "b", "c", data, false /* disallowShadowing */, nil /* stats */, false /* ingestAsWrites */, false, /* shouldLogLogicalOp */
		); err != nil {
			t.Fatalf("%+v", err)
		}
		if r, err := db.Get(ctx, "bb"); err != nil {
	...
	// ... and doing the same thing but via write-batch works the same.
	{
		...
		for i := 0; i < 2; i++ {
			...
			if err := db.AddSSTable(
				ingestCtx, "b", "c", data, false /* disallowShadowing */, nil /* stats */, true /* ingestAsWrites */, false, /* shouldLogLogicalOp */
			); err != nil {
				t.Fatalf("%+v", err)
			}
			if err := testutils.MatchInOrder(collect().String(),
