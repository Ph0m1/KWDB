	...
	if removeIdx >= 0 {
		...
		for i, r := range liveVoterReplicas {
			if r.ReplicaID == replToRemove.ReplicaID {
				// Remove the dead Store replicas or the retired Store replicas from remainingLiveReplicas
		...
		// leaseholder.
		// If the Store where the repl is located is a dead Store or a retired
		// Store, transfer the leaseHolder
	...
	if !rq.store.TestingKnobs().DisableReplicaRebalancing {
		...
		if !ok {
			...
			if len(existingReplicas) == 1 {
				...
				//
				// See https://gitee.com/kwbasedb/kwbase/issues/40333.
				chgs = chgs[:1]
