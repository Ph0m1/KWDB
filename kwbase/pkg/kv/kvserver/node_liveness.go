...
func (nl *NodeLiveness) SetTSNodeLiveness(
	ctx context.Context, nodeID roachpb.NodeID, complete bool,
) {
	log.Infof(ctx, "set node %v to %v", nodeID.String(), complete)
	ctx = nl.ambientCtx.AnnotateCtx(ctx)
	for r := retry.StartWithCtx(ctx, base.DefaultRetryOptions()); r.Next(); {
		...
		err = nl.setTsStatusInternal(ctx, nodeID, liveness, complete)
		if err != nil {
			log.Warningf(ctx, "attempting to set liveness status to %v failed: %v", complete, err)
			continue
		}
		return
	}
...
func (nl *NodeLiveness) SetUpgrading(
	...
	nl.SetTSNodeLiveness(ctx, nodeID, false)
...
func (nl *NodeLiveness) setTsStatusInternal(
	ctx context.Context, nodeID roachpb.NodeID, liveness storagepb.Liveness, complete bool,
) error {
	sem := nl.sem(nodeID)
	...
	if err := nl.updateLiveness(ctx, update, liveness, func(actual storagepb.Liveness) error {
		nl.maybeUpdate(actual)
		if actual.Upgrading == update.Upgrading {
