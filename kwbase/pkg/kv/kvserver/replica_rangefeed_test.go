...
func TestReplicaRangefeed(t *testing.T) {
	...
	checkForExpEvents := func(expEvents []*roachpb.RangeFeedEvent) {
		...
		for i, stream := range streams {
			...
			for idx, event := range events {
				if event.Val != nil {
					event.Val.TxnID = uuid.Nil
				}
				if event.Commit != nil {
					event.Commit.TxnID = uuid.Nil
					event.Commit.Timestamp = hlc.Timestamp{}
				}
				events[idx] = event
			}
	...
	expEvents = append(expEvents, []*roachpb.RangeFeedEvent{
		{Val: &roachpb.RangeFeedValue{
			Key: roachpb.Key("c"), Value: expVal2, RangeID: int64(rangeID), NoTxn: true,
		}},
		{Val: &roachpb.RangeFeedValue{
			Key: roachpb.Key("m"), Value: expVal3, RangeID: int64(rangeID), NoTxn: false,
		}},
		{Commit: &roachpb.Commit{
			WriteKeyCount: 1, Timestamp: hlc.Timestamp{},
		}},
		// TODO: committed send multi commitEvent.
		//       Replication code, remove in the future.
		{Commit: &roachpb.Commit{
			WriteKeyCount: 1, Timestamp: hlc.Timestamp{},
		}},
		{Val: &roachpb.RangeFeedValue{
			Key: roachpb.Key("b"), Value: expVal4, PrevValue: expVal1NoTS, RangeID: int64(rangeID), NoTxn: true,
		}},
		{Val: &roachpb.RangeFeedValue{
			Key: roachpb.Key("b"), Value: expVal5, RangeID: int64(rangeID), NoTxn: false,
		}},
		{Commit: &roachpb.Commit{
			WriteKeyCount: 1, Timestamp: hlc.Timestamp{},
		}},
...
func TestReplicaRangefeedPushesTransactions(t *testing.T) {
	...
	t.Skip("TODO(baozhixiao): fix this test.")
