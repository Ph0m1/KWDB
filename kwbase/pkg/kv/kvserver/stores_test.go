...
func TestStoresClusterVersionIncompatible(t *testing.T) {
	...
	for name, setter := range map[string]testFn{
		"StoreTooNew": func(cv *clusterversion.ClusterVersion, ls *Stores) string {
			...
			return `kwbase version v1\.0-1 is incompatible with data in store <no-attributes>=<in-mem>; use version v9\.0 or later`
		},
		"StoreTooOldVersion": func(cv *clusterversion.ClusterVersion, ls *Stores) string {
			...
			return `store <no-attributes>=<in-mem>, last used with kwbase version v4\.0, is too old for running version v9\.0 \(which requires data from v5\.0 or later\)`
		},
