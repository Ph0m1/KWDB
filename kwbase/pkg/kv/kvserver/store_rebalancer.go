...
func (sr *StoreRebalancer) rebalanceStore(
	...
	// Exclude time-series replicas from the replicas participating in
	// the sorting to avoid rebalance in the background
	hottestRangesWithTS := sr.replRankings.topQPS()
	var hottestRanges []replicaWithStats
	// Select non TS replicas from all replicas for rebalancer
	for _, rep := range hottestRangesWithTS {
		if !rep.repl.isTs() {
			hottestRanges = append(hottestRanges, rep)
		}
	}
