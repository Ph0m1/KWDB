	...
	for {
		...
		case <-slowTimer.C:
			...
			if r.store.cfg.NodeLiveness != nil {
				...
				log.Error(ctx, "NodeLiveness not init")
			}
...
// executeWriteTSBatch checks the initialization status of RaftGroup and wakes up
// the leader, checks the legitimacy of batch execution in the replica, and generates
// a proposal
func (r *Replica) executeWriteTSBatch(
	ctx context.Context, ba *roachpb.BatchRequest, st storagepb.LeaseStatus, g *concurrency.Guard,
) (br *roachpb.BatchResponse, _ *concurrency.Guard, pErr *roachpb.Error) {
	startTime := timeutil.Now()
	...
	// the concurrency guard will be assumed by Raft, so provide the guard to
	// evalAndPropose.
	ch, abandon, maxLeaseIndex, pErr := r.evalAndProposeTS(ctx, ba, g, &st.Lease)
	if pErr != nil {
		r.readOnlyCmdMu.RUnlock()
		if cErr, ok := pErr.GetDetail().(*roachpb.ReplicaCorruptionError); ok {
			r.raftMu.Lock()
...
func (r *Replica) evaluateWriteBatchWrapper(
	...
	var res result.Result
	var pErr *roachpb.Error
	// Process the TsRequest of the time series copy and cancel the pre-write operation
	if r.isTs() && r.isTsRequest(ctx, ba) {
		br, res, pErr = evaluateTSBatch(ctx, idKey, batch, rec, ms, ba, false /* readOnly */)
	} else {
