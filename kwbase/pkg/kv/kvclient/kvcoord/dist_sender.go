...
func (ds *DistSender) Send(
	...
	for len(parts) > 0 {
		...
		if rpl != nil && ba.Txn != nil && rpl.Txn != nil {
			if ba.Txn.BatchWriteKeyCount < rpl.Txn.BatchWriteKeyCount {
				ba.Txn.BatchWriteKeyCount = rpl.Txn.BatchWriteKeyCount
			}
		}
	...
	if len(rplChunks) > 0 {
		...
		for _, rpl := range rplChunks[1:] {
			...
			if reply.Txn != nil && rpl.Txn != nil {
				reply.Txn.BatchWriteKeyCount += rpl.Txn.BatchWriteKeyCount
			}
		}
