...
// ShowJobsUpdate for show jobs update.
func (j *Job) ShowJobsUpdate(
	ctx context.Context,
	txn *kv.Txn,
	jobid int64,
	jobstatus int32,
	...
	//if err := p.extendedEvalCtx.ExecCfg.DB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
	var payload *jobspb.Payload
	var progress *jobspb.Progress
	...
	if payload, err = UnmarshalPayload(row[0]); err != nil {
		return err
	}
	if progress, err = UnmarshalProgress(row[1]); err != nil {
		return err
	}
	...
	if jobstatus == 0 {
		status = "succeeded"
		progress.RunningStatus = string(StatusSucceeded)
		payload.FinishedMicros = finishTime
	} else if jobstatus == 1 {
		status = "failed"
		payload.ErrorMicros = errorTime
		payload.Error = errorInfo
		progress.RunningStatus = string(StatusFailed)
	}
	...
	if err == nil {
		...
		_, err = j.registry.ex.Exec(ctx, "job-update", txn, updateStmt, status, payloadBytes, progressBytes, jobid)
	}
	if err != nil {
		return err
	}
...
// ShowJobsUpdateFailed for show jobs update.
func (j *Job) ShowJobsUpdateFailed(
	ctx context.Context, txn *kv.Txn, jobid int64, errorTime int64, errorInfo string,
) error {
	//if err := p.extendedEvalCtx.ExecCfg.DB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
	var payload *jobspb.Payload
	var progress *jobspb.Progress
	const selectStmt = "SELECT payload, progress FROM system.jobs WHERE id = $1 and status != $2 ORDER BY created asc LIMIT 1"
	row, err := j.registry.ex.QueryRowEx(
		ctx, "log-job", txn, sqlbase.InternalExecutorSessionDataOverride{User: security.RootUser},
		selectStmt, jobid, StatusFailed)
	if err != nil {
		return err
	}
	if row == nil {
	...
	if err == nil {
		...
		_, err = j.registry.ex.Exec(ctx, "job-insert", txn, stmt, jobid, StatusFailed, payloadBytes, progressBytes)
	}
