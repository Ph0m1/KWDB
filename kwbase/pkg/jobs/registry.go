...
type Registry struct {
	...
	DeleteLifeTimeFinish bool
	DeleteInterval       time.Duration
	CompressFuncFinish   bool
...
type planHookMaker func(opName string, user string) (interface{}, func())
...
// MakeJobID generates and returns a new job id.
...
func (r *Registry) Run(ctx context.Context, ex sqlutil.InternalExecutor, jobs []int64) error {
	...
	// populate the kwdb_internal.jobs vtable.
	query := fmt.Sprintf(
...
// RegisterRepl register repl progress id
func (r *Registry) RegisterRepl(replID int64, cancel context.CancelFunc) error {
	if err := r.register(replID, cancel); err != nil {
		return errors.Wrapf(err, "RegisterRepl failed.")
	}
	return nil
}

// UnregisterRepl unregister repl progress id
func (r *Registry) UnregisterRepl(replID int64) {
	r.unregister(replID)
}
...
func (r *Registry) UpdateJobWithTxn(ctx context.Context, jobID int64, updateFunc UpdateFn) error {
	j := &Job{
		id:       &jobID,
		registry: r,
	}
	return j.Update(ctx, updateFunc)
}
...
var maxAdoptionsPerLoop = envutil.EnvOrDefaultInt(`KWBASE_JOB_ADOPTIONS_PER_PERIOD`, 10)
	...
	for _, id := range payload.DescriptorIDs {
		...
		if err := r.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
			...
			if errors.Is(err, sqlbase.ErrDescriptorNotFound) {
				...
				// job payload. See
				// https://gitee.com/kwbasedb/kwbase/45399.
				continue
			}
			return false, err
		}
