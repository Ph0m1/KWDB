...
import (
	...
	"gitee.com/kwbasedb/kwbase/pkg/util/cloudinfo"
...
const baseUpdatesURL = `https://register.kwbasedb.com/api/clusters/updates`
const baseReportingURL = `https://register.kwbasedb.com/api/clusters/report`
...
func init() {
	...
	updatesURL, err = url.Parse(
		envutil.EnvOrDefaultString("KWBASE_UPDATE_CHECK_URL", baseUpdatesURL),
	)
	if err != nil {
		panic(err)
	}
	reportingURL, err = url.Parse(
		envutil.EnvOrDefaultString("KWBASE_USAGE_REPORT_URL", baseReportingURL),
	)
	if err != nil {
		panic(err)
	}
