...
import (
	...
	"gitee.com/kwbasedb/kwbase/pkg/server/diagnosticspb"
...
func TestReportUsage(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		if _, err := db.Exec(`SELECT kwdb_internal.force_error('blah', $1)`, elemName); !testutils.IsError(
			err, elemName,
		) {
			t.Fatal(err)
		}
		if _, err := db.Exec(`SELECT kwdb_internal.force_error('', $1)`, elemName); !testutils.IsError(
			err, elemName,
		) {
			t.Fatal(err)
		}
		if _, err := db.Exec(`SELECT kwdb_internal.set_vmodule('invalid')`); !testutils.IsError(
		...
		//	t.Fatal(err)
		//}
		if _, err := db.Exec(`SELECT kwdb_internal.force_assertion_error('woo')`); !testutils.IsError(
			err, "internal error",
		) {
			t.Fatal(err)
		}
	...
	testutils.SucceedsSoon(t, func() error {
		...
		// Phytium,FT-2000+/64 not support cpu MHz, skip it.
		if r.last.Node.Hardware.Cpu.Mhz == 0.0 && r.last.Node.Hardware.Cpu.Model != "Phytium,FT-2000+/64" {
	...
	expectedFeatureUsage := map[string]int32{
		...
		"sql.plan.builtins.kwdb_internal.force_assertion_error(msg: string) -> int": 1,
	...
	expectedKeys := []string{
		...
		`[opt,nodist,failed] SELECT kwdb_internal.force_assertion_error(_)`,
		`[opt,nodist,failed] SELECT kwdb_internal.force_error(_, $1)`,
		`[opt,nodist,failed] SELECT kwdb_internal.set_vmodule(_)`,
	...
	for appName, expectedStatements := range map[string][]string{
		"": {
			...
			//`SELECT _ / $1`,
			//`SELECT _ / _`,
			`SELECT kwdb_internal.force_assertion_error(_)`,
			`SELECT kwdb_internal.force_error(_, $1)`,
			`SELECT kwdb_internal.set_vmodule(_)`,
