...
func TestSettingsSetAndShow(t *testing.T) {
	...
	if _, err := testuser.Exec(`SELECT * FROM kwdb_internal.cluster_settings`); !testutils.IsError(err,
		`only users with the admin role are allowed to read kwdb_internal.cluster_settings`,
	) {
		t.Fatal(err)
	}
}
