...
func TestVerifyPassword(t *testing.T) {
	...
	for _, user := range []struct {
		...
		{"kwbase", "12345", "", "VALID UNTIL '2100-01-01'", nil},
...
func TestAuthenticationAPIUserLogin(t *testing.T) {
	...
	var validVerifycode = fmt.Sprintf("true/%d", timestamp)
	cmd := fmt.Sprintf("CREATE USER %s WITH PASSWORD '%s'", validUsername, validPassword)
	if _, err := db.Exec(cmd); err != nil {
		t.Fatalf("failed to create user: %s", err)
	}

	tryLogin := func(username, password string, verify string) (*http.Response, error) {
	...
	{
		response, err := tryLogin(validUsername, "wrongpassword", validVerifycode)
	...
	response, err := tryLogin(validUsername, validPassword, validVerifycode)
	if err != nil {
