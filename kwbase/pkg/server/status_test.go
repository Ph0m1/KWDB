...
func TestHealthTelemetry(t *testing.T) {
	...
	rows, err := db.Query("SELECT * FROM kwdb_internal.feature_usage WHERE feature_name LIKE 'monitoring%' AND usage_count > 0;")
	defer func() {
	...
	rows2, err := db.Query("SELECT feature_name, usage_count FROM kwdb_internal.feature_usage WHERE feature_name LIKE 'monitoring%' AND usage_count > 0;")
	defer func() {
...
func TestChartCatalogGen(t *testing.T) {
	...
	// Ensure each of the 10 constant sections of the chart catalog exist.
	if len(chartCatalog) != 10 {
...
// endpoint for txns and the special kwbase_restart savepoint are correct.
...
func TestRemoteDebugModeSetting(t *testing.T) {
	...
	testutils.SucceedsSoon(t, func() error {
		for _, tc := range []struct {
			...
			{"logfiles/local/kwbase.log", &serverpb.LogEntriesResponse{}},
