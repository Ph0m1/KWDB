...
func TestAdminAPIDatabases(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{[]string{"defaultdb", "postgres", "system", testdb}, false},
	} {
...
func getSystemJobIDs(t testing.TB, db *sqlutils.SQLRunner) []int64 {
	rows := db.Query(t, `SELECT job_id FROM kwdb_internal.jobs ORDER BY created DESC;`)
	defer rows.Close()
...
func TestAdminAPIQueryPlan(t *testing.T) {
	...
	for i, testCase := range testCases {
		...
		res.DistSQLPhysicalQueryPlan = strings.Replace(res.DistSQLPhysicalQueryPlan, "\n\n", "", -1)
		res.DistSQLPhysicalQueryPlan = strings.Replace(res.DistSQLPhysicalQueryPlan, " ", "", -1)
...
// KWDB's endpoints works as expected by recording the call counts of `Admin` &
// `Status` requests.
func TestEndpointTelemetryBasic(t *testing.T) {
	...
	require.GreaterOrEqual(t, telemetry.Read(getServerEndpointCounter(
		"/kwbase.server.serverpb.Admin/Locations",
	)), int32(1))
	...
	require.Equal(t, int32(1), telemetry.Read(getServerEndpointCounter(
		"/kwbase.server.serverpb.Status/Statements",
	)))
}
