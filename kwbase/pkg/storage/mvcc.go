...
func (opts *MVCCGetOptions) validate() error {
	if opts.Inconsistent && opts.Txn != nil && !opts.Txn.UnConsistency {
...
func mvccGetInternal(
	...
	if !consistent && txn != nil && !txn.UnConsistency {
...
func mvccPutInternal(
	...
	var metaKeySize, metaValSize int64
	if newMeta.Txn != nil {
		if maybeTooOldErr == nil {
			if !(strings.Contains(roachpb.KeyValue{Key: key}.Key.String(), "/Table/13/") ||
				strings.Contains(roachpb.KeyValue{Key: key}.Key.String(), "/Meta2/") ||
				strings.Contains(roachpb.KeyValue{Key: key}.Key.String(), "/Local/Range/")) {
				newMeta.Txn.BatchWriteKeyCount++
			}
		}
	...
	if maybeTooOldErr == nil {
		...
		if logicalOp == MVCCWriteIntentOpType || logicalOp == MVCCUpdateIntentOpType {
			logicalOpDetails.Value = value
		}
...
func MVCCDeleteRange(
	...
	res, err := MVCCScan(
		ctx, rw, key, endKey, timestamp, MVCCScanOptions{Txn: scanTxn, MaxKeys: max})
	if err != nil {
...
func computeCapacity(
	...
	if maxSizeBytes == 0 || maxSizeBytes >= fsuTotal || path == "" {
		return roachpb.StoreCapacity{
			...
			Available:      fsuAvail,
			Used:           totalUsedBytes,
			TsdbUsed:       tsdbUsedBytes,
			RelationalUsed: relationalUsedBytes,
		}, nil
	}
