...
// MVCCKeyCompare compares kwbase keys, including the MVCC timestamps.
...
func (t *pebbleTimeBoundPropCollector) Finish(userProps map[string]string) error {
	...
	userProps["kwdb.ts.min"] = string(t.min)
	userProps["kwdb.ts.max"] = string(t.max)
	return nil
}
...
func NewPebble(ctx context.Context, cfg PebbleConfig) (*Pebble, error) {
	...
	if cfg.Dir == "" {
		...
		// behavior.
		var err error
		auxDir, err = ioutil.TempDir(os.TempDir(), "kwbase-auxiliary")
		if err != nil {
			return nil, err
		}
	} else {
	...
	db, err := NewPebble(
		...
		PebbleConfig{
			StorageConfig: base.StorageConfig{
				...
				// https://gitee.com/kwbasedb/kwbase/issues/16750
