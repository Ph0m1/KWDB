...
//TODO(mike) ：The replication of this case cannot pass before it is submitted, and replication skips the repair of this case
...
`
	helpExpected := fmt.Sprintf("KwDB command-line interface and server.\n\n%s", expUsage)
	...
	for _, test := range testCases {
		t.Run(strings.Join(test.flags, ","), func(t *testing.T) {
			...
			kwbaseCmd.SetOutput(w)
			defer kwbaseCmd.SetOutput(nil)
...
func TestGenAutocomplete(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{shell: "bash"},
		{shell: "zsh"},
		{shell: "bad", expErr: `invalid argument "bad" for "kwbase gen autocomplete"`},
	} {
