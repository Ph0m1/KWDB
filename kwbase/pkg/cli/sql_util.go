...
// These are translated to a message "perhaps this is not a KwDB node"
// at the top level.
...
func (c *sqlConn) getServerMetadata() (
	...
	rows, err := c.Query("SELECT * FROM kwdb_internal.node_build_info", nil)
	...
	if version == "" {
		...
		c.serverBuild = fmt.Sprintf("KwDB %s %s (%s, built %s, %s)",
			v10fields[0], version, v10fields[2], v10fields[3], v10fields[4])
	}
	return nodeID, version, clusterID, nil
}

// getDefaultTransactionReadOnly can get node read-only status if  read-only = true print The node status is read-only now to user
// if is not read-only do not print
// if has err print the warning to user
func (c *sqlConn) getDefaultTransactionReadOnly() {
	// Retrieve the node ID and server build info.
	row, err := c.QueryRow("SHOW CLUSTER SETTING default_transaction_read_only.enabled", nil)
	if row == nil {
		return
	}
	readOnly := row[0].(bool)
	if err != nil {
		fmt.Fprintf(stderr, "warning: unable to get read-only mode: %s\n", err)
	}
	if readOnly {
		fmt.Println("# The node status is read-only now")
	}
...
// sqlTxnShim implements the kwdb.Tx interface.
//
// It exists to support kwdb.ExecuteInTxn. Normally, we'd hand kwdb.ExecuteInTxn
...
func (c *sqlConn) ExecTxn(fn func(*sqlConn) error) (err error) {
	...
	return kwdb.ExecuteInTx(context.TODO(), sqlTxnShim{c}, func() error {
		return fn(c)
	})
}
...
var sqlConnTimeout = envutil.EnvOrDefaultString("KWBASE_CONNECT_TIMEOUT", "15")
...
// checkAppendEOF check if write eof on execute finished
func checkAppendEOF(w io.Writer) {
	if len(sqlCtx.interactEOF) > 0 {
		// output eof line after no more ResultSet
		fmt.Fprintf(w, "%s\n", sqlCtx.interactEOF)
	}
}
...
func runQueryAndFormatResults(conn *sqlConn, w io.Writer, fn queryFunc) error {
	...
	for {
		...
		if sqlCtx.showTimes {
			...
			// the 1st result; this is subject to change once KwDB
