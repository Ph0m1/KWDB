...
#
`
	helpMessageFmt = `You are using 'kwbase sql', KwDB's lightweight SQL client.
Type:
...
var sqlShellCmd = &cobra.Command{
	...
	Long: `
Open a sql shell running against a kwbase database.
`,
	Args: cobra.NoArgs,
...
var options = map[string]struct {
	...
	`interact_eof`: {
		description:               "display the output complete after all query",
		isBoolean:                 false,
		validDuringMultilineEntry: true,
		set:                       func(_ *cliState, val string) error { sqlCtx.interactEOF = val; return nil },
		reset:                     func(_ *cliState) error { sqlCtx.interactEOF = ""; return nil },
		display:                   func(_ *cliState) string { return sqlCtx.interactEOF },
	},
}
...
// This can only be done from `kwbase demo`.
func (c *cliState) handleDemo(cmd []string, nextState, errState cliStateEnum) cliStateEnum {
	// A transient cluster signifies the presence of `kwbase demo`.
	if demoCtx.transientCluster == nil {
		return c.invalidSyntax(errState, `\demo can only be run with kwbase demo`)
	}
...
// https://gitee.com/kwbasedb/kwbase/issues/46074
...
var cmdHistFile = envutil.EnvOrDefaultString("KWBASE_SQL_CLI_HISTORY", ".kwbasesql_history")
...
func (c *cliState) configurePreShellDefaults() (cleanupFn func(), err error) {
	...
	if cliCtx.isInteractive && cliCtx.terminalOutput {
		...
		c.ins, c.exitErr = readline.InitFiles("kwbase",
			true, /* wideChars */
...
func checkInteractive() {
	...
	// serious hunch they are. For now, only `kwbase sql` *without*
...
func runTerm(cmd *cobra.Command, args []string) error {
	...
	conn, err := makeSQLClient("kwbase sql", useDefaultDb)
	if err != nil {
		return err
	}
	defer conn.Close()
