...
start_test "Check that temp directory does not get wiped upon subsequent failed kwbase start attempt and that a kwbase instance can be subsequently started up after a shutdown"
send "$argv start-single-node --insecure --store=$storedir --background\r"
eexpect ":/# "
# Try to start up a second kwbase instance with the same store path.
send "$argv start-single-node --insecure --store=$storedir\r"
...
glob_exists "$storedir/$tempprefix*"
send "pkill -9 kwbase\r"
# We should be able to start the kwbase instance again.
send "$argv start-single-node --insecure --store=$storedir\r"
eexpect "node starting"
interrupt
eexpect "shutdown completed"
