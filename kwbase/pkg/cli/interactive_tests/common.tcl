...
set histfile "kwbase_sql_history"

set ::env(KWBASE_SKIP_ENABLING_DIAGNOSTIC_REPORTING) "true"
set ::env(KWBASE_CONNECT_TIMEOUT) 15
set ::env(KWBASE_SQL_CLI_HISTORY) $histfile
# Set client commands as insecure. The server uses --insecure.
set ::env(KWBASE_INSECURE) "true"
...
proc report {text} {
    ...
    # Also KwDB doesn't honor umask anyway.
    # So we simply come after the fact and adjust the permissions.
    #
    # The find may race with a kwbase process shutting down in the
    # background; kwbase might be deleting files as they are being
...
proc flush_server_logs {} {
    ...
    system "for i in `seq 1 3`; do
              grep 'hangup received, flushing logs' logs/db/logs/kwbase.log && exit 0;
              echo still waiting
              sleep 1
            done
