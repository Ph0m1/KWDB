...
# that is not a KwDB server.
set spawn_id $shell_spawn_id
interrupt
interrupt
eexpect ":/# "

start_test "Connecting an insecure RPC client to a non-KwDB server"
# In one shell, start a bogus server
send "python2 $fakeserv\r"
eexpect "ready"
# In the other shell, try to run kwbase quit
set spawn_id $client_spawn_id
send "$argv quit --insecure\r"
eexpect "insecure\r\n"
...
interrupt
eexpect ":/# "
# Check that kwbase quit becomes suitably confused.
set spawn_id $client_spawn_id
eexpect "ERROR: server closed the connection."
eexpect "Is this a KwDB node?"
eexpect ":/# "
set spawn_id $shell_spawn_id
end_test

start_test "Connecting an insecure SQL client to a non-KwDB server"
# In one shell, start a bogus server
send "python2 $fakeserv\r"
eexpect "ready"
# In the other shell, try to run kwbase sql
set spawn_id $client_spawn_id
send "$argv sql -e 'select 1' --insecure\r"
eexpect "insecure\r\n"
...
interrupt
eexpect ":/# "
# Check that kwbase sql becomes suitably confused.
set spawn_id $client_spawn_id
eexpect "ERROR: server closed the connection."
eexpect "Is this a KwDB node?"
eexpect "EOF"
