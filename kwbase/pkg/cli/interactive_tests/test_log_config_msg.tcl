...
spawn tail -n 1000 -F logs/db/logs/kwbase.log
...
# Verify that "kwbase.log" is not the file where the server reports
# it just started.
system "if grep -q 'restarted pre-existing node' logs/db/logs/kwbase.log; then false; fi"
# Verify that the last log file does contain the cluster ID.
system "grep -qF '\[config\] clusterID:' logs/db/logs/kwbase.log"
end_test
