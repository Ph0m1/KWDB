...
func findBinaryOrLibrary(binOrLib string, name string) (string, error) {
	...
	if err != nil {
		...
		dirs := []string{
			filepath.Join(gopath, "/src/gitee.com/kwbasedb/kwbase/"),
			filepath.Join(gopath, "/src/gitee.com/kwbasedb/kwbase", binOrLib+suffix),
			filepath.Join(os.ExpandEnv("$PWD"), binOrLib+suffix),
			filepath.Join(gopath, "/src/gitee.com/kwbasedb/kwbase", binOrLib),
		}
...
func initBinariesAndLibraries() {
	...
	kwbaseDefault := "kwbase"
	if !local {
		kwbaseDefault = "kwbase-linux-2.6.32-gnu-amd64"
	}
	var err error
...
// starting and stopping a kwbase cluster on a subset of those machines, and
...
// StopCockroachGracefullyOnNode stops a running kwbase instance on the requested
...
func (c *cluster) StopCockroachGracefullyOnNode(ctx context.Context, node int) error {
	...
	if err := c.RunE(ctx, c.Node(node), "./kwbase quit --insecure --port="+port); err != nil {
		return err
	}
	...
	// However, it serves as a reminder that `./kwbase quit` doesn't yet
...
func (c *cluster) FetchLogs(ctx context.Context) error {
	...
	return contextutil.RunWithTimeout(ctx, "fetch logs", 2*time.Minute, func(ctx context.Context) error {
		...
		if err := c.RunE(ctx, c.All(), "mkdir -p logs/redacted && ./kwbase debug merge-logs --redact logs/*.log > logs/redacted/combined.log"); err != nil {
		...
		return execCmd(
			ctx, c.l, roachprod, "get", c.name, "logs/redacted/combined.log" /* src */, filepath.Join(c.t.ArtifactsDir(), "logs/kwbase.log"),
		)
	})
}
...
func (c *cluster) FetchDebugZip(ctx context.Context) error {
	...
	return contextutil.RunWithTimeout(ctx, "debug zip", 5*time.Minute, func(ctx context.Context) error {
		...
		for i := 1; i <= c.spec.NodeCount; i++ {
			// `./kwbase debug zip` is noisy. Suppress the output unless it fails.
			si := strconv.Itoa(i)
			output, err := execCmdWithBuffer(ctx, c.l, roachprod, "ssh", c.name+":"+si, "--",
				"./kwbase", "debug", "zip", "--url", "{pgurl:"+si+"}", zipName)
			if err != nil {
				c.l.Printf("./kwbase debug zip failed: %s", output)
...
func (c *cluster) CheckReplicaDivergenceOnDB(ctx context.Context, db *gosql.DB) error {
	...
	// see:
	// https://gitee.com/kwbasedb/kwbase/pull/34520
	//
...
FROM
kwdb_internal.check_consistency(true, '', '') as t
...
// Restart restarts the specified kwbase node. It takes a test and, on error,
// calls t.Fatal().
...
// StartE starts kwbase nodes on a subset of the cluster. The nodes parameter
...
// StopE kwbase nodes running on a subset of the cluster. See cluster.Start()
...
func cmdLogFileName(t time.Time, nodes nodeListOption, args ...string) string {
	// Make sure we treat {"./kwbase start"} like {"./kwbase", "start"}.
...
func (m *monitor) Wait() {
	...
	if err := m.WaitE(); err != nil {
		...
		// https://gitee.com/kwbasedb/kwbase/issues/44436
		m.t.Fatal(err)
	}
}
...
func waitForFullReplication(t *test, db *gosql.DB) {
	...
	for ok := false; !ok; time.Sleep(time.Second) {
		if err := db.QueryRow(
			"SELECT min(array_length(replicas, 1)) >= 3 FROM kwdb_internal.ranges",
		).Scan(&ok); err != nil {
			t.Fatal(err)
		}
...
// makeLoadGroups create a loadGroupList that has an equal number of kwbase
