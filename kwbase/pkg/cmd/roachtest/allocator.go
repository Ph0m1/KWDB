...
func registerAllocator(r *testRegistry) {
	runAllocator := func(ctx context.Context, t *test, c *cluster, start int, maxStdDev float64) {
		const fixturePath = `gs://kwbase-fixtures/workload/tpch/scalefactor=10/backup`
...
func printRebalanceStats(l *logger, db *gosql.DB) error {
	...
	// Output standard deviation of the replica counts for all stores.
	{
		var stdDev float64
		if err := db.QueryRow(
			`SELECT stddev(range_count) FROM kwdb_internal.kv_store_status`,
		).Scan(&stdDev); err != nil {
			return err
		}
	...
	{
		rows, err := db.Query(`SELECT store_id, range_count FROM kwdb_internal.kv_store_status`)
		if err != nil {
			return err
		}
		defer rows.Close()
...
func runWideReplication(ctx context.Context, t *test, c *cluster) {
	...
	args := startArgs(
		"--env=KWBASE_SCAN_MAX_IDLE_TIME=5ms",
	...
	zones := func() []string {
		rows, err := db.Query(`SELECT target FROM kwdb_internal.zones`)
		if err != nil {
			t.Fatal(err)
		}
		defer rows.Close()
	...
	countMisreplicated := func(width int) int {
		var count int
		if err := db.QueryRow(
			"SELECT count(*) FROM kwdb_internal.ranges WHERE array_length(replicas,1) != $1",
			width,
		).Scan(&count); err != nil {
			t.Fatal(err)
		}
	...
	numRanges := func() int {
		var count int
		if err := db.QueryRow(`SELECT count(*) FROM kwdb_internal.ranges`).Scan(&count); err != nil {
			t.Fatal(err)
		}
		return count
	}()
...
`
	...
	decom := func(id int) {
		c.Run(ctx, c.Node(1),
			fmt.Sprintf("./kwbase node decommission --insecure --wait=none %d", id))
	}
