...
func (hc *HealthChecker) Runner(ctx context.Context) (err error) {
	...
	for {
		...
		rows, err := db.QueryContext(ctx, `SELECT * FROM kwdb_internal.gossip_alerts ORDER BY node_id ASC, store_id ASC`)
		_ = db.Close()
		if err != nil {
			return err
		}
		...
		r.Add(testSpec{
			...
			Run: func(ctx context.Context, t *test, c *cluster) {
				...
				m.Go(func(ctx context.Context) error {
					...
					c.Run(ctx, c.Node(1), `./kwbase sql --insecure -e "CREATE DATABASE restore2tb"`)
				...
				'gs://kwbase-fixtures/workload/bank/version=1.0.0,payload-bytes=10240,ranges=0,rows=65104166,seed=1/bank'
