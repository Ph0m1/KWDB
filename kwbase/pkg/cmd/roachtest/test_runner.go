	...
	for i := 0; i < parallelism; i++ {
		...
		stopper.RunWorker(ctx, func(ctx context.Context) {
			...
			if err := r.runWorker(
				...
				log.Errorf(ctx, "Worker %d returned with error. Quiescing. Error: %+v", i, err)
...
func (r *testRunner) runTest(
	...
	case <-time.After(timeout):
		...
		// https://gitee.com/kwbasedb/kwbase/issues/45875.
...
func (r *testRunner) collectClusterLogs(ctx context.Context, c *cluster, l *logger) {
	...
	// https://gitee.com/kwbasedb/kwbase/issues/39620
