...
type tpccOptions struct {
	...
	// number of KWDB nodes in the cluster.
	//
...
//
// The command uses `kwbase workload` instead of `workload` so the tpcc
// workload-versions match on release branches. Similarly, the command does not
// specify pgurl to ensure that it is run on a node with a running kwbase
...
func tpccImportCmd(warehouses int, extraArgs ...string) string {
	return fmt.Sprintf("./kwbase workload fixtures import tpcc --warehouses=%d %s",
		warehouses, strings.Join(extraArgs, " "))
}

func setupTPCC(
	ctx context.Context, t *test, c *cluster, opts tpccOptions,
) (kwdbNodes, workloadNode nodeListOption) {
...

	{
		...
		for i, v := range opts.Versions {
			if v == "" {
				...
				c.Put(ctx, binary, "./kwbase", c.Node(i+1))
			}
		}
		c.Put(ctx, kwbase, "./kwbase", regularNodes...)
	}

	// Fixture import needs ./kwbase workload on workloadNode.
	c.Put(ctx, kwbase, "./kwbase", workloadNode)
	c.Put(ctx, workload, "./workload", workloadNode)

	func() {
		db := c.Conn(ctx, 1)
		defer db.Close()
		c.Start(ctx, t, kwdbNodes, startArgsDontEncrypt)
		waitForFullReplication(t, c.Conn(ctx, kwdbNodes[0]))
		switch opts.SetupType {
		case usingImport:
			t.Status("loading fixture")
			c.Run(ctx, kwdbNodes[:1], tpccImportCmd(opts.Warehouses, opts.ExtraSetupArgs))
		case usingInit:
...
func runTPCC(ctx context.Context, t *test, c *cluster, opts tpccOptions) {
	...
	kwdbNodes, workloadNode := setupTPCC(ctx, t, c, opts)
	t.Status("waiting")
...
func runTPCCBench(ctx context.Context, t *test, c *cluster, b tpccBenchSpec) {
	...
	// Fixture import needs ./kwbase workload on loadNodes[0],
	// and if we use haproxy (see below) we need it on the others
	// as well.
	c.Put(ctx, kwbase, "./kwbase", loadNodes)
	...
	const restartWait = 15 * time.Second
	{
		...
		if useHAProxy {
			...
			c.Run(ctx, loadNodes, "./kwbase gen haproxy --insecure --url {pgurl:1}")
...
func registerTPCCBench(r *testRegistry) {
	specs := []tpccBenchSpec{
		...
		// See https://gitee.com/kwbasedb/kwbase/issues/31409 for the next three specs.
		{
			Nodes: 6,
			CPUs:  16,
