...
func computeFollowerReadDuration(ctx context.Context, db *gosql.DB) (time.Duration, error) {
	...
	err := db.QueryRowContext(ctx, "SELECT value FROM kwdb_internal.cluster_settings WHERE variable = 'kv.closed_timestamp.target_duration'").Scan(&targetDurationStr)
	if err != nil {
		return 0, err
	}
	...
	err = db.QueryRowContext(ctx, "SELECT value FROM kwdb_internal.cluster_settings WHERE variable = 'kv.closed_timestamp.close_fraction'").Scan(&closeFraction)
	if err != nil {
		return 0, err
	}
	// target_multiple is a hidden setting which cannot be read from kwdb_internal
