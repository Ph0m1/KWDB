...
func makeSchemaChangeBulkIngestTest(numNodes, numRows int, length time.Duration) testSpec {
	return testSpec{
		...
		Run: func(ctx context.Context, t *test, c *cluster) {
			...
			c.Start(ctx, t, kwdbNodes, startArgs("--env=KWBASE_IMPORT_WORKLOAD_FASTER=true"))

			// Don't add another index when importing.
			cmdWrite := fmt.Sprintf(
				// For fixtures import, use the version built into the kwbase binary
				// so the tpcc workload-versions match on release branches.
				"./kwbase workload fixtures import bulkingest {pgurl:1} --a %d --b %d --c %d --payload-bytes %d --index-b-c-a=false",
