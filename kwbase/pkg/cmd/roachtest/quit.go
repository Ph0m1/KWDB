...
func (q *quitTest) init(ctx context.Context) {
	q.args = startArgs(
		"--env=KWBASE_SCAN_MAX_IDLE_TIME=5ms",                  // iterate fast for rebalancing
		"-a", "--vmodule=store=1,replica=1,replica_proposal=1", // verbosity to troubleshoot drains
	)
	q.c.Put(ctx, kwbase, "./kwbase")
...
func (q *quitTest) restartNode(ctx context.Context, nodeID int) {
	...
	defer db.Close()
	if _, err := db.ExecContext(ctx, `TABLE kwdb_internal.cluster_sessions`); err != nil {
		q.Fatal(err)
	}
}
...
func (q *quitTest) waitForUpReplication(ctx context.Context) {
	...
	err := retry.ForDuration(30*time.Second, func() error {
		...
		row := db.QueryRowContext(ctx, `SELECT min(array_length(replicas, 1)) FROM kwdb_internal.ranges_no_leases`)
		minReplicas := 0
...
func (q *quitTest) checkNoLeases(ctx context.Context, nodeID int) {
	...
	//    https://gitee.com/kwbasedb/kwbase/issues/47100
	//
...
func registerQuitTransfersLeases(r *testRegistry) {
	...
	// Uses 'kwbase quit' which should drain and then request a
	...
	// Uses 'kwbase drain', followed by a non-graceful process
	...
	registerTest("drain", "v20.1.0", func(ctx context.Context, t *test, c *cluster, nodeID int) {
		buf, err := c.RunWithBuffer(ctx, t.l, c.Node(nodeID),
			"./kwbase", "node", "drain", "--insecure", "--logtostderr=INFO",
			fmt.Sprintf("--port={pgport:%d}", nodeID),
		)
		t.l.Printf("kwbase node drain:\n%s\n", buf)
		if err != nil {
			t.Fatal(err)
		}
...
func runQuit(ctx context.Context, t *test, c *cluster, nodeID int, extraArgs ...string) []byte {
	args := append([]string{
		"./kwbase", "quit", "--insecure", "--logtostderr=INFO",
		fmt.Sprintf("--port={pgport:%d}", nodeID)},
		extraArgs...)
	buf, err := c.RunWithBuffer(ctx, t.l, c.Node(nodeID), args...)
	t.l.Printf("kwbase quit:\n%s\n", buf)
	if err != nil {
		t.Fatal(err)
	}
	c.Stop(ctx, c.Node(nodeID),
...
func registerQuitAllNodes(r *testRegistry) {
	// This test verifies that 'kwbase quit' can terminate all nodes
	...
	r.Add(testSpec{
		...
		Run: func(ctx context.Context, t *test, c *cluster) {
			...
			// See: https://gitee.com/kwbasedb/kwbase/issues/48339
