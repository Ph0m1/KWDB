...
type kwdbVersion int

const (
	tpchVecVersion19_2 kwdbVersion = iota
	tpchVecVersion20_1
	tpchVecVersion20_2
)

func toKWDBVersion(v string) (kwdbVersion, error) {
...
func vectorizeOptionToSetting(vectorize bool, version kwdbVersion) string {
	if !vectorize {
		return "off"
	}
	switch version {
...
// skipped).
var queriesToSkipByVersion = map[kwdbVersion]map[int]string{
	tpchVecVersion19_2: {
		5:  "can cause OOM",
		7:  "can cause OOM",
		8:  "can cause OOM",
...
type tpchVecTestCase interface {
	...
	getRunConfig(version kwdbVersion, queriesToSkip map[int]string) tpchVecTestRunConfig
	...
	preTestRunHook(ctx context.Context, t *test, c *cluster, conn *gosql.DB, version kwdbVersion, clusterSetup []string)
	...
	postTestRunHook(ctx context.Context, t *test, c *cluster, conn *gosql.DB, version kwdbVersion)
}
...
func (b tpchVecTestCaseBase) getRunConfig(
	version kwdbVersion, queriesToSkip map[int]string,
...
func (b tpchVecTestCaseBase) postTestRunHook(
	context.Context, *test, *cluster, *gosql.DB, kwdbVersion,
) {
}
...
func (p tpchVecPerfTest) getRunConfig(version kwdbVersion, _ map[int]string) tpchVecTestRunConfig {
...
func (p *tpchVecPerfTest) postTestRunHook(
	ctx context.Context, t *test, c *cluster, conn *gosql.DB, version kwdbVersion,
) {
...
func (b tpchVecBenchTest) getRunConfig(version kwdbVersion, _ map[int]string) tpchVecTestRunConfig {
...
func baseTestRun(
	ctx context.Context, t *test, c *cluster, conn *gosql.DB, version kwdbVersion, tc tpchVecTestCase,
) {
	firstNode := c.Node(1)
...
func (s tpchVecSmithcmpTest) preTestRunHook(
	...
	// bucket at kwbase/smithcmp. The binary shouldn't change much, so it is
	// acceptable.
...
func smithcmpTestRun(
	...
	const (
		...
		configURL  = `https://raw.githubusercontent.com/kwbasedb/kwbase/master/pkg/cmd/roachtest/` + configFile
	)
	firstNode := c.Node(1)
...
func runTPCHVec(
	...
	testRun func(ctx context.Context, t *test, c *cluster, conn *gosql.DB, version kwdbVersion, tc tpchVecTestCase),
) {
	firstNode := c.Node(1)
	...
	version, err := toKWDBVersion(versionString)
	if err != nil {
		t.Fatal(err)
	}
