...
var jepsenNemeses = []struct {
	...
	// https://gitee.com/kwbasedb/kwbase/issues/35599
...
func initJepsen(ctx context.Context, t *test, c *cluster) {
	...
	if err := c.GitClone(
		ctx, t.l,
		"https://github.com/kwbasedb/jepsen", "/mnt/data1/jepsen", "tc-nightly", controller,
	); err != nil {
		t.Fatal(err)
	}
	...
	// Jepsen expects a tarball that expands to kwbase/kwbase
	// (which is not how our official builds are laid out).
	c.Run(ctx, c.All(), "tar --transform s,^,kwbase/, -c -z -f kwbase.tgz kwbase")
...
func runJepsen(ctx context.Context, t *test, c *cluster, testName, nemesis string) {
	...
	t.Status("running")
	run(c, ctx, controller, "rm -f /mnt/data1/jepsen/kwbasedb/store/latest")

	// Install the jepsen package (into ~/.m2) before running tests in
	// the kwbase package. Clojure doesn't really understand
	...
	// Also remove the invoke.log from a previous test, if any.
	{
		err := runE(c, ctx, controller, "bash", "-e", "-c",
			`"cd /mnt/data1/jepsen/jepsen && ~/lein install && rm -f /mnt/data1/jepsen/kwbasedb/invoke.log"`)
		if err != nil {
...
cd /mnt/data1/jepsen/kwbasedb && set -eo pipefail && \
 ~/lein run test \
   --tarball file://${PWD}/kwbase.tgz \
   --username ${USER} \
...
"`, nodesStr, testName, nemesis))
	...
	if testErr != nil {
		...
		run(c, ctx, controller, "tail -n 100 /mnt/data1/jepsen/kwbasedb/invoke.log")
		...
		if err := runE(c, ctx, controller,
			`grep -E "(Oh jeez, I'm sorry, Jepsen broke. Here's why|Caused by)" /mnt/data1/jepsen/kwbasedb/invoke.log -A1 `+
		...
		cmd := exec.CommandContext(ctx, roachprod, "run", c.makeNodes(controller),
			...
			"tar -chj --ignore-failed-read -C /mnt/data1/jepsen/kwbasedb -f- store/latest invoke.log")
		...
		for _, file := range collectFiles {
			cmd := c.LoggedCommand(ctx, roachprod, "get", c.makeNodes(controller),
				"/mnt/data1/jepsen/kwbasedb/store/latest/"+file,
		...
		if anyFailed {
			...
			cmd := c.LoggedCommand(ctx, roachprod, "get", c.makeNodes(controller),
				"/mnt/data1/jepsen/kwbasedb/invoke.log",
