...
func registerKVContention(r *testRegistry) {
	...
	r.Add(testSpec{
		...
		Run: func(ctx context.Context, t *test, c *cluster) {
			...
			args := startArgs("--env=KWBASE_TXN_LIVENESS_HEARTBEAT_MULTIPLIER=600")
...
func registerKVQuiescenceDead(r *testRegistry) {
	r.Add(testSpec{
		...
		Run: func(ctx context.Context, t *test, c *cluster) {
			...
			qps := func(f func()) float64 {
				...
				numInserts := func() float64 {
					...
					if err := db.QueryRowContext(
						ctx, `SELECT value FROM kwdb_internal.node_metrics WHERE name = 'sql.insert.count'`,
					).Scan(&v); err != nil {
						t.Fatal(err)
					}
					return v
			...
			c.Run(ctx, c.Node(nodes), "./kwbase quit --insecure --host=:{pgport:3}")
...
func registerKVGracefulDraining(r *testRegistry) {
	r.Add(testSpec{
		...
		Cluster:     makeClusterSpec(4),
		Skip:        "flaky",
		SkipDetails: "https://gitee.com/kwbasedb/kwbase/issues/53760",
		Run: func(ctx context.Context, t *test, c *cluster) {
			nodes := c.spec.NodeCount - 1
			c.Put(ctx, kwbase, "./kwbase", c.Range(1, nodes))
			c.Put(ctx, workload, "./workload", c.Node(nodes+1))
			...
			// Because we're specifying a --max-rate well less than what kwbase
			...
			Run: func(ctx context.Context, t *test, c *cluster) {
				...
				c.Start(ctx, t, c.Range(1, nodes),
					startArgs(
						...
						"--env", "KWBASE_MEMPROF_INTERVAL=1m KWBASE_DISABLE_QUIESCENCE="+strconv.FormatBool(!item.quiesce),
						"--args=--cache=256MiB",
					))
