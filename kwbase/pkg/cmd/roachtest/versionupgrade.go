...
-- versions of KWDB using their FQNs. Prevents bugs such as #43141, where
...
func runVersionUpgrade(ctx context.Context, t *test, c *cluster, buildVersion version.Version) {
	...
	if false {
		...
		// empty string which uses the local kwbase binary.
		newV := "20.1.7"
	...
	backupStep := func(ctx context.Context, t *test, u *versionUpgradeTest) {
		...
		dest := fmt.Sprintf("nodelocal://01/%d", timeutil.Now().UnixNano())
	...
	u := newVersionUpgradeTest(c,
		...
		// main ./kwbase binary (i.e. the one being tested in this run).
...
func (u *versionUpgradeTest) binaryVersion(ctx context.Context, t *test, i int) roachpb.Version {
	...
	var sv string
	if err := db.QueryRow(`SELECT kwdb_internal.node_executable_version();`).Scan(&sv); err != nil {
		t.Fatal(err)
	}
...
func makeVersionFixtureAndFatal(
	...
	newVersionUpgradeTest(c,
		...
		func(ctx context.Context, t *test, u *versionUpgradeTest) {
			...
			c.Run(ctx, c.All(), kwbase, "debug", "pebble", "db", "checkpoint",
