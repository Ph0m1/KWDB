...
func cdcBasicTest(ctx context.Context, t *test, c *cluster, args cdcTestArgs) {
	...
	if args.cloudStorageSink {
		...
		// kwbase-tmp is a multi-region bucket with a TTL to clean up old
		// data.
		sinkURI = `experimental-gs://kwbase-tmp/roachtest/` + ts
	} else {
	...
	if args.workloadType == tpccWorkloadType {
		...
		tpcc := tpccWorkload{
			...
			// TolerateErrors if kwdbChaos is true; otherwise, the workload will fail
			// if it attempts to use the node which was brought down by chaos.
			tolerateErrors: args.kwdbChaos,
		}
	...
	if args.targetTxnPerSecond > 0.0 {
		verifyTxnPerSecond(
			ctx, c, t, kwdbNodes.randNode(), workloadStart, workloadEnd, args.targetTxnPerSecond, 0.05,
		)
	}
}
...
func runCDCSchemaRegistry(ctx context.Context, t *test, c *cluster) {
	...
	kwdbNodes, kafkaNode := c.Node(1), c.Node(1)
...
func registerCDC(r *testRegistry) {
	...
	r.Add(testSpec{
		Name:    fmt.Sprintf("cdc/kwdb-chaos"),
		Owner:   `cdc`,
		Skip:    "#37716",
		Cluster: makeClusterSpec(4, cpu(16)),
		Run: func(ctx context.Context, t *test, c *cluster) {
...
func (k kafkaManager) install(ctx context.Context) {
	...
	k.c.Run(
		...
		k.nodes,
		fmt.Sprintf(
			`for i in $(seq 1 5); do curl --retry 3 --retry-delay 1 -o /tmp/confluent.tar.gz https://storage.googleapis.com/kwbase-fixtures/tools/confluent-oss-4.0.0-2.11.tar.gz && break || sleep 15; done && tar xvf /tmp/confluent.tar.gz -C %s`,
			folder,
		),
	)
	if !k.c.isLocal() {
...
func (tw *tpccWorkload) install(ctx context.Context, c *cluster) {
	// For fixtures import, use the version built into the kwbase binary so
	// the tpcc workload-versions match on release branches.
	c.Run(ctx, tw.workloadNodes, fmt.Sprintf(
		`./kwbase workload fixtures import tpcc --warehouses=%d --checks=false {pgurl%s}`,
