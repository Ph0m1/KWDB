...
func runDecommission(ctx context.Context, t *test, c *cluster, nodes int, duration time.Duration) {
	...
	waitReplicatedAwayFrom := func(downNodeID int) error {
		...
		for {
			...
			if err := db.QueryRow(
				...
				"SELECT count(*) FROM kwdb_internal.ranges WHERE array_position(replicas, $1) IS NOT NULL",
				downNodeID,
			).Scan(&count); err != nil {
				return err
			}
	...
	waitUpReplicated := func(targetNode, targetNodeID int) error {
		...
		for {
			...
			stmtReplicaCount := fmt.Sprintf(
				`SELECT count(*) FROM kwdb_internal.ranges WHERE array_position(replicas, %d) IS NULL and database_name = 'kv';`, targetNodeID)
	...
	m.Go(func() error {
		getNodeID := func(node int) (int, error) {
			...
			var nodeID int
			if err := dbNode.QueryRow(`SELECT node_id FROM kwdb_internal.node_runtime_info LIMIT 1`).Scan(&nodeID); err != nil {
				return 0, err
			}
			return nodeID, nil
		}
		...
		stop := func(node int) error {
			...
			return c.RunE(ctx, c.Node(node), "./kwbase quit --insecure --host=:"+port)
		}
		...
		decom := func(id int) error {
			...
			return c.RunE(ctx, c.Node(pinnedNode), fmt.Sprintf("./kwbase node decommission --insecure --wait=all --host=:%s %d", port, id))
		}
...
// Header from the output of `kwbase node decommission`.
var decommissionHeader = []string{
	"id", "is_live", "replicas", "is_decommissioning", "membership", "is_draining", "is_ts_migrating",
}

// Footer from the output of `kwbase node decommission`, after successful
// decommission.
...
// Header from the output of `kwbase node status`.
var statusHeader = []string{
	"id", "address", "sql_address", "build", "started_at", "updated_at", "locality", "is_available", "is_live",
}

// Header from the output of `kwbase node status --decommission`.
...
func elideInsecureDeprecationNotice(csvStr string) string {
	...
	// See: https://gitee.com/kwbasedb/kwbase/issues/53404
...
// expectIDsInStatusOut constructs a matching regex for output of `kwbase
