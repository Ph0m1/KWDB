...
// kwbaseToxiWrapper replaces the kwbase binary. It modifies the listening port so
// that the nodes in the cluster will communicate through toxiproxy instead of
// directly.
const kwbaseToxiWrapper = `#!/usr/bin/env bash
set -eu
...
// nodes. On these nodes, the kwbase binary must already have been populated
...
func Toxify(ctx context.Context, c *cluster, node nodeListOption) (*ToxiCluster, error) {
	...
	if err := func() error {
		...
		if err := c.RunE(ctx, node, "mv kwbase kwbase.real"); err != nil {
			return err
		}
		if err := c.PutString(ctx, kwbaseToxiWrapper, "./kwbase", 0755, node); err != nil {
			return err
		}
	...
	for _, i := range node {
		...
		proxy, err := tc.toxClients[i].CreateProxy("kwbase", fmt.Sprintf(":%d", tc.poisonedPort(port)), fmt.Sprintf("127.0.0.1:%d", port))
		if err != nil {
			return nil, errors.Wrap(err, "toxify")
		}
...
// of `./kwbase sql`. This is simplistic and does not perform proper
...
func (tc *ToxiCluster) Measure(ctx context.Context, fromNode int, stmt string) time.Duration {
	...
	b, err := tc.cluster.RunWithBuffer(ctx, tc.cluster.l, tc.cluster.Node(fromNode), "time", "-p", "./kwbase", "sql", "--insecure", "--port", strconv.Itoa(port), "-e", "'"+stmt+"'")
