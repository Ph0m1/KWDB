...
SELECT string_agg(source_id::TEXT || ':' || target_id::TEXT, ',')
  FROM (SELECT * FROM kwdb_internal.gossip_network ORDER BY source_id, target_id)
`
...
func runGossipPeerings(ctx context.Context, t *test, c *cluster) {
	...
	for i := 1; timeutil.Now().Before(deadline); i++ {
		...
		// https://gitee.com/kwbasedb/kwbase/issues/48005
		time.Sleep(3 * time.Second)
	}
}

func runGossipRestart(ctx context.Context, t *test, c *cluster) {
	t.Skip("skipping flaky acceptance/gossip/restart", "https://gitee.com/kwbasedb/kwbase/issues/48423")
...
func runGossipRestartNodeOne(ctx context.Context, t *test, c *cluster) {
	args := startArgs("--env=KWBASE_SCAN_MAX_IDLE_TIME=5ms", "--encrypt=false")
	c.Put(ctx, kwbase, "./kwbase")
	...
	if err := retry.ForDuration(30*time.Second, func() error {
		const query = `SELECT count(*) FROM kwdb_internal.gossip_nodes`
		var count int
...
`
	...
	// current infrastructure which doesn't know about kwbase nodes started on
	// non-standard ports.
	g := newGossipUtil(ctx, c)
...
func runCheckLocalityIPAddress(ctx context.Context, t *test, c *cluster) {
	...
	for i := 1; i <= c.spec.NodeCount; i++ {
		...
		rows, err := db.Query(
			`SELECT node_id, advertise_address FROM kwdb_internal.gossip_nodes`,
		)
		if err != nil {
			t.Fatal(err)
		}
