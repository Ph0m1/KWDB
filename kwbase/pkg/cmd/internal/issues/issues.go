...
const (
	...
	tagsEnv                = "TAGS"
	goFlagsEnv             = "GOFLAGS"
	// CockroachPkgPrefix is the kwdb package prefix.
	CockroachPkgPrefix = "gitee.com/kwbasedb/kwbase/pkg/"
...
var (
	githubUser = "kwbasedb" // changeable for testing
	githubRepo = "kwbase"   // changeable for testing
)
...
{{end}}
[See this test on roachdash](https://roachdash.kwdb.dev/?filter={{urlquery "status:open t:.*" .TestName ".*" }}&sort=title&restgroup=false&display=lastcommented+project)
...
func (p *poster) execTemplate(ctx context.Context, tpl string, data TemplateData) (string, error) {
	tlp, err := template.New("").Funcs(template.FuncMap{
		...
		"commiturl": func(sha string) string {
			return fmt.Sprintf("https://gitee.com/kwbasedb/kwbase/commits/%s", sha)
		},
