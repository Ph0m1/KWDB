...
func main() {
	...
	kwdb, err := build.Import(fmt.Sprintf("github.com/%s/%s", org, repo), "", build.FindOnly)
	if err != nil {
		log.Fatal(err)
	}
	...
	if target == "checkdeps" {
		...
		if vendorChanged {
			...
			// Check for diffs.
			var foundDiff bool
			for _, dir := range []string{filepath.Join(kwdb.Dir, "vendor"), kwdb.Dir} {
		...
		if len(pkgs) > 0 {
			for name, pkg := range pkgs {
				...
				cmd.Env = append(os.Environ(), "KWBASE_NIGHTLY_STRESS=true")
				cmd.Dir = kwdb.Dir
				cmd.Stdout = os.Stdout
				cmd.Stderr = os.Stderr
				log.Println(cmd.Args)
