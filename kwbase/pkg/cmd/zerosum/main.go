...
import (
	...
	"gitee.com/kwbasedb/kwbase/pkg/acceptance/localcluster"
...
func (z *zeroSum) worker() {
	...
	for {
		...
		err := kwdb.ExecuteTx(context.Background(), db, nil, func(tx *gosql.Tx) error {
...
func (z *zeroSum) rangeInfo() (int, []int) {
	...
	rows, err := db.Query(`SELECT array_length(replicas, 1) FROM kwdb_internal.ranges`)
	if err != nil {
		z.maybeLogError(err)
		return -1, replicas
	}
