...
import (
	...
	cld "gitee.com/kwbasedb/kwbase/pkg/cmd/roachprod/cloud"
	"gitee.com/kwbasedb/kwbase/pkg/cmd/roachprod/config"
	rperrors "gitee.com/kwbasedb/kwbase/pkg/cmd/roachprod/errors"
	"gitee.com/kwbasedb/kwbase/pkg/cmd/roachprod/install"
	"gitee.com/kwbasedb/kwbase/pkg/cmd/roachprod/ssh"
	"gitee.com/kwbasedb/kwbase/pkg/cmd/roachprod/ui"
	"gitee.com/kwbasedb/kwbase/pkg/cmd/roachprod/vm"
	"gitee.com/kwbasedb/kwbase/pkg/cmd/roachprod/vm/aws"
	_ "gitee.com/kwbasedb/kwbase/pkg/cmd/roachprod/vm/azure"
	"gitee.com/kwbasedb/kwbase/pkg/cmd/roachprod/vm/gce"
	"gitee.com/kwbasedb/kwbase/pkg/cmd/roachprod/vm/local"
	"gitee.com/kwbasedb/kwbase/pkg/util/flagutil"
...
Examples:
  ...
  roachprod sql local:2 -- -e "select * from kwdb_internal.node_runtime_info"
  roachprod stop local
  roachprod wipe local
  roachprod destroy local

The above commands will create a "local" 3 node cluster, start a kwbase
...
var (
	...
	clusterType       = "kwbase"
	secure            = false
	nodeEnv           = []string{"KWBASE_ENABLE_RPC_COMPRESSION=false"}
	nodeArgs          []string
	tag               string
	external          = false
	adminurlOpen      = false
...
starting kwbase nodes and "roachprod {run,ssh}" for running arbitrary
...
The --details flag adjusts the output format to include per-node details:
  ...
  marc-test: [aws gce] 5h33m57s remaining
    marc-test-0001	marc-test-0001.us-east1-b.kwbase-ephemeral	10.142.0.18	35.229.60.91
    marc-test-0002	marc-test-0002.us-east1-b.kwbase-ephemeral	10.142.0.17	35.231.0.44
    marc-test-0003	marc-test-0003.us-east1-b.kwbase-ephemeral	10.142.0.19	35.229.111.100
    marc-test-0004	marc-test-0004.us-east1-b.kwbase-ephemeral	10.142.0.20	35.231.102.125
  Syncing...
...
kwbase software is installed by default on a newly created cluster.

The --args and --env flags can be used to pass arbitrary command line flags and
environment variables to the kwbase process.
` + tagHelp + `
...
If the KWBASE_DEV_LICENSE environment variable is set the enterprise.license
...
shutdown kwbase. The --wait flag causes stop to loop waiting for all
...
The "init" command bootstraps the cluster (using "kwbase init"). It also sets
...
var monitorCmd = &cobra.Command{
	...
	Long: `Monitor the status of kwbase nodes in a cluster.
	...
	Run: wrap(func(cmd *cobra.Command, args []string) error {
		...
		case "kwbase":
			sha, err := install.StageRemoteBinary(
				c, applicationName, "kwbase/kwbase", versionArg, debugArch,
			)
			if err != nil {
				return err
			}
			...
			for _, library := range []string{"libgeos", "libgeos_c"} {
				if err := install.StageOptionalRemoteLibrary(
					c,
					library,
					fmt.Sprintf("kwbase/lib/%s", library),
					sha,
					debugArch,
					libExt,
				); err != nil {
...
var sqlCmd = &cobra.Command{
	...
	Short: "run `kwbase sql` on a remote cluster",
	Long:  "Run `kwbase sql` on a remote cluster.\n",
	Args:  cobra.MinimumNArgs(1),
	Run: wrap(func(cmd *cobra.Command, args []string) error {
		c, err := newCluster(args[0])
		if err != nil {
		...
		kwbase, ok := c.Impl.(install.Cockroach)
		if !ok {
			return errors.New("sql is only valid on kwbase clusters")
		}
		return kwbase.SQL(c, args[1:])
	}),
}
...
func main() {
	...
	logsCmd.Flags().StringVar(
		&logsProgramFilter, "logs-program", "^kwbase$", "regular expression of the name of program in log files to search")
	...
	for _, cmd := range []*cobra.Command{
		...
		case startCmd:
			...
			cmd.Flags().StringVarP(
				&clusterType, "type", "t", clusterType, `cluster type ("kwbase" or "cassandra")`)
			cmd.Flags().BoolVar(
		...
		case sqlCmd:
			cmd.Flags().StringVarP(
				...
				"the remote kwbase binary to use")
			fallthrough
