...
import (
	...
	clog "gitee.com/kwbasedb/kwbase/pkg/util/log"
...
func (c *SyncedCluster) GetInternalIP(index int) (string, error) {
	...
	if err != nil {
		return "", kwdberrors.Wrapf(err, "GetInternalIP: failed dial %s:%d", c.Name, index)
	}
	defer session.Close()
...
sudo rm -fr /mnt/data*/{auxiliary,local,tmp,cassandra,kwbase,kwbase-temp*,mongo-data} &&
sudo rm -fr logs &&
`
			if !preserveCerts {
...
func (c *SyncedCluster) Status() {
	...
	c.Parallel(display, len(c.Nodes), 0, func(i int) ([]byte, error) {
		...
		binary := kwbaseNodeBinary(c, c.Nodes[i])
...
if [ -n "${out}" -a -n "${vers}" ]; then
  echo ${out} | sed "s/kwbase/kwbase-${vers}/g"
else
  echo ${out}
fi
`
		...
		var msg string
		if err != nil {
			return nil, kwdberrors.Wrapf(err, "~ %s\n%s", cmd, out)
		}
...
// NodeMonitorInfo is a message describing a kwbase process' status.
...
// (emitting new information as the status of the kwbase process changes).
//
...
func (c *SyncedCluster) Monitor(ignoreEmptyNodes bool, oneShot bool) chan NodeMonitorInfo {
	...
	for i := range nodes {
		...
		go func(i int) {
			...
			// On each monitored node, we loop looking for a kwbase process. In
	...
	c.Parallel(display, len(nodes), 0, func(i int) ([]byte, error) {
		...
		if stream {
			...
			if errors[i] != nil {
				...
				err = kwdberrors.WithDetail(errors[i], detailMsg)
		...
		if err != nil {
			detailMsg := fmt.Sprintf("Node %d. Command with error:\n```\n%s\n```\n", nodes[i], cmd)
			err = kwdberrors.WithDetail(err, detailMsg)
			err = rperrors.ClassifyCmdError(err)
			errors[i] = err
			msg += fmt.Sprintf("\n%v", err)
		}
...
func (c *SyncedCluster) Wait() error {
	...
	c.Parallel(display, len(c.Nodes), 0, func(i int) ([]byte, error) {
		...
		errs[i] = kwdberrors.New("timed out after 5m")
		return nil, nil
	})
	...
	if foundErr {
		return kwdberrors.New("not all nodes booted successfully")
	}
	return nil
}
...
`
		...
		if err := sess.Run(cmd); err != nil {
			return nil, kwdberrors.Wrapf(err, "%s: stderr:\n%s", cmd, stderr.String())
		}
		sshTar = stdout.Bytes()
		return nil, nil
	})
	...
	c.Parallel("distributing ssh key", len(nodes), 0, func(i int) ([]byte, error) {
		...
		if out, err := sess.CombinedOutput(cmd); err != nil {
			return nil, kwdberrors.Wrapf(err, "%s: output:\n%s", cmd, out)
		}
		return nil, nil
	})
	...
	c.Parallel("retrieving hosts", len(c.Nodes), 0, func(i int) ([]byte, error) {
		for j := 0; j < 20 && ips[i] == ""; j++ {
			...
			if err != nil {
				return nil, kwdberrors.Wrapf(err, "pgurls")
			}
...
func (c *SyncedCluster) DistributeCerts() {
	...
	if !c.IsLocal() {
		...
		c.Parallel("", len(nodes), 0, func(i int) ([]byte, error) {
			...
			return nil, kwdberrors.Wrapf(err, "IPs")
		})
	}
...
`, kwbaseNodeBinary(c, 1), strings.Join(nodeNames, " "))
		if out, err := sess.CombinedOutput(cmd); err != nil {
...
// `kwbase debug merge-logs <dest>/*/*` with the optional flag for filter.
...
func (c *SyncedCluster) Logs(
	...
	rsyncNodeLogs := func(ctx context.Context, idx int) error {
		...
		if err := cmd.Run(); err != nil {
			...
			return kwdberrors.Errorf("failed to rsync from %v to %v: %v\n%s",
	...
	mergeLogs := func(ctx context.Context, prev, t time.Time) error {
		cmd := exec.CommandContext(ctx, "kwbase", "debug", "merge-logs",
			dest+"/*/*",
		...
		if err := cmd.Run(); err != nil && ctx.Err() == nil {
			return fmt.Errorf("failed to run kwbase debug merge-logs:%v\n%v",
				err, errBuf.String())
		}
		return nil
	}
	...
	if err := os.MkdirAll(dest, 0755); err != nil {
		return kwdberrors.Errorf("failed to create destination directory: %v", err)
	}
	...
	go func() { <-ch; cancel() }()
	// TODO(ajwerner): consider SIGHUP-ing kwbase before the rsync to avoid the delays
	prev := from
	if prev.IsZero() {
	...
	for to.IsZero() || prev.Before(to) {
		...
		if err := rsyncLogs(ctx); err != nil {
			return kwdberrors.Errorf("failed to sync logs: %v", err)
		}
		...
		go func(i int) {
			...
			if c.IsLocal() {
				...
				copy = func(src, dest string, info os.FileInfo) error {
					...
					// See:
					// https://gitee.com/kwbasedb/kwbase/issues/44843
					mode := info.Mode() | 0444
					if info.IsDir() {
			...
			if err == nil {
				...
				// The KWDB process intentionally sets a 0007 umask (resulting in
...
func (c *SyncedCluster) pghosts(nodes []int) map[int]string {
	...
	c.Parallel("", len(nodes), 0, func(i int) ([]byte, error) {
		var err error
		ips[i], err = c.GetInternalIP(nodes[i])
		return nil, kwdberrors.Wrapf(err, "pghosts")
	})
...
func (c *SyncedCluster) Init() {
	...
	if err != nil {
		log.Fatalf("unable to retrieve kwbase version: %v", err)
	}
