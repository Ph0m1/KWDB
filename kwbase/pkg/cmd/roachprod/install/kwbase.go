...
func kwbaseNodeBinary(c *SyncedCluster, node int) string {
	...
	if err != nil {
		...
		// look in the kwbase repo.
		gopath := os.Getenv("GOPATH")
		if gopath == "" {
			return config.Binary
		}
		path = gopath + "/src/gitee.com/kwbasedb/kwbase/" + config.Binary
		var err2 error
...
func getCockroachVersion(c *SyncedCluster, node int) (*version.Version, error) {
	...
	cmd := kwbaseNodeBinary(c, node) + " version"
	...
	if len(matches) != 2 {
		return nil, fmt.Errorf("unable to parse kwbase version output:%s", out)
	}
...
func (r Cockroach) Start(c *SyncedCluster, extraArgs []string) {
	...
	c.Parallel("", len(nodes), parallelism, func(nodeIdx int) ([]byte, error) {
		...
		// NB: if kwbase started successfully, we ignore the output as it is
		...
		//   `generateStartArgs`),which prompts KWDB to auto-initialize. For
...
func (r Cockroach) SQL(c *SyncedCluster, args []string) error {
	if len(args) == 0 || len(c.Nodes) == 1 {
		...
		binary := kwbaseNodeBinary(c, c.Nodes[0])
	...
	c.Parallel(display, len(c.Nodes), 0, func(nodeIdx int) ([]byte, error) {
		...
		cmd += kwbaseNodeBinary(c, c.Nodes[nodeIdx]) + " sql --url " +
...
type kwdbInstallHelper struct {
	c *SyncedCluster
	r Cockroach
}

func (h *kwdbInstallHelper) startNode(
...
func (h *kwdbInstallHelper) generateStartCmd(
	nodeIdx int, extraArgs []string, vers *version.Version,
) (string, error) {
	...
	// For everything else we'll fall back to using `kwbase start`.
	var startCmd string
	...
	binary := kwbaseNodeBinary(h.c, nodes[nodeIdx])
	...
	cmd := fmt.Sprintf(`
		...
		GOTRACEBACK=crash KWBASE_SKIP_ENABLING_DIAGNOSTIC_REPORTING=1 %[5]s \
		%[6]s %[7]s %[8]s >> %[1]s/kwbase.stdout.log \
		                 2>> %[1]s/kwbase.stderr.log \
			|| (x=$?; cat %[1]s/kwbase.stderr.log; exit $x)`,
		logDir,                  // [1]
		keyCmd,                  // [2]
		nodes[nodeIdx],          // [3]
		h.c.Tag,                 // [4]
...
func (h *kwdbInstallHelper) generateStartArgs(
	nodeIdx int, extraArgs []string, vers *version.Version,
) ([]string, error) {
	var args []string
	nodes := h.c.ServerNodes()
	...
	if !h.useStartSingleNode(vers) {
		// --join flags are unsupported/unnecessary in `kwbase
		...
		// node 1 also points to itself, and an explicit `kwbase init` is
		// needed.
...
func (h *kwdbInstallHelper) initializeCluster(nodeIdx int) (string, error) {
	nodes := h.c.ServerNodes()
...
func (h *kwdbInstallHelper) setClusterSettings(nodeIdx int) (string, error) {
	nodes := h.c.ServerNodes()
...
func (h *kwdbInstallHelper) generateClusterSettingCmd(nodeIdx int) string {
	nodes := h.c.ServerNodes()
	license := envutil.EnvOrDefaultString("KWBASE_DEV_LICENSE", "")
	if license == "" {
		fmt.Printf("%s: KWBASE_DEV_LICENSE unset: enterprise features will be unavailable\n",
			h.c.Name)
	}
	...
	clusterSettingCmd += fmt.Sprintf(`
		if ! test -e %s ; then
			KWBASE_CONNECT_TIMEOUT=0 %s sql --url %s -e "
...
func (h *kwdbInstallHelper) generateInitCmd(nodeIdx int) string {
	nodes := h.c.ServerNodes()
	...
	initCmd += fmt.Sprintf(`
		if ! test -e %[1]s ; then
			KWBASE_CONNECT_TIMEOUT=0 %[2]s init --url %[3]s && touch %[1]s
		fi`, path, binary, url)
	return initCmd
}

func (h *kwdbInstallHelper) generateKeyCmd(nodeIdx int, extraArgs []string) string {
	if !StartOpts.Encrypt {
		return ""
	}
...
func (h *kwdbInstallHelper) useStartSingleNode(vers *version.Version) bool {
...
func (h *kwdbInstallHelper) distributeCerts() {
...
func (h *kwdbInstallHelper) shouldAdvertisePublicIP() bool {
...
func (h *kwdbInstallHelper) getEnvVars() string {
	var buf strings.Builder
	for _, v := range os.Environ() {
		if strings.HasPrefix(v, "KWBASE_") {
			if buf.Len() > 0 {
				buf.WriteString(" ")
			}
			buf.WriteString(v)
...
func (h *kwdbInstallHelper) run(nodeIdx int, cmd string) (string, error) {
	nodes := h.c.ServerNodes()
