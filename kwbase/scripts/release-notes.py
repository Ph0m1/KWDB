...
class P:
    def __init__(self, name, addr):
        ...
        self.kwdb = '@cockroachlabs.com' in addr
        if self.kwdb:
            kwdb_folk.add(self)
    def __repr__(self):
...
if not os.path.exists('AUTHORS'):
    ...
    with open('AUTHORS', 'r') as f:
        for line in f.readlines():
            ...
            if p is None:
                continue
            p.kwdb = '@cockroachlabs.com' in line
            if p.kwdb:
                kwdb_folk.add(p)
...
ext_contributors = individual_authors - kwdb_folk
...
<div id="os-tabs" class="clearfix">
    <a href="https://binaries.kwbasedb.com/kwbase-""" + current_version + """.darwin-10.9-amd64.tgz"><button id="mac" data-eventcategory="mac-binary-release-notes">Mac</button></a>
    <a href="https://binaries.kwbasedb.com/kwbase-""" + current_version + """.linux-amd64.tgz"><button id="linux" data-eventcategory="linux-binary-release-notes">Linux</button></a>
    <a href="https://binaries.kwbasedb.com/kwbase-""" + current_version + """.windows-6.2-amd64.zip"><button id="windows" data-eventcategory="windows-binary-release-notes">Windows</button></a>
    <a href="https://binaries.kwbasedb.com/kwbase-""" + current_version + """.src.tgz"><button id="source" data-eventcategory="source-release-notes">Source</button></a>
</div>
""")
...
~~~shell
$ docker pull kwbasedb/kwbase""" + ("-unstable:" if "-" in current_version else ":") + current_version + """
~~~
""")
    print()
...
for pr in sorted(seenprs):
    print("[%s]: https://gitee.com/kwbasedb/kwbase/pull/%s" % (pr, pr[1:]))
for sha in sorted(seenshas):
    print("[%s]: https://gitee.com/kwbasedb/kwbase/commit/%s" % (sha, sha))
print()
